...
- Polaris 本機 toolchain：Node >= 22.12.0、pnpm 10.10.0、Python 3、Playwright、Mockoon 與 docs viewer。Clone 後先跑 `bash scripts/polaris-toolchain.sh doctor --required`；若缺工具，跑 `bash scripts/polaris-toolchain.sh install --required` 後再重跑 doctor。
...
Root `pnpm` scripts 是常用 framework 指令的薄入口，不取代 `scripts/*.sh`；script manifest 與 command catalog 仍是治理來源：
