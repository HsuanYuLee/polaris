...
- **Worktree isolation for batch implementation**: each sub-agent in `engineering` batch mode must use `isolation: "worktree"` to prevent file overwrites or git conflicts during parallel development. Not required for single-ticket sub-agents, but recommended when multiple files are changed
...
| **Execute / Fix** | `"sonnet"` | Implementation sub-agent, engineering revision worktree, CI fixes, rebase conflict |
