...
| `branch-switch-requires-worktree` | Any op that mutates main checkout HEAD/branch/working tree (`git checkout`/`switch`/`pull` rebase/`stash`+switch) must run in a `git worktree add` copy. Universal default — predates and covers batch/revision/planning sub-cases. See `rules/sub-agent-delegation.md` § Branch switching = worktree | `git checkout <branch>` / `git switch` / `git checkout -b` / `git pull` in main checkout path executed by Strategist, skill, or sub-agent; or sub-agent dispatched without `isolation: "worktree"` when its flow will switch branches | High |
...
| `codecov-patch-gate` | engineer-delivery-flow Step 2a: if repo has a Codecov patch gate (`codecov.yml` contains `type: patch` OR workflow references `codecov/patch`), engineering must run coverage on changed source files and write evidence before push. TDD smart judgment CANNOT豁免 based on 「改動小」 when patch gate exists | Push succeeds but CI `codecov/patch/*` fails on untested new lines; OR tdd-smart-judgment report omits `Coverage gate:` line in a repo with a gate; OR source file changes merged without corresponding test additions in a gated repo | **Critical** |
...
| `revision-r5-mandatory` | Revision mode R5（重跑完整驗收）is mandatory for ALL revision paths — code drift fix, rebase-only, QA-reported. Push without behavioral verification is never allowed. Rebase-only（R2 empty signals）must still run R5 before push | `git push` in revision mode without prior behavioral verification (Layer B) output in conversation; or Strategist rationalizes "只是 rebase，沒改 code" to skip R5 | **Critical** |
...
|----|------|-------------|--------|
| `verification-evidence-required` | `gh pr create` blocked unless `/tmp/polaris-verified-{TICKET}.json` exists with valid ticket, timestamp (< 4h), non-empty results, and `runtime_contract` metadata. If `runtime_contract.level=runtime`, gate additionally requires live `runtime_verify_target`, `verify_command_url`, and host alignment | PreToolUse hook on Bash, exit 2 to block | `scripts/verification-evidence-gate.sh` |
...
| `coverage-evidence-required` | `git push` blocked in repos with Codecov patch gate (`codecov.yml` `type: patch` or workflow `codecov/patch`) unless `/tmp/polaris-coverage-{branch-slug}.json` exists with `status: PASS` and timestamp < 4h. Bypass: `POLARIS_SKIP_COVERAGE=1` or `wip:` HEAD commit prefix. Skipped for main/develop. Writer: `scripts/write-coverage-evidence.sh`. Rationale: KB2CW-3847 shipped untested `useFetch key` change that passed local quality but failed `codecov/patch/main-core` — this gate forces coverage BEFORE push, not discovered AFTER | PreToolUse hook on Bash, exit 2 to block | `.claude/hooks/coverage-gate.sh` |
...
12. `verification-evidence-required` / `quality-evidence-required` / `coverage-evidence-required` / `test-sequence-warning` / `post-compact-context-restore` / `stop-todo-check` / `auto-compact-window` (deterministic hooks — low audit priority because hooks enforce automatically)
