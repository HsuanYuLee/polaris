...
| `post-implementation-flow-gap-audit` | After implementation verification and before delivery handoff, explicitly audit bypass/fallback/false-pass/ignored-runtime-artifact risks. Framework/config/release-flow changes must include a deterministic validator or `--no-ignore` equivalent scan where local ignored state matters | Final/handoff says complete without mentioning bypass/fallback/false-pass disposition; release verification uses tracked-only scans for a migration involving ignored artifacts; `NO_*_CONFIGURED` or `config: (none)` is treated as pass without a deliberate skip reason | **Critical** |
...
| `repo-knowledge-to-handbook-not-feedback` | Repo-specific knowledge (code patterns, API conventions, test strategies, env setup) belongs in workspace-owned handbook sub-files, not feedback memories | New feedback memory created for repo-specific knowledge that should be in `{company}/polaris-config/{project}/handbook/*.md` | High |
...
| `handbook-injection-in-subagent` | Implementation sub-agent dispatch prompts must include handbook reading instruction for both company and repo handbooks. Repo handbook source is `{company}/polaris-config/{project}/handbook/index.md` + linked child docs; repo-local `.claude/rules/handbook/` is legacy/native compatibility only | Sub-agent writes code without prior Read of repo handbook index and linked child docs; OR reads only a repo-local compatibility overlay when workspace-owned handbook exists; OR violates coding convention already documented in company/repo handbook | High |
...
| `handbook-config-machine-source` | Script-consumable project runtime / test / URL mapping / key library fields come from `{company}/polaris-config/{project}/handbook/config.yaml`; narrative Markdown is rationale only. `start-test-env.sh` uses handbook config first, workspace-config fallback only when config is absent, and duplicate field conflicts fail loud | Script parses handbook Markdown for machine fields; or silently chooses workspace-config over existing handbook config; or treats workspace-config / handbook drift as pass | High |
...
| `verify-fallback-declared` | Verify fallback is allowed only when task.md explicitly declares `## Verify Fallback Command`; `run-verify-command.sh` must execute the primary command first and write evidence with `verification_mode=fallback`, primary exit, and fallback exit | Agent manually runs a different grep/build output path after Verify Command fails, or reports AC-equivalent PASS without `run-verify-command.sh` fallback evidence | **Critical** |
| `stack-replay-manifest-required` | Manual stacked-branch rebuilds that include cherry-pick/drop/reorder/force-push must produce a Stack Replay Manifest and pass `scripts/stack-replay-manifest-check.sh` before push/closeout | Agent force-pushes a rebuilt stacked PR based on commit judgment without a machine-checked included/excluded commit ledger | High |
| `pr-review-thread-disposition-required` | Existing open PR revision/rebase/stack rewrite must fetch GitHub reviewThreads and pass `scripts/pr-review-thread-disposition-gate.sh`; every unresolved, not-outdated thread needs `fixed` / `reply_only` / `not_actionable` / `deferred_with_reason` disposition before push/closeout | Agent relies on approval/reviewDecision/top-level comments and misses unresolved inline review threads; or force-pushes without a thread disposition manifest | **Critical** |
...
| `epic-folder-structure-compliance` | All Epic artifacts (mockoon fixtures, VR baselines, verification evidence, task.md, refinement) must be written to `specs/{EPIC}/` per `references/epic-folder-structure.md`. No Epic data in framework config roots or other locations | Skill writes mockoon fixtures to a config root or verification evidence to `/tmp` only (without local copy in `specs/{EPIC}/verification/`) | Medium |
| `pre-work-rebase` | engineering must rebase before development (§ 4.5) / before revision (§ R0), not only at delivery flow Step 5. Rebase includes cascade when base is a feature branch. Conflict → stop, do not start coding. Deterministic backup: existing PR pushes require `revision-rebase.sh` head-bound evidence via `gate-revision-rebase.sh` | engineering starts coding (Edit/Write on source files) without prior `git rebase` in the same session; or rebase only appears after code changes (at delivery flow Step 5) with no pre-work rebase; or gate blocks missing R0 evidence and agent bypasses it | High |
