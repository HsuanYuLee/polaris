...
| `breakdown-escalation-intake` | breakdown reads sidecar (highest `count` for the lineage), may re-classify flavor, must log `accepted flavor: X` or `re-classified to Y: reason`; reuses Planning Path user-confirmation gate before any task.md edit / JIRA write; marks sidecar `processed: true` post-confirm | breakdown session updates task.md from a sidecar without an explicit accepted/re-classified line; OR new user-confirmation gate invented instead of reusing Planning Path Step 8/11; OR sidecar not marked `processed: true` after task.md / JIRA writes complete | Medium |
