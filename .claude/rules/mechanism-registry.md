...
| dp-keyed-source-symmetry | contract_pointer | 新增 resolver / reader / validator / container-enum surface 時只處理 `design-plans/DP-*`、`resolve_by_dp` 或 `DP-*` composite，而沒有 companies/JIRA-Epic 對稱分支，導致 DP source 可走、JIRA Epic source fail-closed 或漂移 | `scripts/lint-dp-keyed-source-symmetry.sh` 對 DP-keyed resolver/reader/container logic fail-closed exit 2 + `POLARIS_DP_KEYED_SOURCE_ASYMMETRY`；合法 DP-only surface 必須登記在 `scripts/lib/spec-source-parity-allowlist.txt` `[resolver-logic]`。`scripts/validate-spec-source-parity.sh` 將此 lint 作為 resolver-logic-layer face 納入 aggregate parity gate。selftest：`scripts/selftests/lint-dp-keyed-source-symmetry-selftest.sh` 與 `scripts/selftests/validate-spec-source-parity-selftest.sh` |
...
| evidence-bearing-command-direct-call | contract_pointer | evidence-bearing 命令（negative `! rg`、`rg --pcre2` / `-P`、`git apply`、`git diff --no-index`、`cksum` / `sha*sum` 兩檔比對）以**直接 Bash tool call** 跑來蒐集 DP 驗證證據——其 stdout / exit code 就是驗證結論，卻被透明 command-rewrite proxy（如 rtk）改寫成假性 PASS / 假性 identical（DP-356 incident：本機 `rg` 被改寫成 BSD grep，`! rg` 反轉成 exit 0 假 PASS；rtk diff proxy 對兩個不同檔報 identical）。A 類 gateable 半邊由 lint fail-closed；殘餘「sub-agent 為臨時蒐證直接打 tool call」無持久 callsite（B 類，對齊 DP-299 prose-vs-gate A/B 分類） | A 類可 gate 半邊：`scripts/lint-evidence-command-direct-call.sh` 指向 evidence-gathering target 時對枚舉 pattern fail-closed exit 2 + `POLARIS_EVIDENCE_DIRECT_CALL`，並不誤標一般 dev grep/diff（AC-NEG1）、不誤標已走 immune 路徑者（routed through `scripts/run-evidence-command.sh` / 絕對 binary 路徑，AC-NEG2）；selftest：`scripts/selftests/lint-evidence-command-direct-call-selftest.sh`（8 positive + adversarial flag-order/變數內插 + NEG1/NEG2 + mutation guard）。lint 是 fixture-proven、指向 evidence target 用，**刻意不**作為 whole-repo `--self-check` release gate（既有 framework script 以 rg/git/checksum 做自身 control flow，非 DP 驗證證據，blanket 掃會誤標）。B 類殘餘（sub-agent ad-hoc 蒐證、無 callsite）由 post-task reflection 事後偵測，發現時寫 feedback memory（帶本 mechanism ID）；不偽裝成「已全 gate」。immune 執行 helper 為 `scripts/run-evidence-command.sh`（DP-356 T1） |
| active-thread-writer-trigger-gap | contract_pointer | parking / session switch 發生時 anchor 寫入端（`scripts/update-active-thread.sh`）未被觸發：唯一 trigger `stop-active-thread-reminder.sh` 為 advisory-only（永遠 exit 0、never blocks），無 skill callsite 把 parking 綁到 anchor write，導致 parked work 逃逸到 ad-hoc `/tmp`（DP-300 incident：本 session anchor 仍停在前一條 `/auto-pass DP-298`）。A 類 gateable invariant，不得退回 advisory prose。DP-314 補強並行 session 偽陽：另一 session 在本 session block 後寫入更新 baseline 不得讓本 session 永遠 block，且過期 baseline 不得無限期觸發 | `.claude/hooks/stop-active-thread-reminder.sh` 升為 fail-closed Stop gate（DP-300 T2）：incomplete-work 訊號 AND 本 session 未刷新 anchor 兩者皆成立才 block，提示刷新指令；無 parked work / 明確 bypass 時 exit 0 不誤擋。DP-314 T1 加：(D1) block 時寫 per-session block-state（`$RUNTIME_DIR/stop-gate-block-state/<session_id>.json` 記 `blocked_at_epoch`），同 session anchor 在該時間戳之後刷新即放行（即使並行 session 已寫更新 baseline），但 anchor 未刷新時 block-state 不是無條件通行證仍 block；(D2) baseline mtime 超過 freshness window（預設 7 天，`POLARIS_STOP_GATE_BASELINE_WINDOW_DAYS` 覆寫）不算 incomplete-work 訊號；缺 session_id / 壞 payload / 缺 runtime dir / 損毀 block-state JSON 一律 fail-open exit 0。`scripts/selftests/stop-active-thread-reminder-selftest.sh`（AC1 race / AC2 window / AC4 fail-open / AC-NEG1 block / AC-NEG2 bypass+gate / AC-NEG3 block-state-not-pass / loop guard 全態）。DP-353 T1 補 remove-half（closeout-driven recycle）：`scripts/archive-spec.sh` 在 single-arg 與 `--sweep --apply` 兩條路徑成功搬檔後，derive canonical source key（DP container→`DP-NNN`、JIRA-Epic container→basename ticket key）並 best-effort 呼叫 `scripts/update-active-thread.sh --key <key> --done` 回收同名 parked thread；精確 `<!-- thread:<key> -->` 比對（prefix-similar 如 DP-30 vs DP-303 不誤刪）、anchor 缺失 / writer 非零 / root 不可解析一律 warn 不影響 archive 主 exit、`--dry-run` 不回收、無同名 thread 時 anchor byte-identical。selftest：`scripts/selftests/archive-spec-thread-recycle-selftest.sh`（AC1 DP / AC2 JIRA-Epic parity / AC3 sweep / AC4 best-effort / AC5 byte-identical / AC6 dry-run / AC7 exact-key） |
...
| ci-local-content-hash-staleness | contract_pointer | 生成的 `ci-local.sh` staleness guard 用 source 檔 mtime 比對（`_src_mtime > SCRIPT_MTIME`）判斷 CI 宣告是否在 generation 後變動；worktree checkout 重寫 source mtime（內容未變）時 guard 誤觸發 `CI config changed` false-positive，需人工 `--force` regenerate 才能繞過（DP-338 D3 incident） | `scripts/ci-local-generate.sh` `file_fingerprint()` 捕捉每個 source 的 `content_sha`（SHA256），生成的 staleness guard 改以 runtime content hash 比對嵌入的 `content_sha`（`shasum -a 256` / `sha256sum` fallback，缺則 `POLARIS_TOOL_MISSING:shasum` exit 2），移除 mtime 比對；touch（mtime-only、內容相同）不再觸發 stale，內容變更 / source 缺失仍 detect。selftest：`scripts/selftests/ci-local-content-hash-staleness-selftest.sh`（clean FRESH / touch FRESH / content-edit STALE / missing STALE / 生成腳本無 `_src_mtime`） |
| branch-setup-base-context-cwd-independent | contract_pointer | `engineering-branch-setup.sh` 預設 first-cut path 的 base 解析、branch-chain cascade、duplicate guard、worktree cleanup 用 cwd-dependent `git rev-parse --show-toplevel` / 裸 `git`，假設 caller cwd 就是 product repo；從非 repo cwd（如 workspace root / 其他目錄）呼叫時誤判 base branch not found、worktree 建錯位置（DP-338 D5 incident）；或 `polaris-pr-create.sh` 在無 `--repo` 時 default 到 `$(pwd)` 而非 task.md 的 product repo，使 PR 建立 / gate 綁到錯的 cwd | `scripts/engineering-branch-setup.sh` 預設 path 以 `resolve_repo_from_task_md`（複用 `resolve-task-base.sh::derive_repo_path` 的 canonical `{workspace_root}/{Repo}` 約定，非第二套 repo resolver）解出 `REPO`，所有 git 操作走 `git -C "$REPO"`；task.md 無法 derive repo（legacy / framework workspace `Repo: polaris-framework` 指向 workspace root）時 fallback 回 cwd toplevel（非 synthesize）。`scripts/polaris-pr-create.sh` 無 `--repo` 時改由 `--task-md` derive product repo（explicit `--repo` 仍最高優先，--head 仍顯式帶），fallback 同。aggregate-release retained-bootstrap path 不在此 scope（維持自有 `repo_toplevel`）。selftest：`scripts/selftests/branch-setup-base-resolution-selftest.sh`（非 repo cwd 解析成功 + base 正確 + worktree 落 workspace / repo-cwd 回歸 / legacy fallback），pr-create 端由 `scripts/selftests/polaris-pr-create-repo-cwd-independent-selftest.sh` 維持綠 |
| worktree-cleanup-stop-worktree-scoped-dev-server | contract_pointer | `engineering-worktree-cleanup.sh --apply` 對任何 live process（`lsof +D <worktree>` 命中）一律 BLOCKED `reason=live_process`，即使該 process 是 worktree 自己的 dev server（cwd 在 worktree 內）；finalize 因此無法移除掛著 dev server 的 worktree，需人工先停 dev server 才放行（DP-338 D6 incident）；反向風險是若改成無差別 kill，會誤停 cwd 在 worktree 外的共享服務（colima / nginx / dev.exampleco.com） | `scripts/engineering-worktree-cleanup.sh` 以 `partition_live_pids`（複用既有 `live_process_pids` 的 `lsof +D` 枚舉，不寫第二套 process scanner）按 PID cwd（`lsof -a -d cwd -p <pid> -Fn`，canonicalize 後比對 worktree path）分 scoped（cwd 在 worktree 內）/ foreign（cwd 在外）：全部 scoped 時 classify SAFE，`--apply` 在 `git worktree remove` 前以 SIGTERM→grace→SIGKILL 停止 scoped PID；任一 foreign 仍維持 BLOCKED `reason=live_process`、不停任何 process；dry-run 永不停 process、永不移除。selftest：`scripts/selftests/worktree-cleanup-stop-dev-server-selftest.sh`（AC6 worktree-scoped 停 + 移除 / AC-NEG3 foreign-cwd 共享服務不停 + 仍移除 clean worktree / dry-run safety）+ embedded `--self-test`（worktree-scoped 停 + foreign-cwd 仍 block + 不停 foreign） |
