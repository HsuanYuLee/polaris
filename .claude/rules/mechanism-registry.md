...
| `spec-status-mark-on-done` | Epic/Bug/task 完成後須將 `specs/{TICKET}/refinement.md` 或 `plan.md` 的 frontmatter `status` 標為 `IMPLEMENTED`，讓 docs-viewer sidebar 標灰+✅（對齊 DP 行為）。Writer：verify-AC 全 AC PASS（Epic）、check-pr-approvals MERGED 偵測（Bug / ad-hoc task）。Helper：`scripts/mark-spec-implemented.sh {TICKET}`（idempotent）。使用者可手動編輯 frontmatter override | Epic verify-AC 全 PASS 後 refinement.md `status` 仍非 `IMPLEMENTED`；或 check-pr-approvals 偵測 MERGED 但未呼叫 helper；或 sidebar 顯示已 merge 的 Bug spec 無灰底樣式 | Medium |
