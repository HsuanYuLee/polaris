...
| `coverage-evidence-required` | `git push` blocked in repos with Codecov patch gate (`codecov.yml` `type: patch` or workflow `codecov/patch`) unless `/tmp/polaris-coverage-{branch-slug}.json` exists with `status: PASS` and timestamp < 4h. Bypass: `POLARIS_SKIP_COVERAGE=1` or `wip:` HEAD commit prefix. Skipped for main/develop. Writer: `scripts/write-coverage-evidence.sh`. Rationale: TASK-123 shipped untested `useFetch key` change that passed local quality but failed `codecov/patch/main-core` — this gate forces coverage BEFORE push, not discovered AFTER | PreToolUse hook on Bash, exit 2 to block | `.claude/hooks/coverage-gate.sh` |
