...
6. **Investigating before routing** — when the user says "沒修好" + PR URL, do NOT run `gh pr view`, `gh api`, or `gh pr diff` to "understand the problem first". Invoke `fix-pr-review` immediately. The skill reads review comments and CI status itself. (Graduated from prior session analysis: 2 sessions, 4+ occurrences)
