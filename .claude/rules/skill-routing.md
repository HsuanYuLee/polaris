...
當使用者訊息同時具備 fix intent（「修這個」、「幫我修」、「fix this」）與 Slack URL，但**沒有 JIRA ticket key** 時，Strategist 必須先建 ticket，再 route 到 `refinement` 的 Bug source mode：
...
4. **Route to `/refinement {BUG_KEY}`** with Bug source mode (`source_kind=bug`)
...
> **為什麼不直接交給 generic refinement？**
> Bug source mode 需要既有 Bug ticket key 才能綁定 reproduction / root cause / source PR /
> severity-impact 欄位。把建票留在 Strategist 層，能讓 `refinement` 開始前 source 已經存在。

## Routing Quick Reference

| User Intent | Trigger Patterns | Skill |
|-------------|-----------------|-------|
| Review someone's PR | "review PR", "review 這個 PR", "review 此 PR", "review 該 PR", PR URL + review 意圖（主語省略或主語=self；無「同仁/大家/人名」主語）| `review-pr` |
...
| Check own PR approvals / 催 review | "我的 PR", "PR 狀態", "催 review", "催 PR", "請同仁 review", "請同仁幫我 review", "請大家 review", "請大家幫我 review", "請大家幫忙看一下", "找人 review", "找誰 review", "請\[人名/角色\]幫我 review", "請\[人名/角色\] review" | `check-pr-approvals` |
...
| Estimate a ticket | "估點", "estimate", "評估" + ticket | `breakdown` (Story/Task/Epic) or `refinement` Bug source mode (Bug without current source) |
...
| Triage/plan a bug | "修 bug", "fix bug", "分析 bug", "triage bug" + ticket | `refinement` Bug source mode |
| Triage a bug (no ticket) | "修這個", "fix this" + Slack URL, no JIRA key | Strategist pre-processing → create Bug ticket → `refinement` Bug source mode |
...
- 若 resolver 回報 no `task.md` 或 multiple matches，**fail-stop**：不開 branch、不 patch
  code，回 owning planning skill（`refinement` / `breakdown`）建立或修復 work order。
...
完整 workflow intent 以 source 層級結案。使用者要求完成 DP、source、Epic 或完整流程時，
只完成單一 task、blocker hotfix、PR、version tag、framework-release closeout 或 local-extension
deliverable，都不等於完成；只要 sibling tasks、V tasks、verification disposition、source status
或 parent lifecycle closeout 仍未完成，就必須視為 partial state。

Routing 必須保留這個 invariant：

- `engineering` 可以完成一份 authoritative `task.md`，但除非 source 已沒有剩餘 sibling tasks
  且 verification 已完成，否則不可關閉 source。
- `framework-release` 可以 release framework task 的 workspace PR，但 release tail 完成不等於
  使用者要求的 DP/source workflow 已完成。
- 若流程需要 blocker task 才能解除阻塞，blocker release 後必須回到 `auto-pass` / owning source
  orchestration；不得把原本的完整 workflow intent 當成已完成。
...
- Ticket key + negative tone（Bug）→ `refinement` Bug source mode（若尚無 plan）或 `engineering`（若已有 plan）
...
7. **中文「請\[主語\]幫我 X」主語盲點**：句型中真正的執行者是\[主語\]，不是 assistant。判斷 review intent 時：
    - 主語為「同仁 / 大家 / 團隊 / 任何人名 / 角色」→ 催 review，route 到 `check-pr-approvals`（例：「請同仁幫我 review 這個 PR」「請大家 review 一下」「請阿桂幫我 review」）
    - 主語省略或主語=self → 自己 review，route 到 `review-pr`（例：「review 這個 PR」「幫我 review 這個 PR」無團隊主語）
    - 「review 大家的 PR」「掃 PR」這類**動作對象**是大家的 PR（非主語）→ `review-inbox`，不要與催 review 混淆
   不要把 "幫我 review" 當作獨立 trigger 做 substring 比對；必須先讀主語
