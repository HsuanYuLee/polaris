...
mcp__claude_ai_Atlassian__createJiraIssue
  ...
  description: |
    ...
    ## 測試資料
    - URL: https://dev.yourapp.com/zh-tw/category/jp-japan/sightseeing-tours/list
...
啟動後必須等頁面真正可訪問才能繼續。**必須透過 your-dev-proxy 的 nginx proxy 層打**，這才是標準開發流程。
...
其中 `<default-locale>` 用專案的預設語系路徑（your-app 用 `zh-tw`）。**必須打一個真實頁面路徑**（如 `http://dev.yourapp.com/zh-tw`），不能只打 `/`（Nuxt 可能 redirect 或回 404）。
...
| 90 秒後仍 503 | ❌ **FAIL**。503 = nginx 連不到 Nuxt upstream。回報：「Dev server 啟動失敗（nginx 回 503），請檢查：(1) Nuxt dev server 是否在正確 port 上（your-app 用 3001），(2) your-dev-proxy 是否在跑，(3) terminal 的 server log」 |
| 90 秒後 connection refused (000) | ❌ **FAIL**，回報：「nginx proxy 未啟動（connection refused），請確認 your-dev-proxy 容器是否在執行」 |
...
✅ Category page SSR 正常載入 → TASK-123 完成
✅ Product page SSR 正常載入 → TASK-123 完成
✅ i18n 翻譯正確顯示（直連 api-lang）→ TASK-123 完成
🔧 ab_test 直連 — 嘗試驗證失敗（原因：...）→ TASK-123 開放
✅ error isolation → TASK-123 完成
✅ Mockoon timing parallel 改善 → TASK-123 完成
✅ 無 hydration mismatch → TASK-123 完成
✅ SSR init route 單元測試 → TASK-123 完成（自動補寫）
...
未通過項目：
- TASK-123 [驗證] ab_test cache TTL — BLOCKED（Mockoon 未啟動）
- TASK-123 [驗證] cache hit 確認 — BLOCKED（依賴 3505）
