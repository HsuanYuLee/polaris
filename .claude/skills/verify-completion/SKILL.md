...
description: >
  ...
  Use this skill whenever: (1) another skill (bug-triage, work-on, unit-test) finishes
...
**Read-only 原則**：驗證 sub-agent 的預設行為是**只讀 + 報告**。不可修改專案檔案來讓驗證通過（既是裁判又是球員）。唯一例外是明確標記為「可自動修復」的項目（如補測試），這些項目的修復動作必須在驗證報告中透明記錄。
...
⚠️ 預設限制：你是驗證者，不是修復者。不可修改專案原始碼來讓驗證通過。
發現問題時回報（FAIL + 問題描述），由 Implementer 修復後重新驗證。
唯一例外：項目類型明確標記為「可自動修復」時，可以執行修復動作。
