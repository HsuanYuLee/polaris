...
usage() {
  cat >&2 <<EOF
Usage: bash .claude/skills/framework-release/scripts/gate-runtime-instruction-manifest.sh [--repo <path>]

Runs .claude/instructions/compile.sh --check against the repo root.
...
COMPILE="$REPO_ROOT/.claude/instructions/compile.sh"
...
echo "$PREFIX Repair: run 'bash .claude/instructions/compile.sh' then commit the regenerated targets, and re-push." >&2
exit 1
