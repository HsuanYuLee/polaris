...
setup_tree() {
  rm -rf "$WORK/repo"
  mkdir -p "$WORK/repo/.claude/skills/framework-release/scripts" "$WORK/repo/.claude/skills"
  cp "$SCRIPT" "$WORK/repo/.claude/skills/framework-release/scripts/"
}
...
run() { bash "$WORK/repo/.claude/skills/framework-release/scripts/sync-company-skill-links.sh" "$@" 2>&1; }
