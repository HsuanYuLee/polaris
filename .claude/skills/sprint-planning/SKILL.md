...
## Pre-Step: Learning Queue Graduation Scan

在 sprint planning 開始前（Step 1 之前），掃 learning queue 檢查畢業候選：
...
若有候選達閾值，提醒使用者：「Learning queue 有 {tag} 畢業候選，建議先跑 `/learning --graduate {tag}` 再開始 planning（畢業的 checklist 項目可能影響本 sprint 的 breakdown 品質）。」
