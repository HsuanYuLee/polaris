...
run_sweep() {
  ...
  if [[ "$moved" -gt 0 ]]; then
    ...
    echo "For static/search verification, start a preview viewer, then run: .claude/skills/visual-regression/scripts/polaris-toolchain.sh run docs.viewer.verify -- --ports <preview-port> --preview"
  fi
}
