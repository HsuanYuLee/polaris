...
description: >
  ...
  "幫我 review", "code review", (2) user shares a PR URL and asks to review it,
  (3) user says "看一下這個 PR", "檢查 PR", "review pull request". This skill is
...
- `review 這個 slack 訊息裡的 PR: https://your-company.slack.com/archives/C12345/p1234567890`
...
- `slack_channel_id`：頻道 ID（如 `YOUR_CHANNEL_ID`，config: slack.channels.ai_notifications，fallback: YOUR_CHANNEL_ID）
...
### 1. 辨識專案
從 PR URL 提取 repo 名稱，依序搜尋本地路徑：`./`、`~/work/<repo>`、`~/vbox/<repo>`（VirtualBox 共享資料夾環境）。
**重要**：先用 `ls ~/work/<repo>` 確認本地目錄是否存在。
...
### 3. 讀取專案規範
- 本地模式：用 Read tool 讀取 ~/work/<repo>/.claude/rules/ 下所有檔案
- 遠端模式：用 gh api 讀取
...
1. 當前工作目錄（`./`）
2. `~/work/<repo名稱>`
3. `~/vbox/<repo名稱>`（VirtualBox 共享資料夾開發環境）

例如：
- `github.com/your-org/your-app/pull/1882` → `~/work/your-app`
- `github.com/your-org/your-backend/pull/300` → `~/work/your-backend`
...
萃取到 lesson 時，寫入 `~/work/<repo>/.claude/rules/review-lessons/` 目錄：
...
```
📝 Review Lesson 萃取：
- 新增 N 條 lesson 到 ~/work/<repo>/.claude/rules/review-lessons/<file>.md
```
...
學習點寫入對應專案的 `~/work/<repo>/.claude/rules/review-learnings.md`。
...
- 已寫入 ~/work/<repo>/.claude/rules/review-learnings.md
```
...
- 本地 clone 搜尋順序：`./`、`~/work/<repo>`、`~/vbox/<repo>`（VirtualBox 共享資料夾開發環境，選用 — 無本地 clone 時自動 fallback 到 GitHub API 遠端讀取）
