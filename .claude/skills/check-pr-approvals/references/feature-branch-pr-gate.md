...
```bash
bash .claude/skills/check-pr-approvals/scripts/resolve-pr-work-source.sh --repo {repo_dir} --intent mutable
bash .claude/skills/check-pr-approvals/scripts/pr-state-snapshot.sh --repo {repo_dir} --intent mutable
```
...
irror`，執行 `ci-local.sh`（lint + test + coverage 由 repo CI config 推導內含）
