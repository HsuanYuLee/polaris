...
# 記名字，不記路徑（DP-496 L-P2）。寫下來的那一條路徑在單被搬動之後就是死指標——實測 19
# 條存過的單路徑全部指向已經不存在的目錄。（那 19 條是重算自己搬出來的；DP-661 之後重算
# 不搬了，但人還是會搬，而這一條擋的是「路徑會變」，不是「誰讓它變」。）位置要用的時候問 `spine-loop-state.sh find`，而讀這份紀錄的東西（釋出尾段的
