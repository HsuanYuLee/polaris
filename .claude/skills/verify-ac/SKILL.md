...
### 一條實例：某個輸出的消費者，要量那個輸出，不量它的替身

**「這個值換了來源之後，每一個讀它的地方拿到的東西都沒變」是第一列，不是第三列。** 它會被
機械執行，所以它的量法是真的去取那個輸出——渲染出來的那一塊、API 回應的那一格——不是一支
單元測試說它組出來的字串對。單元測試量的是組裝那一步，而消費者拿到的是組裝、傳遞、快取、
渲染全部跑完之後的東西；那幾層之中有一層換了來源，測試照樣綠。

**要比就說出比的是哪一段。** 兩次部署之間整份回應幾乎不會逐位元組相同（時間戳、cache
busting、build id），所以這一條要指名的是那個消費者印出來的那一塊文字或那一個欄位，不是
整份 response。

**跨 repo 的單還要說出哪一邊算「改動後」。** 一個值流經好幾個服務的時候，「新舊兩個 build」
不是一台機器——哪一台裝的是這一輪的改動、哪幾台維持原樣，由那張單自己說出來，判定這一站
不替它挑。
...
## 施工方自己挑的 oracle，漏掉了什麼

上面那三層擋得住偽造的證據，**擋不住一條從第一天就量錯東西的命令**——它一直是綠的，因為
沒有人拿一份壞掉的實作去試它會不會紅。挑那條命令的人跟寫實作的是同一個，而他挑的時候
已經知道自己寫了什麼。

所以多一條路：**讓一個看不到你的對話的 agent，從同一份 assertion 自己推一組探針，跑完再
跟你的比。**

```bash
# 1. 它只拿得到一份去掉施工痕跡的單（只有 plan 與 assertion）。那份放在單以外的地方，
#    名字也不帶單號——放在單底下的話，它離你的量測腳本只有一個 `..`。
bash .claude/skills/verify-ac/scripts/build-independent-oracle-prompt.sh \
  --issue {issue} --tree <要量的 checkout> --phase 1
# 把印出來的那份指示原樣交給一個 agent，等它寫出 {issue}/.spine/independent/phase1.json

# 2. phase1.json 在了，第二份指示才產得出來。這時才給它你的量測登錄。
bash .claude/skills/verify-ac/scripts/build-independent-oracle-prompt.sh \
  --issue {issue} --tree <同一棵> --phase 2

# 3. 收成報告：三類各幾條、量不到的有哪些、這一趟花了多少
bash .claude/skills/verify-ac/scripts/record-independent-oracle.sh \
  --issue {issue} [--cost-tokens N] [--cost-seconds N]
```

**順序由檔案系統擋住，不由叮嚀擋住。** 第一階段那份指示裡沒有你的命令，而且產得出第二份
的前提是第一階段的結果已經落地。「先自己推」因此不是一句請求。

**那份指示沒有第三個參數，這是刻意的。** 你交得出去的只有「單在哪」「樹在哪」。一個
「可選的補充說明」看起來無害，而它正好會把這一趟唯一的根據送掉——那個 agent 的價值完全
來自它沒看過你的說法。

**它產出的是意見，不是判定。** `record-independent-oracle.sh` 的離場碼說的是「收不收得成」，
不是「過了沒」；判「這張單能不能出貨」的仍然只有交付紀錄。把一個 LLM 的判斷接到會擋人的
路徑上是往下走——EACL 2026 量到，一行「this is correct」的註解就能讓 LLM-as-judge 的判對率
動 34 個百分點。

**三類都要說出來，空的也要說**：只有你量到的、只有它量到的、兩邊都量到的。`independent_only`
那一類是這一趟的產出；它是空的**也是一個結果**，照實記著——那是「這一層值不值得」要被重估
的證據，不是一個要被補滿的空格。

**不要把這一趟寫成 held-out 驗證，也不要寫成獨立稽核。** held-out 的定義是那份測資被驗的人
優化不到，而這裡：派工的是你、跑的是同一棵樹、探針由 LLM 從同一份 assertion 推，三件都不
滿足；信任域也沒有換（見〈這一層擋得住什麼、擋不住什麼〉）。它拿掉的那一樣東西是**敘事
污染**——那個 agent 看不到你的自我背書。能宣稱的就是這樣，不多。

**它碰得到的東西要有上界，而那個上界只寫在指示裡。** 2026-09-21 實測：這台機器上一個背景
sub-agent 手上瀏覽器、JIRA、Slack 送訊息全部叫得到，憑證檔讀得到，工作目錄預設是主
checkout。所以指示裡逐條寫明不得送 Slack、不得寫 JIRA、不得送出 review、不得推 ref、不得
改動那棵樹。**這是一份會被自願遵守的界線，不是一道閘**——這一句要先講，一份把自己說成
安全網的說明會讓下一個人不去問「那誰在擋」。真正在擋的仍然是那棵樹上的 git hook 與看
diff 的人。
...
同一條命令通常被好幾條 assertion 共用，所以重跑會去重。**去重的鍵是三樣**：命令、在哪棵樹跑、
要哪些工具。八條 assertion 共用一條命令、跑在同一棵樹、要同一組工具，那就是同一個鍵——跑一趟。

**正負向樣式不在鍵裡，這是刻意的。** 它們決定的是判定，不是執行：那一趟跑完之後，八條各自拿
自己的樣式去判同一份輸出，各自寫自己的證據檔。少了那一步，第二條 assertion 會拿到第一條的答案，
而它自己的樣式從來沒有被檢查過——**一條沒被量到的 assertion 看起來就跟過了一樣**。所以去重放寬
的是執行，不是判定。

要估時間就照命令數（加上樹與工具）去數。**重跑那一步在跑第一趟之前會把趟數印出來**，不用
自己數。

**以前不是這樣。** 樣式曾經也在鍵裡，於是 2026-08-27 有一張單 21 條 assertion 共用一條命令、
各帶各的樣式，跑了 21 趟同一件事——差出來的形狀是一個兩分鐘的逾時。那個行為在 DP-630
（`524aaacb`）被改掉了，同一張單現在跑 1 趟。這個例子留著，是因為它解釋了為什麼會有人相信
另一個版本。
