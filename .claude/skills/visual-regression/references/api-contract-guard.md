...
```bash
# Check all fixtures for a ticket (path per references/vr-artifact-location.md)
...
- **`fixtures.mockoon` capability**: starts/stops Mockoon through `pnpm --dir .claude/skills/visual-regression/toolchain mockoon:start` / `mockoon:stop`. Contract-check does NOT start Mockoon — it reads fixture files directly and hits live APIs
