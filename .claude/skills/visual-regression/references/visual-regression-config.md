...
| `runner` | string | 是（如果有 fixtures block） | 啟動 fixture server 的命令（預設 `pnpm --dir tools/polaris-toolchain mockoon:start`）。Skill 在 runtime 把 `-- {company_specs_dir}/{EPIC}/tests/mockoon` 接在它後面 |
