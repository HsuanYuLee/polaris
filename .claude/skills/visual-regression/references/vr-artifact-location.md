...
**位置是狀態的投影。** `placement.json` 說的是這張單現在該在哪；`--execute` 會把搬得動的
那幾張搬過去，搬不動的（不是鏈頂、身上沒有 `.spine/`、單號有兩條路徑、目的地已經有東西）
留在原地，所以路徑與那份紀錄對不上仍然會發生。要問「這張單走到哪了」讀那份紀錄，不要讀
資料夾名。想讓一張單離開待辦清單，讓它收斂，不是搬它。
...
```bash
pnpm --dir .claude/skills/visual-regression/toolchain mockoon:start -- \
