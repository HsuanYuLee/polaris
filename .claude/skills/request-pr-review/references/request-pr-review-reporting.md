...
**格式從哪拿：`plan-pr-notify.sh` 最後一欄的目的地就是那一層的答案，格式跟目的地一起在
那裡面。** 它對核心是不透明的，但它是給人讀的——那一層印得出 `message_format` 與一個路徑
時，去讀那個檔案，照它說的形狀組訊息。沒有那一欄就是那一層沒有版型，這時候形狀由組訊息
的人自己定，而**那件事要說出來**，不要當成正常情況：同一個目的地會收到形狀每次都不一樣的
請求。

`notify` 那一欄的形狀由宣告方自己定，核心不解讀它——所以這裡不寫死欄位名的清單，讀到什麼
就照它說的做。
