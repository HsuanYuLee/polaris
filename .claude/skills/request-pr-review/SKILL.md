...
**命令裡的相對路徑，錨是宣告它的那份 `SKILL.md` 所在的目錄**，不是呼叫者當下的 cwd——核心
跑它之前會先切過去。宣告行寫得出來的只有相對路徑，而「相對於我自己」在 skill 被單獨帶到
別的環境時仍然成立，「這個 workspace 的 root 在哪」在那裡沒有答案。
...
- **`known`** → 照宣告那一支 skill 自己的說明送出。**怎麼送、送到哪、訊息長什麼樣，都是
  那一層的事**——這支 skill 不認得任何一個目的地的形狀。目的地那一欄裡指出一份訊息版型時
  就去讀它，照它說的形狀組；沒有版型的話說出來，不要安靜地自己定一個形狀。
