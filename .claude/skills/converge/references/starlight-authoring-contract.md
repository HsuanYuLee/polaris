...
```bash
bash .claude/skills/converge/scripts/validate-specs-collection-shape.sh --all
```
...
- Deterministic drift 先用 `.claude/skills/converge/scripts/sync-spec-sidebar-metadata.sh --apply <path>` 修正，再跑 `.claude/skills/converge/scripts/validate-dp-metadata.sh <path>` 驗證 DP metadata。
...
```bash
.claude/skills/converge/scripts/validate-starlight-authoring.sh path/to/file.md
.claude/skills/converge/scripts/validate-starlight-authoring.sh path/to/spec-container
```
