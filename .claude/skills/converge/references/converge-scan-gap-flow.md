...
對有 review state 的 PR，把 PR metadata 與 review threads 一起餵進：

```bash
bash .claude/skills/converge/scripts/pr-review-state-classifier.sh \
...
fix, wait, or reviewer handoff。若需要 `mergeability`、`base_freshness`、或 shared readiness
vocabulary，先讀 `resolve-pr-work-source.sh` → `pr-state-snapshot.sh`；converge 不得自行拼
出 `mergeable_ready` 或忽略 `stale_downstream`。

優先使用 shared PR state scripts：

- `resolve-pr-work-source.sh`
- `pr-state-snapshot.sh`
- `pr-action-classifier.sh`

Feature branch readiness 仍使用 `references/scripts/check-feature-pr.sh`。
