...
```bash
bash .claude/skills/converge/scripts/resolve-pr-work-source.sh --repo {repo_dir} --intent mutable
bash .claude/skills/converge/scripts/pr-state-snapshot.sh --repo {repo_dir} --intent mutable
```
