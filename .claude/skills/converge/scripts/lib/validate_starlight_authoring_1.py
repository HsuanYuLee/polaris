"""Structured validator authority extracted from scripts/validate-starlight-authoring.sh."""
...
def is_dir_walk_excluded(file: Path) -> bool:
    ...
    for idx in range(len(dir_parts) - 1):
        if (
            dir_parts[idx] == "artifacts"
            and dir_parts[idx + 1] in EXCLUDED_ARTIFACT_SUBDIRS
        ):
            return True
    return False
...
def markdown_files(path: Path):
    ...
    if is_generated_path(path):
        print(
            f"error: generated output path is not a source input: {path}",
            file=sys.stderr,
        )
        sys.exit(2)
    if path.is_file():
...
def source_link_findings(text):
    ...
    for target in link_targets:
        ...
        if legacy_source in target:
            findings.append(
                ("source-link", "link points at legacy docs-viewer specs source")
            )
    return findings
    ...
    if fm_end is None:
        ...
        if not normalize(fm.get("description", "")):
            add(
                "manual-needed",
                "missing-description",
                "description must be authored or safely inferred",
            )
