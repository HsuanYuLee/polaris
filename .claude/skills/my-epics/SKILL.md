...
⚠️ 狀態不同步（JIRA board 顯示 active 但實際已完成）：
  - PROJ-123 [AEO] 方案資訊結構化資料 → 實際狀態：完成
  - PROJ-123 [AEO] 修正商品頁結構化資料 → 實際狀態：完成
```
...
🔧 In Development
  1. PROJ-123 [CWV] TTFB 優化 — 17 SP — PR #88 approved, 待 merge 🟢
  2. PROJ-123 [CWV] JS Bundle 瘦身 — PR #92 open, CI ✅
  3. PROJ-123 CWV 報表 mobile — 觀察中，明天收

📋 待辦 — Highest
  4. PROJ-123 [SEO] 首頁結構化資料 — 未估點
  5. PROJ-123 [SEO] FAQ H3 優化 — 未估點
  ...

📋 待辦 — High
  10. PROJ-123 AI 爬蟲調查 — 未估點
  ...

⚠️ 狀態不同步（已自動排除）
  - PROJ-123, PROJ-123 — 實際已完成

💡 建議下一步
  - PROJ-123 快完成了，merge 後可收
  - 待辦 Highest 有 6 張未估點，建議批次估點：做 PROJ-123 PROJ-123 PROJ-123
══════════════════════════════════════
```
