...
Polaris 提供的 commit message 兜底規範。當 repo **L1 tooling**（commitlint config / husky `commit-msg` hook）與 **L2 handbook**（`{company}/polaris-config/{project}/handbook/**`）皆未宣告 commit convention 時，LLM 依此 spec 寫 commit message。
...
| **L2 Repo handbook** | `{company}/polaris-config/{project}/handbook/**/*.md` 的 commit convention 段 | **次高**（補 L1 未宣告的敘述要求；L1 有則 L2 僅作補充 context） |
...
| Branch name 中的 ticket pattern（`task/TASK-3788-*` / `feat/EPIC-478-*`）| 2（fallback） |
...
- Framework **不**新增 commit lint script，**不**收進 `ci-local.sh`（時序不 match：`ci-local.sh` 在 Step 2 跑、commit message 在 Step 6a 才寫）
...
```
feat(TASK-3788): 產品頁 BreadcrumbList 加首頁 entry
...
```
refactor(EPIC-521): 抽出 useProductHeading composable
```
...
```
fix(TASK-3788): 修正 SSR 階段 breadcrumb hydration mismatch
