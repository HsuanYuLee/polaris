...
| Branch name 中的 ticket pattern（`task/KB2CW-3788-*` / `feat/GT-478-*`）| 2（fallback） |
...
```
feat(KB2CW-3788): 產品頁 BreadcrumbList 加首頁 entry
...
```
refactor(GT-521): 抽出 useProductHeading composable
```
...
```
fix(KB2CW-3788): 修正 SSR 階段 breadcrumb hydration mismatch
