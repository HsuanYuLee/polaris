...
### Packaging-Only Plan-Defect Lane（scope=packaging）

`plan-defect` 的 scope 再細分：

- **`scope=packaging`**：修正 **只**動 per-task packaging 欄位（Allowed Files /
  estimate_points）的純機械 backfill。一行 packaging 補欄不是 source intent 變更，由
  breakdown 直接改原 task.md，走 `route=task_update`，**不**建 refinement-inbox record、
  **不**遞增 `escalation_count`（一行 packaging backfill 不消耗 loop cap）。
- **`scope=full`**（預設）：plan-defect 需要動 packaging 以外的 task contract（Test
  Command / Verify Command / Test Environment 等）或牽涉 source intent，照原 task.md 修正
  流程，照常受 loop-cap 計數。

判定 test：問「這個 escalation 的修法是否**只**是把缺的 Allowed Files / estimate_points
補回去？」是 → `scope=packaging` + `route=task_update`；否 → `scope=full`。

不得把 packaging-only plan-defect bounce 到 `route=refinement`（DP-212 amendment loop）。
amendment loop 是給 source intent 變更用的；純 packaging backfill 屬 breakdown 的 task.md
writer path（DP-341 boundary：refinement.json `tasks[]` 是 intent，per-task packaging 由
breakdown 落地）。
...
scripts/validate-breakdown-escalation-intake.sh \
  ...
  --scope "{packaging|full}" \
  --decision "{planner decision}"
```

`--scope` 預設 `full`。packaging-only plan-defect 帶 `--scope packaging`：validator 強制
`flavor=plan-defect` 且 `route=task_update`，PASS 行帶 `escalation_count_delta=0`（packaging
backfill 不消耗 loop cap）。

exit != 0 時停止。

落地規則：

- `plan-defect`（`scope=full`）：由 breakdown 直接修原 task.md。
- `plan-defect`（`scope=packaging`，route=`task_update`）：由 breakdown 直接修原 task.md 的
  packaging 欄位（Allowed Files / estimate_points），**不**建 refinement-inbox record、**不**
  遞增 `escalation_count`。
