...
| Tool requirement handoff | `tool_requirements[]` / task.md `Required Tools` schema, install authority enum, ticket-scoped `goes_to_mise=false`, fail-fast `POLARIS_TOOL_*` token vocabulary | `validate-refinement-json.sh`, `validate-task-md.sh`, `parse-task-md-selftest.sh` | `deterministic` |
...
| Local specs tracking guard | `docs-manager/src/content/docs/specs/**` is local-only; tracked/staged specs are blocked before commit / push / PR create | `gate-no-tracked-specs.sh`, `gate-no-tracked-specs-selftest.sh`, `polaris-pr-create.sh`, `install-git-hooks.sh` | `already_deterministic_reduce_audit` |
| Handbook config runtime contract | project handbook machine fields schema, handbook-first runtime config resolution, workspace-config fallback / conflict detection | `resolve-handbook.sh`, `handbook-config-validator.sh`, `resolve-handbook-selftest.sh`, `handbook-routing-retirement-selftest.sh`, `start-test-env.sh --resolve-config-only`, `deterministic-hooks-registry.md` | `already_deterministic_reduce_audit` |
...
| Script topology manifest | scripts inventory ownership, runner, selftest disposition, lifecycle posture, relocation decision, and root script registration | `scripts/manifest.json`, `check-script-manifest.sh`, `check-script-manifest-selftest.sh`, `framework-release-pr-lane.sh` | `deterministic` |
