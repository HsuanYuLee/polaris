...
| `handoff_advisory` | refinement | `refinement.json.handoff_advisories[]`（schema 詳見 `refinement-artifact.md`） | `refinement.md` Handoff Advisories section | breakdown、refinement handoff gate、auto-pass route-back classifier | `scripts/validate-refinement-json.sh`、`scripts/selftests/validate-refinement-json-selftest.sh`、`scripts/selftests/render-refinement-md-selftest.sh` | 下游 skill 只讀 advisory disposition；不得從 stderr transcript、對話或 final answer 補 scope | `fail_stop_on_pending_or_malformed_advisory`；derived view drift → render script 重跑 |
...
| `lifecycle_marker` | engineering（除 verify marker 外） | `.polaris/evidence/{producer}/{work_item_id}-{head_sha}.json`（schema 詳見 `auto-pass-proof-of-work.md`） | final answer status、JIRA comment、PR body summary | auto-pass runner / probe、closeout chain、framework-release tail | `scripts/lib/evidence-producers.json` | LLM 不得用 prose 補 PASS / terminal；必須讀 durable marker | `fail_stop_on_missing_marker`；prose-only PASS → blocked_by_gate_failure |
| `engineering_self_review_outcome` | engineering writer（semantic verdict 由 Critic 擁有） | `scripts/write-engineering-self-review-result.sh` 產生的 `.polaris/evidence/engineering-self-review/*.json` | Phase 3 Detail summary | engineering Phase 3 dispatcher | `scripts/validate-engineering-self-review-result.sh`、`scripts/selftests/write-engineering-self-review-result-selftest.sh`、`scripts/selftests/validate-engineering-self-review-result-selftest.sh` | Critic 判 code quality；writer 只綁 HEAD、worktree state、round、remediation count 與 next action | `fail_stop_on_stale_review_state`；stale outcome → 同 round current-state re-review，terminal round 4 FAIL → human_review |
...
| `fail_stop_on_stale_review_state` | Self-Review artifact 的 reviewed HEAD 或 worktree state digest 不等於 current state 時 fail-stop；不得用 chat / session reset 取代 current-state Critic outcome |
