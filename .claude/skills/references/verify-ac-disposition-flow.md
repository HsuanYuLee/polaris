...
1. Implementation drift：code 未達 AC 行為，建立 per-AC Bug，交給 refinement Bug source mode。
...
不填 assignee；refinement Bug source mode 對 assignee blind。

同時寫 verify-fail handoff artifact，供 refinement Bug source mode AC-FAIL path on-demand 讀。Artifact 必須：
...
建立 Bug 後，在 AC ticket comment 回寫追蹤 Bug keys，並提示 `refinement Bug source mode {BUG_KEY}`。
