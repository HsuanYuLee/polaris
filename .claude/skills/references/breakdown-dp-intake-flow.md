...
```text
docs-manager/src/content/docs/specs/design-plans/DP-NNN-{slug}/plan.md
...
讀 `plan.md` 的 Goal / Decisions / Blind Spots / Acceptance Criteria / Technical
...
DP-backed tasks，不是 JIRA sub-tasks：
...
```bash
scripts/validate-task-md.sh {dp_folder}/tasks/T{n}.md
scripts/validate-task-md-deps.sh {dp_folder}/tasks/
```

全部 pass 後，更新 `plan.md` Implementation Checklist / Work Orders linkage。Validator
fail 時修 artifact，不 handoff engineering。
