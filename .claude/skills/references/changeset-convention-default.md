...
deliverables:
  changeset:
    package_scope: "@kkday/web"          # L1 .changeset/config.json packages 推導
...
| 元件 | 規則 |
|------|------|
| `ticket-kebab` | task.md ticket 小寫 + 連字符（`KB2CW-3788` → `kb2cw-3788`） |
...
|------------|--------------|
| `[KB2CW-3788] 產品頁 BreadcrumbList 加首頁 entry` | `kb2cw-3788-product-heading-mobile-layout`*（短描述由 breakdown 決定，title 過長時可裁剪）* |
| `[GT-521] BreadcrumbList SEO 優化` | `gt-521-breadcrumblist-seo` |
...
| 模式 | 行為 | 範例 |
|------|------|------|
| `strip`（**L3 default**） | 移除 `[TICKET]` / `TICKET:` 前綴 | `[KB2CW-3788] 產品頁 BreadcrumbList 加首頁 entry` → `產品頁 BreadcrumbList 加首頁 entry` |
| `keep` | 保留原樣 | `[KB2CW-3788] 產品頁 BreadcrumbList 加首頁 entry` → `[KB2CW-3788] 產品頁 BreadcrumbList 加首頁 entry` |
