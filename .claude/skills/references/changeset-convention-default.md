...
deliverables:
  changeset:
    package_scope: "@your-company/web"          # L1 .changeset/config.json packages 推導
...
| 元件 | 規則 |
|------|------|
| `ticket-kebab` | task.md ticket 小寫 + 連字符（`TASK-123` → `kb2cw-3788`） |
...
|------------|--------------|
| `[TASK-123] 產品頁 BreadcrumbList 加首頁 entry` | `kb2cw-3788-product-heading-mobile-layout`*（短描述由 breakdown 決定，title 過長時可裁剪）* |
| `[PROJ-123] BreadcrumbList SEO 優化` | `gt-521-breadcrumblist-seo` |
...
| 模式 | 行為 | 範例 |
|------|------|------|
| `strip`（**L3 default**） | 移除 `[TICKET]` / `TICKET:` 前綴 | `[TASK-123] 產品頁 BreadcrumbList 加首頁 entry` → `產品頁 BreadcrumbList 加首頁 entry` |
| `keep` | 保留原樣 | `[TASK-123] 產品頁 BreadcrumbList 加首頁 entry` → `[TASK-123] 產品頁 BreadcrumbList 加首頁 entry` |
