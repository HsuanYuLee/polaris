...
## Framework DP Release Topology

DP-392 後，framework DP 的多 task PR lane 必須是 declared stack PR。Cascade rebase
消費 task.md `Branch chain` 時，應維持 `task/DP-NNN-T1 -> task/DP-NNN-T2 -> ...`
的線性 ancestry；不得把 downstream task branch rebase 回同一個 `feat/DP-NNN` 而形成
multi-head sibling task PR release。release tail 會用 task metadata、GitHub PR
base/head structured fields 與 git ancestry 檢查：任一 task head 不是 final release
head ancestor，或 promotion 後 `main` 出現 final merge bubble / per-task merge commit /
GitHub merge commit，都必須 fail-stop 並要求重建 stack。
