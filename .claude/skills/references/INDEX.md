...
| [auto-pass-execution-flow.md](auto-pass-execution-flow.md) | auto-pass execution loop：dependency-gated breakdown / engineering / verify-AC dispatch、proof-of-work probes、pause taxonomy 與 retry cap | auto-pass, execution loop, proof-of-work probe, route-back |
| [friction-capture-contract.md](friction-capture-contract.md) | friction capture canonical contract：emit stage / kind enum、writer path、deterministic trigger map 與 framework-release tail capture | auto-pass, framework-release, append-auto-pass-friction, friction-log, observability |
| [auto-pass-report.md](auto-pass-report.md) | auto-pass terminal report contract：workspace PR ready、verification current、dogfood evidence、synthetic fixture marker、follow-up DP seed 與 framework-release handoff | auto-pass, final report, dogfood, framework-release handoff |
| `scripts/auto-pass-runner.sh` (script) | auto-pass orchestrator deterministic runner — emit stable JSON `next_action`/`next_skill`/`terminal_status` across source/breakdown/engineering/verify-AC/amendment/resume fixtures；thin SKILL 的主鏈狀態來源 | auto-pass orchestrator runner, runner JSON, dispatch envelope, downstream agent |
...
| [refinement-artifact.md](refinement-artifact.md) | Refinement 結構化 artifact JSON schema — breakdown derive work order、verify-AC 讀 verification method/detail（engineering 只消費 breakdown 產出的 task.md，不直接讀 refinement.json） | refinement (Tier 2+), breakdown, verify-AC |
...
| [mechanism-canary-entries.md](mechanism-canary-entries.md) | Mechanism canary 的完整敘述與 deterministic evidence 對照 — 從 mechanism-registry.md 拆出以降低 rules 載入成本（50KB、無程式 parse、86% 為 contract_pointer） | post-task mechanism audit（需查特定機制的 canary signal 時） |
