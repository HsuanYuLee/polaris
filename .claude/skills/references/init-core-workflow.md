---
title: "Init Core Workflow"
description: "/init 的核心公司設定流程：precheck、language、company basics、GitHub、JIRA、Confluence、Slack、Kibana、projects、scrum、infra 與 write。"
---

# Core Setup Contract

這份 reference 負責 `/init` 在 runtime 與 post-setup extensions 之前的公司設定流程。

## Precheck And Re-init

若 root `workspace-config.yaml` 存在，先讀取並顯示 existing companies。接著詢問要
add company、edit existing company、re-init missing upgrade sections，或 cancel。

Re-init 時讀取選定 company config，只執行缺漏 sections：
...
Repo selection 後，依 `init-interaction-patterns.md` 做 AI repo detection，填入 project
