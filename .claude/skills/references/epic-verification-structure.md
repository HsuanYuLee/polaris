...
{無 → 省略此段；有依賴 → 列出被依賴的 AC 編號}
- AC#1
```

### depends_on 欄位

驗收單之間有順序依賴時（例：AC#2 須先通過 AC#1 才有意義），在 description 加 `## depends_on` 段列出被依賴的 AC 編號。

- **無 depends_on**：可與其他 AC 並行驗證
- **有 depends_on**：verify-AC skill 於 Epic 模式下依依賴排序，被依賴者 PASS 後才跑

判斷原則：AC 之間若無強依賴，不要硬加 — 平行執行更快，也讓 regression 偵測更誠實。
...
| 條件 | 驗收單點數 | 範例 |
|------|----------|------|
| curl / 檢視原始碼 / 簡單手動操作 | 0 pt | EPIC-521 檢查 JSON-LD |
