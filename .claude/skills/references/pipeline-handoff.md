...
| `runtime` | 需要 live endpoint（curl / dev server / nginx）才能跑 Verify Command | 依 `dev_environment.requires` 啟動 dependencies（如 `exampleco-web-docker`）+ `start_command` 起 dev server + `health_check` 驗證 ready，**若 Fixtures 非 N/A**，同時起 `scripts/polaris-toolchain.sh run fixtures.mockoon.start -- {fixture_path}` |
...
- `requires`（依賴的其他 service，如 `exampleco-web-docker`）
...
設計決策：2026-04-13，EPIC-521 breakdown v2 試跑後討論 pipeline 權責拆分。
