...
| "This module can't do what we need, let me replace it" | Replacement is T3 — confirm with user. First exhaust: (1) official API docs, (2) npm README, (3) GitHub issues/discussions. EPIC-521 lost 3 rounds because compiled source was treated as API truth |
...
```json
{
  ...
  "runtime_contract": {
    "level": "runtime",
    "runtime_verify_target": "https://dev.exampleco.com/zh-tw",
    "runtime_verify_target_host": "dev.exampleco.com",
    "verify_command": "curl -sk https://dev.exampleco.com/zh-tw | ...",
    "verify_command_url": "https://dev.exampleco.com/zh-tw",
    "verify_command_url_host": "dev.exampleco.com"
  }
}
```

**Writer**: `scripts/run-verify-command.sh --ticket TASK-1234 --task-md <path/to/task.md> --repo <repo> -- <verify command>` — called by engineering (engineer-delivery-flow Step 3). Manual verification must still go through this script so evidence is tied to the current `head_sha`.
