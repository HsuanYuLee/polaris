---
title: "Init Interaction Patterns"
description: "/init 的 smartSelect、AI repo detection、audit trail 與 sub-agent completion envelope 規則。"
---

# Interaction Pattern Contract

這份 reference 定義 `/init` 共用互動規則。
...
每個 decision append 一行 JSON 到 `{company}/.init-audit.jsonl`。
...
| `step` | init step number or decimal extension |
...
Audit 是 append-only。重跑 `/init` 時，先 append restart marker，再寫入新 entries；
不可覆寫舊 audit lines。
