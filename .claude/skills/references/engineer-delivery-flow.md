...
- **`engineering`**（Maintainer Release 角色，DP-backed framework work order，終態交給 `framework-release`）
...
| 項目 | Developer | Maintainer Release | Admin |
|------|-----------|--------------------|-------|
| Branch | Task branch 已 checkout（`task/PROJ-NNN-*`）| DP task branch 已 checkout（`task/DP-NNN-Tn-*`）| 非 main 的工作 branch |
| Code | 實作完成、可 commit 的狀態 | 實作完成、可 release handoff 的狀態 | 變更完成 |
| task.md | 路徑或完整內容（含 Repo、測試計畫、行為驗證 Layer B）；所有欄位讀取走 `scripts/parse-task-md.sh`（DP-032 D8） | DP-backed task.md，同 Developer | 不需要 |
| JIRA ticket key | 必填 | 不需要；使用 DP pseudo-task ID | 不需要 |
| Base branch | 從 task.md 或 JIRA parent 推導 | 從 DP task.md 推導；release 前整合到 workspace main | 當前 branch upstream 或 `origin/main` |
| Role declaration | context 明確說 `role: developer` | context 明確說 `role: maintainer-release` | context 明確說 `role: admin` |
...
- **Layer A+B(+C) evidence AND gate**：所有必要 evidence 檔案存在且 `head_sha` 匹配當前 HEAD 才放行 PR 或 maintainer release handoff
...
- Maintainer Release：不開 PR；交付給 `framework-release`，由 release final verification 回傳 workspace commit、template commit、tag、release URL（若有）
...
6. **終態是 evidence AND gate** — Layer A（`ci-local.sh`）+ Layer B（`run-verify-command.sh`）+ Layer C（`run-visual-snapshot.sh`, conditional）三檔 evidence 全部 `head_sha` 匹配當前 HEAD 才放行 PR 或 maintainer release handoff。
...
│                                        │  │   → 6 Commit → 7 PR/Release → 8 JIRA/Release  │
└────────────────────────────────────────┘  └──────────────────────────────────────────────┘
```

**Step 完整序列**：Step 1 Simplify → Step 1.3 Self-Review → Step 1.5 Scope Gate → Step 2 前置 Rebase → Step 2 Local CI Mirror → Step 3 Verify → Step 3.5 VR → Step 5 Base Freshness → Step 6 Commit+Changeset → Step 7 PR / Maintainer Release Handoff → Step 8 JIRA / Release Verification → Step 8a IMPLEMENTED → Step 8.5 Completion Gate

## Role Matrix

| 步驟 | Developer（engineering） | Maintainer Release（engineering + framework-release） | Admin（git-pr-workflow） |
|------|---------------------|-----------------------------------------------|------------------------|
| Input | task.md（含 Repo、測試計畫、行為驗證）；active `tasks/` 找不到時 fallback `tasks/pr-release/`（DP-033 D8）| DP-backed task.md（同 Developer schema） | 無 — 直接讀當前 diff |
| Step 1 Simplify | ✅ | ✅ | ✅ |
...
| Step 6 Commit + Changeset | ✅ | ✅（commit/integration handoff；release commit 由 `framework-release` 最終擁有） | ✅ |
| Step 7 PR Create（含 7a Evidence AND Gate） | ✅ | ⏭️ 改走 `framework-release` handoff | ✅ |
| Step 8 JIRA Transition → CODE REVIEW | ✅ | ⏭️ 跳過（無 JIRA ticket） | ⏭️ 跳過（無 JIRA ticket） |
| Step 8a Mark IMPLEMENTED | ✅ | ✅ 但只在 release final verification 成功後 | ⏭️ 跳過 |
| **Step 8.5 Completion Gate（`check-delivery-completion.sh`）** | ✅ | ⚠️ `--maintainer-release` helper 待補；暫時手動跑 Layer A+B gates + `framework-release` final verification | ✅（`--admin`） |
| Evidence file 鍵 | `/tmp/polaris-verified-{TICKET}-{head_sha}.json` | `/tmp/polaris-verified-{DP-NNN-Tn}-{head_sha}.json` | `/tmp/polaris-verified-{branch-slug}-{head_sha}.json` |

**Role 由呼叫端傳入**，reference 不做角色偵測。呼叫端在 dispatch prompt 或 context 說明「你是 Developer / Maintainer Release / Admin」。
...
## Step 7 — PR Create / Maintainer Release Handoff

### Maintainer Release role

Maintainer Release role 不建立一般 PR。Step 7 的 precondition 仍相同：Layer A+B(+C) evidence 必須存在且匹配 task head；不同處在於通過後不是呼叫 `polaris-pr-create.sh`，而是組 `framework-release` handoff package。
...
`framework-release` owns all release side effects after handoff:

1. confirm workspace repo and template repo state
2. integrate the validated task head into workspace main（prefer fast-forward; fail loud if impossible）
3. write / verify VERSION and CHANGELOG when a version bump is required
4. commit and push workspace main
5. run `sync-to-polaris.sh --push`
6. verify template commit, tag, GitHub release, repo cleanliness, and active GitHub account restoration

Maintainer Release role must not write fake `deliverable.pr_url`. Until release deliverable helper lands, report release metadata explicitly and mark task lifecycle as pending if it cannot be recorded safely.

### 7a. Evidence AND Gate（pre-PR / pre-release verification）

Before creating PR 或 maintainer release handoff，驗證所有必要 evidence 檔案存在且 `head_sha` 匹配當前 HEAD：

| Layer | Evidence file | Required? |
|-------|--------------|-----------|
| A (CI) | `/tmp/polaris-ci-local-{branch}-{head_sha}.json` | Always（if `ci-local.sh` exists） |
| B (Verify) | `/tmp/polaris-verified-{ticket-or-dp-task-id}-{head_sha}.json` | Developer / Maintainer Release with verify_command |
| C (VR) | `/tmp/polaris-vr-{ticket}-{head_sha}.json` | Only if Step 3.5 triggered |

Missing 或 stale evidence → **halt**。不繼續 PR creation 或 maintainer release handoff。

**Portable gate fallback**：`gate-evidence.sh` + `gate-ci-local.sh` 在 git pre-push 及 `polaris-pr-create.sh` wrapper 中也強制檢查；`gate-pr-title.sh` + `gate-changeset.sh` 在 `polaris-pr-create.sh` 與 completion gate 中強制檢查；`gate-pr-body-template.sh` 在 `polaris-pr-create.sh` 中阻擋未保留 repo template headings 的 body。Skill-level check here is **L2 cross-LLM authoritative**（所有 LLM 都走 SKILL.md → 一定到這步）。
...
`polaris-pr-create.sh` wrapper（DP-032 Wave δ）在 `gh pr create` 前依序執行 gate-base-check + gate-evidence + gate-ci-local + gate-pr-title + gate-pr-body-template + gate-changeset — `--base` 值與 resolve 結果不符、Developer title 不符、PR body 未保留 repo template headings、或 task changeset 缺失時 wrapper 直接 block。
...
Maintainer Release mode 暫行（`check-delivery-completion.sh --maintainer-release` 尚未落地）：
...
再以 `framework-release` final verification（workspace commit、template commit、tag/release、兩 repo clean、GitHub account restored）作為 release completion evidence。不得呼叫 Developer completion gate 後忽略其 PR deliverable failure。

### Script contract（Developer / Admin；Maintainer Release helper 待補）
...
Completion gate 不查詢或等待遠端 repo CI。只要本地 LLM gates 與 mechanical evidence gates 已通過，queued / pending / running 的遠端 CI 不阻擋完成回報。Maintainer Release lane 另需 `framework-release` final verification，不能只靠本地 evidence gates 回報完成。
...
PR 建立 / 既有 PR branch push 完成，或 maintainer release final verification 完成後，task deliverable / release metadata 已回寫、Completion Gate PASS，才清掉本次 implementation worktree。不要手動猜路徑或直接 `rm -rf`，一律用 helper 做 guard 後清理：
...
4. Layer A + B（+ C if triggered）must ALL be present and PASS before PR creation or maintainer release handoff — **AND gate, not OR**
...
| `check-delivery-completion.sh` | user-facing completion report | Layer A always if `ci-local.sh` exists; Layer B for Developer / Maintainer Release | 四通 |
