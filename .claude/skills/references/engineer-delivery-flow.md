...
若 repo 有 `.changeset/` 目錄：

```bash
bash "${POLARIS_ROOT}/scripts/polaris-changeset.sh" new --task-md "<path/to/task.md>"
```
...
**L1 hook fallback**：`verification-evidence-gate.sh` + `ci-local-gate.sh` 在 `gh pr create` / `git push` 時也強制檢查。Skill-level check here is **L2 cross-LLM authoritative**（works in Codex/Cursor too）。
...
**Hook 保險**：`.claude/hooks/pr-base-gate.sh`（DP-028 D4）同時擋 `gh pr create --base <X>` 與 `gh pr edit --base <X>` 不符 resolve 結果 — 跳過 resolve helper 在 Step 7d 或 R0 步驟 4 都會被 block。
...
### Hook Enforcement（Claude Code）

| Hook | Trigger | Checks |
|------|---------|--------|
| `ci-local-gate.sh` PreToolUse | `git commit` / `git push` / `gh pr create` | Layer A evidence |
| `verification-evidence-gate.sh` PreToolUse | `gh pr create` / `git push` | Layer B evidence（+ Layer C if VR was triggered per task.md） |
| `no-direct-evidence-write.sh` PreToolUse | Write/Edit on `/tmp/polaris-verified-*` / `/tmp/polaris-ci-local-*` / `/tmp/polaris-vr-*` | **Blocks** — LLM cannot forge evidence |

### Cross-LLM（Codex/Cursor — no hooks）

- L2 skill embed in this flow（Step 7a AND gate）provides authoritative check
- `writer` whitelist in `verification-evidence-gate.sh` catches LLM-forged evidence
