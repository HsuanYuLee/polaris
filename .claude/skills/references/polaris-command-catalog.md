...
DP-202 後，root runtime toolchain 的 canonical entrypoint 是 `mise run <task>`。Root
`pnpm ...` commands 只代表 package-local thin aliases 或既有相容入口；`pnpm` 不再被視為
Polaris root runtime manager。
...
| 用途 | 指令 |
|--------|---------|
| 初始化 required framework runtime toolchain | `mise run bootstrap` |
| 檢查 required framework runtime toolchain | `mise run doctor -- --profile runtime` |
| 輸出 required capability manifest | `pnpm toolchain:manifest` |

`pnpm toolchain:*` 若存在，只能是相容用 thin aliases；新增 root runtime 行為應落在
`mise.toml` 的 public task，並由對應 Polaris wrapper 實作。
