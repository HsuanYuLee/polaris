...
A per-repo architectural document that gives AI agents (and humans) a structured understanding of what a repository is and how it works.

Source-of-truth lives in the Polaris workspace under `{company}/polaris-config/{project}/handbook/`. A repo-local copy at `{repo}/.claude/rules/handbook/index.md` is only an ignored compatibility overlay when tooling needs repo-cwd files.

DP-079 ownership stance: Polaris is an add-on harness, not the owner of repo AI config. Repo root `CLAUDE.md` / `AGENTS.md` / Copilot files are repo-owned adapter surfaces; `AGENTS.md` is not required, and native adapter deferred is the default until the repo owner opts in.
...
- 所有檔案都在 workspace-owned `polaris-config` 下；sub-agent 必須依 consumer protocol 明確 Read，不依賴 repo native auto-loading
...
2. **「換同公司另一個 repo 還適用嗎？」**
   - Yes → **company handbook**（`{company}/polaris-config/handbook/`）
   - No → **repo handbook**（`{company}/polaris-config/{project}/handbook/`）

3. **更新後通知**：
   - repo handbook → 記錄更新路徑；同 repo sub-agent 的後續 dispatch 必須重新讀 SoT
...
```
{company}/polaris-config/handbook/
...
Company handbook 在 workspace-owned `polaris-config` 下，由 consumer skill / Strategist 明確讀取或注入。不要依賴 workspace root `.claude/rules/` 自動載入。
...
1. **Read the handbook** — `{company}/polaris-config/{project}/handbook/index.md`
...
1. **Check for handbook**: `{company}/polaris-config/{project}/handbook/index.md`
...
- Path: `{company}/polaris-config/{project}/handbook/index.md`
- Workspace-owned local config; gitignored in the workspace and never committed to the product repo
- Repo-local ignored overlays under `{repo}/.claude/rules/handbook/` are allowed only for legacy/native tooling compatibility
- Maintained by writing the workspace-owned `polaris-config` source of truth directly; no deploy/copy-back step is part of the steady-state flow
...
### 總量
handbook/ 不假設 native 自動載入。成熟 repo 的 handbook 可能有 10+ 子文件 + 數千行；consumer skill 必須讀 index 與必要子文件，不靠 repo AI config 注入。
