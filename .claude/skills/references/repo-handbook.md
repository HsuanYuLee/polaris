...
- Workspace-owned and tracked by Polaris where appropriate; never committed to the product repo
- Repo-local ignored overlays under `{repo}/.claude/rules/handbook/` are allowed only for legacy/native tooling compatibility
- Managed by `polaris-sync.sh` only when a repo-local overlay is explicitly required
