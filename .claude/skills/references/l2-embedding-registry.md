...
> 來源：DP-030 BS#8（`specs/design-plans/DP-030-llm-to-script-migration/plan.md`）。Phase 1 POC 落地 `cross-session-carry-forward`（L2+L1）+ `no-cd-in-bash`（L1 only）。Phase 2 逐 canary 擴充時必須同步更新這份 registry。
