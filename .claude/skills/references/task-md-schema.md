...
Framework design-plan work orders（DP-047）使用同一份 Implementation Schema，只是 root 從 product specs folder 換成：
...
| Cell | 內容 | Required |
|------|------|----------|
| `Task JIRA key` | 該 task 的 JIRA key（如 `KB2CW-3900`）；DP-backed task 使用 pseudo-task ID（如 `DP-047-T1`） | **Hard** |
| `Parent Epic` | Epic key（如 `GT-478`）；DP-backed task 使用來源 DP（如 `DP-047`） | **Hard** |
...
| `Branch chain` | 從 integration branch 到本 task branch 的完整 rebase 鏈（例：`develop -> feat/GT-478-... -> task/KB2CW-3711-... -> task/KB2CW-3900-...`）；engineering 用 `scripts/cascade-rebase-chain.sh` 消費；PR base 仍由 `Base branch` + `resolve-task-base.sh` 決定 | **Soft**（新 breakdown 必填；legacy task 缺漏時 reader fallback） |
...
具體 instance 見 `kkday/specs/GT-478/tasks/T1.md`、`T9.md`（或完結後的 `kkday/specs/GT-478/tasks/pr-release/T1.md`）。
