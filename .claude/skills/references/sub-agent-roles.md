...
## Completion Envelope Standard

All sub-agents must return results in a structured envelope so the orchestrator can programmatically determine success/failure without parsing prose. This applies to every role below.

**Mandatory fields** (first 3 lines of every sub-agent return):

```markdown
## Status: {DONE | BLOCKED | PARTIAL}
**Artifacts**: {list of files created/modified, URLs, ticket keys — or "none"}
**Summary**: {one-sentence result}
```

- `DONE` — task completed successfully. Orchestrator proceeds.
- `BLOCKED` — cannot proceed. Must include a `**Blocker**:` line explaining why.
- `PARTIAL` — some items completed, some not. Must include a `**Remaining**:` line listing what's left.

After the envelope header, include role-specific details in the format defined by each role's 回傳格式 section below. The envelope is **in addition to**, not a replacement for, role-specific output.

**Why**: Without a standard envelope, the orchestrator must read the full sub-agent output to determine if it succeeded. With it, a 3-line scan suffices — especially important when running multiple parallel sub-agents.
