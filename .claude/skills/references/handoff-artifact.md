...
Skill pipeline (`refinement → breakdown → engineering → verify-AC → bug-triage`) currently hands off **conclusion documents only** (refinement.md, task.md, plan.md, JIRA comments). Raw tool return — grep matches, error traces, endpoint responses, commit hashes — gets lost after the producing skill's session ends.
