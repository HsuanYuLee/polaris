...
| 範例 |
|------|
| `[TASK-3788] 產品頁 BreadcrumbList 加首頁 entry` |
| `[EPIC-521] BreadcrumbList SEO 優化` |
...
```md
## AC Coverage
- [x] AC1: 產品頁 breadcrumb 含首頁 entry → [驗證報告](https://example.atlassian.net/browse/TASK-3791)
- [x] AC2: JSON-LD 移至 `<head>` → [驗證報告](https://example.atlassian.net/browse/TASK-3792)
- [ ] AC3: GA 事件追蹤（out of scope, 見 EPIC-522）
```
...
5. 尚無 RCA → 詢問：「Bug 單尚無 RCA 紀錄。要順便補寫嗎？」
   - 確認 → 提示改走 Bug triage / refinement RCA flow；不再 chain 到已 sunset 的 dedicated RCA skill
   - 拒絕 → 不阻擋
...
`polaris-pr-create.sh` 會先執行 `gate-work-source.sh`，確認 current branch 有合法
`task.md` source；此 gate 不可用 `--skip-gates` 跳過。之後才執行
`gate-pr-body-template.sh` 與 PR language gate，確認 body 保留 repo PR template 的 `##`
section headings 且符合 workspace 語言政策；請使用 `--body-file`，不要用 shell inline
`--body` 手拼 Markdown，避免 backtick / code block 被 shell escape 弄壞。裸
`gh pr create`、generic publisher、source-less PR 與 `gh pr create --draft` 不可作為
Developer delivery endpoint；completion gate 會再讀 remote PR metadata/body，阻擋
draft、非 open、missing required assignee、或 invalid remote PR body。Codex / non-Claude
runtime 不保證會套用 Claude Code hook；若必須從 Codex 路徑建立 PR，使用
`scripts/codex-guarded-gh-pr-create.sh` 或直接呼叫 `scripts/polaris-pr-create.sh`。
...
- Do: create 走 `polaris-pr-create.sh`，並確保 branch 能 resolve 到合法 task source
- Do: edit 先跑 template gate + language gate
- Don't: 裸用 `gh pr create`、generic publisher、`gh pr create --draft`、或 `gh pr edit --body`
