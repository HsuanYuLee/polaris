...
這份 state 只作為 local planning / standup handoff；它不是 shared workflow state，也不能
覆蓋 JIRA、PR、verification、或 release gate 的 authoritative result。
...
`standup` 只能把它當成排序與 progress comparison 的輔助訊號；不得把 triage state 寫成
`mergeable_ready`、`release_completed`、或其他 shared stage conclusion。
