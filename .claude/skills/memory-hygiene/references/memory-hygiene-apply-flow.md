...
```bash
set -o pipefail
.claude/skills/memory-hygiene/scripts/memory-hygiene-tiering.py dry-run --memory-dir "{memory_dir}" --json \
  | bash .claude/skills/memory-hygiene/scripts/validate-memory-hygiene-plan.sh \
  | .claude/skills/memory-hygiene/scripts/memory-hygiene-tiering.py apply --memory-dir "{memory_dir}"
```
