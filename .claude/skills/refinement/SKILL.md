...
**`human` 問的是誰想的，不是那段字掛在誰的帳號底下。** 這兩件事會分開，而分開的時候沒有
任何顯示看得出來：對外送出的留言（ticket 的 comment、PR 的 review 與回覆）掛的是那個人的
帳號，內容卻可能是上一輪的我寫的。**帶著「由 Claude Code 代發」這個標記的東西就是那一種
——它是我自己上一輪的輸出，不是任何人的意圖。**

所以那樣的一段字：

- **不得標成 `source: human`。** 它連 `inferred_confirmed` 都不是——那一格是「我推的、他
  點頭了」，而一則沒有人回過的留言沒有那個點頭。
- **不得當成規格權威。** 提單的人寫在描述正文裡的東西才是；我自己的留言只是我自己的話。
- **不得拿去反駁提單的人。** 「你單上寫的」在這種情況下是假的。

2026-08 量到的一條六步的鏈：提單的人在描述裡列了六個**建議**之一 → 我在自己的留言裡把它
升格成一個工作項目 → 那則留言用他的帳號送出 → 下一輪讀回來，「規格權威：那則留言，不是
描述正文」 → `plan.what` 照抄它並標 `source: inferred_confirmed` → 斷言照它簽。使用者事後
的原話是「你創造一個荒謬的做法然後說是我做的」。

**沒有標記的舊留言分不出來，這一點要說出來。** 標記是這一版才開始加的，在那之前送出去的
東西讀起來就是那個人寫的。碰到分不出來的，問人，不要挑一個看起來比較順的答案。
...
### 改一個共用輸出的來源時，消費者的清單要跨得出你站的那個 repo

**判準是「這個值換了來源之後，誰還在讀它」，而清單的邊界是那個值真的流到的每一個地方，
不是列清單的人站的那一個 repo。** 共用欄位、共用回應、共用索引欄位——只要這一版改的是它的
來源、或就地覆蓋它的值，這一條就適用。

那份清單是一條現況主張，所以它照上面那三欄寫（`claim`／`verified_by`／`observed`），不另立
格式。要多寫的只有一件事：**`verified_by` 那條命令掃過哪些地方，以及哪些地方你沒有掃、
為什麼。** 不在本機的（另一個團隊的服務、mobile 那一端、只在 CI 裡存在的中間層）也要寫出來
——**寫出「這裡我看不到」跟沒有提到它，在下一個人眼裡差很多。**

2026-09-15 的標本：一張單把商品簡述換成讀新欄位，`verified_by` 是
`grep -rni introduction <一個 repo>/app/`，`observed` 逐處列了四個讀取點——**清單有、驗過了、
行號都對**。漏掉的兩個消費者不在那個 repo 裡：一個把這個值當作沒設特色時的商品特色印在
畫面上，另一個把它印成搜尋卡與高亮卡的文字。同一個 needle 逐 repo 掃過去，
命中數是 17／26／41／68／0（2026-09-15 量到的五個 repo，作者掃的是第一個），
而清單只涵蓋第一個。**needle 沒有錯，範圍錯了。**

那一版的 assertion 因此只涵蓋作者自己改到的三處 SEO 欄位，判定全綠，問題是看 diff 的人問
出來的，時間點在上線前。

**這一格沒有關卡在守，是刻意的**（照 `CLAUDE.md`〈檢查類腳本的門檻〉）：那一次擋下它的正是
看 diff 的人，所以「看 diff 的人看不出來」那一條不成立，三個條件缺一就不要寫。
...
### 重簽：逐條對回原始理由，而那顆 commit 要對得上一筆 clearance

**重簽不是「把被點名取消的那幾條刪掉」。** 設計換了之後，fence 上每一條都要被問一次同樣的
問題：**這一條當初是為了什麼而在？那個理由現在還在嗎？** 理由消失的要跟著退場——沒有人會
來點名它，而它會留在 fence 上，看起來跟還成立的那幾條一模一樣。

範圍變了的時候特別會留下這種條目：搬出改成共存、兩層收成一層、某一端不做了。被拿掉的那一
端原本有幾條 assertion 在守，那幾條現在守的是一個不存在的東西。

**重簽的依據要留在停點那一格，不是留在 commit 訊息裡。** 停 `assertion_wrong` 再解停點的
時候，`advance --authorization` 收的那句話就是依據，它落在 `{issue}/.spine/loop-state.json`
的 `clearances[]`（帶 `by` 與 `at`）。**所以一顆重簽的 commit 要對得上一筆 clearance**——
對不上就表示這一次重簽沒有人授權過，而 commit 訊息裡寫不寫得漂亮不改變這件事。

反過來也要說：**commit 訊息裡看不到依據，不等於沒有依據。** 依據的家是 `clearances[]`，
要查就查那裡。

**施工的那一站不自己往 fence 裡加 A-P 條目。** 施工途中發現 assertion 不夠，那是
`assertion_wrong` 停點，走回這一站——那條路會留下一筆 clearance 與一顆有人看得到的 commit；
就地擴寫不會，而擴寫過的 fence 跟一開始就簽成那樣的 fence 讀起來完全一樣。
