...
```
Round 1-N（本地迭代）
  .claude/designs/{EPIC_KEY}/refinement.md  ← Strategist 每輪更新
...
```bash
python3 scripts/refinement-preview.py .claude/designs/{EPIC_KEY}/refinement.md
...
將 Step 1-5 的所有產出整合寫入 `.claude/designs/{EPIC_KEY}/refinement.md`：

```markdown
# Refinement — {EPIC_KEY}: {Summary}

> Tier: {N} | Round: {M} | Date: {YYYY-MM-DD}
...
根據 `references/refinement-artifact.md` schema 產出 `.claude/designs/{EPIC_KEY}/refinement.json`。
