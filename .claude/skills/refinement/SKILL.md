...
- refinement 產生的 external write drafts、JIRA comment body、manual validation output 若要保留，
  ...
  或 `artifacts/research/`；`.codex/external-writes/` 只能作為 temporary transport cache，
  不可作為 durable artifact。
...
- temporary body files：gate pass 後刪除，不得留下 `.codex/external-writes/` residue。
