...
**Tier 2+ 從 Phase 1 Step 2（Codebase Exploration + Runtime 驗證）開始前建立 worktree — 不動使用者的主 checkout。**

Refinement Tier 2+ 會跑 build / 起 dev server 取 baseline 數據（Lighthouse、curl SSR 輸出等），這些操作會干擾使用者 WIP。完整規則（何時建、絕對規則、執行流程、canary、sub-agent dispatch、例外條款）見共用 reference：

→ [planning-worktree-isolation.md](../references/planning-worktree-isolation.md)

Refinement 專用參數：
- `{skill}` = `refinement`
- `{TICKET_KEY}` = Epic key（如 `PROJ-123`）
- Base ref = `origin/develop`
- 輸出目錄 `{company_base_dir}/specs/{EPIC_KEY}/` 在主 workspace，與 worktree 無關（豁免於 worktree 要求）
...
**§ 子單結構 preview**：跑 `skills/references/infra-first-decision.md` 的決策樹（以 Step 4 hardened AC 的 `verification.method` + Step 2 `modules[].api_change` 為輸入），將輸出摘要寫在 § 子單結構 table 之前。這一步確保 refinement preview 與 breakdown Step 5.5 的實際產出一致（同一份邏輯，不同時機執行）。
...
> infra-first 評估（見 `skills/references/infra-first-decision.md`）：需要 {N} 張 infra 前置子單（{原因，如：AC 含 lighthouse + playwright → 需 Mockoon fixtures + VR baseline}）。總計 {N+M} 張，預估 {total} pt。
> 若 `skipped: true` → 顯示 skip reason（i18n_only / static_config / no_runtime_ac / ...）。

| # | Key | Summary | Points | 備註 |
|---|-----|---------|--------|------|
| 1 | TBD | [infra] Mockoon fixtures... | 2 | 前置 |
| ... |
