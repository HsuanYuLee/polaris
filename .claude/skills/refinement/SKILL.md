...
name: refinement
description: >
  Iteratively enriches incomplete JIRA Epics into estimation-ready specs. Reads the Epic + codebase
...
```
── 問題分析 ─────────────────────────────
📍 位置：apps/main/composables/useProductPrice.ts
...
Summary:  [b2c] 重構 useProductPrice composable — 加入 error handling、cache、測試
Priority: Medium

Description:
## 背景
useProductPrice composable 被 12 個檔案引用，但缺乏 error handling、cache 和測試。
API 不穩時會導致商品頁白屏。
...
2. 60 秒內的重複請求走 cache
3. useProductPrice 測試覆蓋率 ≥ 80%
4. 拆為 useProductPriceFetch + useProductPriceFormat

## Scope
- 修改：composables/useProductPrice.ts
- 新增：composables/useProductPrice.test.ts
- 影響：需確認 12 個使用者不受影響

## QA 影響範圍
- 商品頁價格顯示（正常 + API error）
- 所有使用 useProductPrice 的頁面
```
...
| **Scope（影響範圍）** | 涉及哪些頁面、元件、API | 「商品頁 PriceSection 元件 + /api/product/price endpoint」 |
...
5. 多幣別 → 依使用者設定的幣別顯示（沿用現有 useCurrency composable）

📝 Scope（根據 codebase 分析）：
- 修改：apps/main/pages/product/[id]/_components/PriceSection.vue
- 修改：apps/main/composables/useProductPrice.ts
- 新增：server/api/product/price.get.ts（如果 BFF 需要）
- 影響：Design System 的 KkPrice 元件可能需要支援 loading state

📝 Edge cases（根據現有程式碼推測）：
- 目前 PriceSection 沒有 error handling，需要加
- useProductPrice 目前只拉一次資料，切換日期需要改成 reactive

📝 依賴：
- 需確認 /api/product/price 是否已上線（目前 codebase 中沒有這個 endpoint）
- KkPrice 元件是否需要改？如果需要，要先開 DS 的單
...
```
── Option A: 擴展現有元件 ──────────────
Approach:  修改 KkPrice 元件，加入 loading/error state
Pros:      複用現有元件，改動範圍小
...
**Decision**: Option A — 擴展 KkPrice 元件
**Reason**: 複用性高，長期維護成本低
**Key decisions**:
...
- Atlassian MCP 已連線
- 對應專案已 clone 到 ~/work/（用於 codebase 分析）
