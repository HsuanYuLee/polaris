...
```
Round 1-N（本地迭代）
  {company_base_dir}/designs/{EPIC_KEY}/refinement.md  ← Strategist 每輪更新
...
```bash
python3 scripts/refinement-preview.py {company_base_dir}/designs/{EPIC_KEY}/refinement.md
...
將 Step 1-5 的產出整合寫入 `{company_base_dir}/designs/{EPIC_KEY}/refinement.md`（ticket workspace 模式，見 memory: `project_designs_at_company_level`）。

**產出格式規則：只放實作需要的資訊，不放討論過程或歷史沿革。**

完整性 checklist、歷史比較分析、「上次為什麼走錯」等推導過程是討論階段的工具，不進入產出文件。下游（breakdown, work-on）只需要知道「該怎麼做」，不需要知道「為什麼上次搞砸」。
...
根據 `references/refinement-artifact.md` schema 產出 `{company_base_dir}/designs/{EPIC_KEY}/refinement.json`。
