...
| auto-pass amendment dispatch（DP-backed source LOCKED + inbox 出現）| Auto-pass driven amendment | `refinement-dp-source-mode.md` § LOCKED Scope Guard |
...
mode：只消費 `{source}/refinement-inbox/*.md`、不向使用者發問、不重做 Phase 0/1/2 discovery、
