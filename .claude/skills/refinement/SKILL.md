...
|-------------|--------|-----------|------|
| `jira` | `refinement EPIC-478` | `{company_specs_dir}/{TICKET}/` + JIRA issue | 既有 JIRA-backed refinement；定版後寫 JIRA comment / label / description |
...
Round 1-N（本地迭代）
  ...
  docs-manager Starlight route                ← browser 即時預覽（大螢幕討論用）
  ↕ 使用者 + 其他 RD 討論、修正
...
# → http://127.0.0.1:8080/docs-manager/
```

**Flow：**
1. Phase 1 Step 1-4 的產出寫入 `{source_container}/refinement.md`（不寫 JIRA）
2. 啟動或重用 docs-manager，使用者在 browser 查看 Starlight route、和團隊討論
3. 使用者回饋 → Strategist 更新 `refinement.md` → docs-manager dev route 自動刷新
4. 重複直到使用者說「定版」
...
- DP `plan.md` / `refinement.md` 每次建立或更新後，必須依
  ...
  docs-manager dev/preview verification、使用者 review 或 downstream handoff。
...
- 讀取該 repo 的 handbook（`{company}/polaris-config/{project}/handbook/`）
...
curl -s https://www.example.com/zh-tw/product/12156 | python3 check_jsonld_position.py
```
...
將 Step 1-5 的產出整合寫入 `{company_specs_dir}/{EPIC_KEY}/refinement.md`（ticket workspace 模式，見 memory: `project_designs_at_company_level`）。
...
**5c. 啟動 docs-manager preview**

```bash
bash scripts/polaris-toolchain.sh run docs.viewer.dev
```

告知使用者：「Preview 已開在 docs-manager，可以在大螢幕上跟團隊討論。有修改告訴我，我更新 markdown 後 Starlight route 會刷新。定版後說「寫回 JIRA」。」

**不寫 JIRA** — 所有中間輪次的產出留在本地 markdown，省去每輪寫入成本。

### Step 6 — 多輪迭代（本地）

使用者或團隊提出修改 → Strategist 更新 `refinement.md` → docs-manager route 自動刷新。
...
- 保持或關閉 docs-manager dev server，依使用者是否還需要瀏覽 specs 決定
