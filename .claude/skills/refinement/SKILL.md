...
## Source-Level Verification Strategy (DP-364)

Refinement 在 LOCK 前擁有 source-level AC verification strategy 決策，並寫入
`refinement.json.verification_strategy`。下游 `breakdown` / deterministic gates 只消費此
structured field，不以 `source.type` 分路。

- `mode=per_task_self_verify`：每個 T task 以自身 `verification.verify_command` 自驗；
  `tasks[]` 不要求 V task。
- `mode=source_level_v_required`：LOCK 前 `tasks[]` 必須含 V task；V task 走
  V-specific schema / deps readiness，不套 T-task Allowed Files / Scope Trace /
  Gate Closure checklist。
- `mode=external_ac_ticket`：source-level AC 由外部 AC ticket 承接；strategy 必須帶
  ticket identity。

所有 mode 都必須填 `reason` 與 `authority`，讓下游可回溯此 source 為何採用該驗收形態。
...
- **`tasks[].repo`**：optional per-task repo override。cross-repo same-Epic task 若落在不同
  product repo，必須在 task entry 寫入該 repo；缺欄位時 fallback `source.repo`。
- **`tasks[].base_branch`**：optional per-task base branch override。若 task 宣告
  `tasks[].repo`，base branch 必須讀該 task repo 的
  `{company}/polaris-config/{repo}/handbook/config.yaml` `base_branch`；缺欄位時 fallback
  `source.base_branch`。

### Base Branch Resolution（不得硬猜 — EC3）

解析 `source.base_branch` 或 `tasks[].base_branch` 時：

1. 依目標 repo 讀 `{company}/polaris-config/{project}/handbook/config.yaml` 的
   `base_branch` 欄位；per-task repo override 使用該 task repo 的 config。
2. **有對應 entry** → 寫入 `source.base_branch` 或 `tasks[].base_branch`。
...
> `tasks[].jira_key` / `tasks[].repo` / `tasks[].base_branch`；
> `validate-refinement-json.sh` 對 dp source 出現任一 jira-only 欄位以
> `POLARIS_REFINEMENT_JIRA_ONLY_FIELD` fail-closed（jira-only 鬆綁不外洩到 dp 分支）。
