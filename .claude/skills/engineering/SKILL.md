...
> **⚠️ Step 2（`ci-local.sh`）在 revision mode 尤其重要**：revision 常見原因就是 CI fail（codecov patch coverage 不足等）。修完後必須重跑 `ci-local.sh` 確認本地模擬 CI 通過，否則 push 後 CI 可能再次失敗。`gate-ci-local.sh` 在 git pre-commit / pre-push 擋 evidence cache miss — revision mode 的 push 若 cache miss，gate 會同步實跑 `ci-local.sh`（FAIL 物理擋下）。
