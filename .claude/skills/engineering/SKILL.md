...
1. **偵測 base branch**：經 `scripts/parse-task-md.sh <task_md_path> --field base_branch` 取得 Operational Context 的 `Base branch` 字面值（rebase 實際 target 仍經 `resolve-task-base.sh`，見 § Base Branch Resolution），或以 JIRA parent 查 feature branch
...
1. **定位 task.md**（hook 同邏輯）：
   ```bash
   TASK_MD=$("${CLAUDE_PROJECT_DIR}/scripts/resolve-task-md-by-branch.sh" --current)
   ```
   找不到 → fallback 到 PR 的 `baseRefName`（legacy PR 無施工圖場景）
2. **取 resolved base**（不讀 PR `baseRefName`；讀 task.md 經 resolve layer）：
   ```bash
   RESOLVED_BASE=$("${CLAUDE_PROJECT_DIR}/scripts/resolve-task-base.sh" "$TASK_MD")
   ```
3. **Rebase onto RESOLVED_BASE**（依 `references/cascade-rebase.md` 邏輯，含 cascade if needed）。Conflict → **停止**，回報使用者，不進入 R1
4. **PR base sync**（DP-028 gap fill）：
   ```bash
   PR_BASE=$(gh pr view "$PR_NUMBER" --json baseRefName --jq .baseRefName)
   if [[ "$PR_BASE" != "$RESOLVED_BASE" ]]; then
     gh pr edit "$PR_NUMBER" --base "$RESOLVED_BASE"
   fi
   ```
   `pr-base-gate.sh` hook 會同時擋 `gh pr edit --base` 不符 resolve 結果——跳過 resolve helper 就會在這裡被 block。

> **為什麼不讀 PR baseRefName**：PR 可能建立時就指向錯 base（pre-DP-028 手動建 PR 或其他漂移），把它當事實會複製錯誤。task.md 是 snapshot source-of-truth，resolve helper 是動態調整層。
...
修正完成後（或 rebase-only 無修正時），重跑完整 engineer-delivery-flow：

讀取 `references/engineer-delivery-flow.md`，以 **Role: Developer** 執行 Step 1-8。
這確保修正後的 code 仍通過所有品質門檻（Simplify → **Local CI Mirror (Step 2, `ci-local.sh`)** → Behavioral Verify Layer A+B → Pre-PR Review → Rebase → Commit → Push）。
