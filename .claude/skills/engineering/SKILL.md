...
1. **新格式（優先）** — 以 JIRA key 定位 task.md：
   ...
   **取得路徑後，所有欄位讀取一律走 `scripts/parse-task-md.sh <task_md_path> [--field <key>]`（DP-032 D8）**——本 grep 只用於檔案定位，不可拿來抽 section 內容。
...
- Branch setup：`bash "${POLARIS_ROOT}/scripts/engineering-branch-setup.sh" "<path/to/task.md>"`（D4 — worktree isolation + branch naming + base resolve + polaris-sync deploy 一站完成）
...
- **Test Command**：以 `scripts/parse-task-md.sh <task_md_path> --field test_command` 取得指令；非空 → **必須使用該指令跑測試**（不可自行推導）。回傳空（task.md 無此欄位）→ 讀專案 CLAUDE.md 或 workspace-config 取得正確指令
- **測試環境硬門檻**：test command 執行失敗（exit ≠ 0、`#imports` resolver error、vitest config 找不到）→ **立刻停止，回報問題**，不可靜默跳過或假設 CI 會處理
- **Test Environment 消費硬門檻**：以 `scripts/parse-task-md.sh <task_md_path> --field level`（或 `--field env_bootstrap_command` / `--field runtime_verify_target` / `--field fixtures`）取得 Test Environment 各欄位，不可自行 grep `## Test Environment`。若 `level=runtime`，必須先執行 `env_bootstrap_command`，再對 `runtime_verify_target` 做連線檢查（至少一次 HTTP request；接受 2xx 或任務明確預期碼），不可跳過
- **Runtime 一致性硬門檻**：`level=runtime` 但 `verify_command`（同樣經 parser 取得）只做 static 檢查（僅 grep/檔案存在性）時，停止並回報「work order 不一致」，要求回 upstream 修正 task.md，不得自行當作通過
- **Test Environment（Verify Command 前置）**：依 `parse-task-md.sh --field level` 回傳值分流：
  - `static` → 不需起環境，直接跑 Verify Command
  - `build` → 先跑 `pnpm build`（或 repo 對應 build 指令）產 `.output/` / `dist/`，無需啟動 dev server
  - `runtime` → 跑 D11 L3 orchestrator：`bash {polaris_root}/scripts/start-test-env.sh --task-md {task_md_path} [--with-fixtures]`。orchestrator 鏈接 `ensure-dependencies → start-command → health-check → [fixtures-start]`，自己讀 task.md 抽 project name + fixture path，讀 workspace-config 推 dependencies / start_command / health_check / cwd；每步輸出 JSON 證據，任一 FAIL → exit 1。`--with-fixtures` 由 `parse-task-md.sh --field fixtures` 是否非空決定（非空 → 加 flag；orchestrator 會自抽 fixture path）。**不要自行 `docker compose up` / `pnpm dev` / `mockoon-runner.sh start`**，這些已被 orchestrator 包住
  - 啟動失敗 → **停止，回報使用者**，不繼續跑 Verify Command（Verify 會假 pass 或誤報 fail）。回報時列出 orchestrator 哪一步 FAIL（從 stdout JSON `step` 欄位讀）
- **TDD 智慧判斷**：依 `references/tdd-smart-judgment.md` 判斷每個檔案是否走 TDD 循環
- **Local CI Mirror (Dimension B)**：若 repo 有 `scripts/ci-local.sh`（由 `scripts/ci-local-generate.sh` 從 repo CI config 推導產出，見 `references/engineer-delivery-flow.md` § Step 2），會在 push 前模擬跑 repo CI；repo 沒配就跳過。patch coverage 歸 repo 責任，不以「改動小」為由豁免 repo 自己的 CI 要求
- 發現情況不同時，在 JIRA 新增 comment 標註修正版

### 3. 交付流程（Two-Segment Architecture，DP-032 D21）
- 讀取 `references/engineer-delivery-flow.md`，以 **Role: Developer** 執行完整步驟
- **LLM 實作段（Phase 3，可迭代）**：Simplify → Self-Review（Step 1.3，Phase 3 exit gate）
- **機械自驗段（Phase 4，線性 fail-stop）**：Step 1.5 Scope Gate（`check-scope.sh`，D20）→ Step 2 前置 Rebase（`engineering-rebase.sh`，D6）→ Step 2 Local CI Mirror（`ci-local.sh`，D12）→ Step 3 Verify（`run-verify-command.sh`，D15）→ Step 3.5 VR（conditional，D18）→ Step 5 Base Freshness（`check-base-fresh.sh`，D19）→ Step 6 Commit → Step 7 PR（含 Evidence AND Gate）→ Step 8 JIRA
- **Phase 3 exit condition**（D21）：test 綠 + simplified + Self-Review `passed: true`。任一項未滿足 → 不進 Step 1.5 機械自驗段。Self-Review blocking → 回 Phase 3（不只回 /simplify），hard cap 3 輪，**NO bypass**
- **Revision mode R5 不重跑 Phase 3**（含 Self-Review）— R5 只跑機械自驗段（Layer A+B+C evidence）
...
- **格式**：frontmatter（`skill: engineering`、`ticket: {ticket_key}`、optional `scope: revision`、`timestamp`、`truncated: false`、`scrubbed: false`）+ `## Summary`（≤ 500 字：實作要點、test/coverage/quality pass 狀態、Layer B behavioral verify 結論）+ `## Raw Evidence`（final commit SHAs、test command + 完整輸出、`ci-local.sh` 結果、Layer B 行為驗證（curl output / screenshot 路徑 / dev-server logs）、evidence-file JSON 內容）
...
ticket 狀態是「開放」或「SA/SD」？
  └→ JIRA transition → IN DEVELOPMENT（`polaris-jira-transition.sh {TICKET} in_development`，D25 soft-fail）→ 繼續建 branch
...
ticket 狀態是 IN DEVELOPMENT 且沒有 branch？
  └→ **AC-FAIL Bug 偵測**：ticket description 或 comment 含 `[VERIFICATION_FAIL]`？
     ...
     └→ 否 → 使用 `engineering-branch-setup.sh`（D4）：
         `bash "${POLARIS_ROOT}/scripts/engineering-branch-setup.sh" "<path/to/task.md>"`
         （script 自動處理 worktree isolation + branch naming + base resolve + dependency branch detection + polaris-sync deploy）
...
### 4.5. Pre-Development Rebase（強制，D6）

Branch checkout 完成後、開發前，rebase 到最新 base。使用 `scripts/engineering-rebase.sh`（D6）：
...
| stdout | 動作 |
|--------|------|
| `REBASE_NOOP` | 繼續 Step 5 |
| `REBASE_OK` | 繼續 Step 5（post-rebase hook 已自動清理 inherited changeset，D24） |
| `REBASE_CONFLICT: <files>` | **停止**，回報衝突檔案。Conflict resolution 是 LLM semantic work，解完後 resume |

Admin mode（無 task.md）：同行為。
...
2. **TDD 開發**：讀取 `unit-test` SKILL.md + 專案 CLAUDE.md，以 Red-Green-Refactor 循環實作。依 `references/tdd-smart-judgment.md` 判斷哪些檔案走 TDD。**Test Command**：task.md 若有 `## Test Command` → 必須用該指令；無則讀專案 CLAUDE.md 取得。**測試環境硬門檻**：test command 失敗 → 停止確認，不靜默跳過。**Coverage Gate**：若 repo 有 `ci-local.sh`（patch coverage / 其他 workflow checks 由 `ci-local-generate.sh` 從 repo CI config 推導），改動的 source file 一律補測試，不以「改動小」為由豁免
3. **交付流程**：開發完成後讀取 `references/engineer-delivery-flow.md`，以 **Role: Developer** 執行 Two-Segment Architecture：LLM 實作段（Simplify → Self-Review）→ 機械自驗段（Step 1.5 Scope Gate → Step 2 前置 Rebase → Step 2 `ci-local.sh` → Step 3 `run-verify-command.sh` → Step 3.5 VR → Step 5 Base Freshness → Step 6 Commit → Step 7 PR（含 Evidence AND Gate）→ Step 8 JIRA）。Step 2 是**必跑步驟**（repo 有 `ci-local.sh` 時），產出的 evidence 會被 `ci-local-gate.sh` PreToolUse hook 在 `git commit` / `git push` / `gh pr create` 前檢查（cache miss 時 hook 同步重跑 `ci-local.sh`）。**AC 驗證不在本 skill**，PR 開完後由 verify-AC 接手
...
Revision mode 進入後、讀施工圖前，**先 rebase PR branch 到 task.md 規定的 base**，同步修正 PR 的 base 欄位。理由同 first-cut § 4.5：先發現 conflict 再動手；且 PR 的 `baseRefName` 本身可能就是錯的（這正是 DP-028 要洗掉的情境）。
...
> **Hook 保險**：`pr-base-gate.sh`（DP-028 D4）在 `gh pr edit --base` 仍會擋不符 resolve 結果的呼叫——跳過 script 自己手動跑等價指令也會在這裡被 block。
...
- AC 驗收標準（從 task.md 的 Operational Context 或 JIRA parent Epic — `--field ac_verification_ticket` 取驗收單 key）
...
讀取 `references/engineer-delivery-flow.md`，以 **Role: Developer** 執行機械自驗段：Step 1.5 Scope Gate → Step 2 前置 Rebase → Step 2 `ci-local.sh` → Step 3 `run-verify-command.sh` → Step 3.5 VR → Step 5 Base Freshness → Step 6 Commit → Step 7 Push（含 Evidence AND Gate）→ Step 8 JIRA。
這確保修正後的 code 仍通過所有機械品質門檻。

> **⚠️ Step 2（`ci-local.sh`）在 revision mode 尤其重要**：revision 常見原因就是 CI fail（codecov patch coverage 不足等）。修完後必須重跑 `ci-local.sh` 確認本地模擬 CI 通過，否則 push 後 CI 可能再次失敗。`ci-local-gate.sh` PreToolUse hook 同時擋 `git commit` / `git push` / `gh pr create` — revision mode 的 push 若 evidence cache miss，hook 會同步實跑 `ci-local.sh`（FAIL 物理擋下）。
