...
- **本地完成權限**：當 Phase 3 LLM gates + Phase 4 mechanical gates（`ci-local.sh` / `run-verify-command.sh` / post-implementation flow gap audit / VR if triggered / evidence AND gate / completion gate）全通過，engineering 可回報 complete。遠端 repo CI 的 queued / pending / running 狀態不阻擋 complete，也不要求等待；已完成且明確 fail 的遠端 check 才作為 revision signal 處理。
...
**Handbook gate（不可省略）**：開始實作或 revision 分類前，必須讀取 workspace-owned handbook SoT：`{company}/polaris-config/handbook/index.md` 並展開 index 內引用的子文件；接著讀取 `{company}/polaris-config/{project}/handbook/index.md` 並展開引用子文件。只讀 index、不讀 linked child docs = 未完成 handbook gate。若 company handbook 不存在，明確記錄 `company handbook absent`，但不可因此跳過 repo handbook。

**Repo config boundary**：repo root `CLAUDE.md` / `AGENTS.md` / `.github/copilot-instructions.md` 是 repo-owned adapter surface，engineering 不要求它存在、也不修改它來套用 Polaris。`AGENTS.md` is not required for Polaris correctness；native adapter deferred 到 repo owner 明確決策。Polaris 只消費 workspace-owned `polaris-config` 與 task.md，repo-local ignored overlay 僅能作為 `legacy-compat` 或 tool cwd 限制的 fallback。
...
- 先讀 company handbook：{company}/polaris-config/handbook/index.md + 引用子文件；再讀 repo handbook：{company}/polaris-config/{project}/handbook/index.md + 引用子文件
...
1. 讀 handbook gate：先讀 `{company}/polaris-config/handbook/index.md` + index 引用子文件；再讀 `{company}/polaris-config/{project}/handbook/index.md` + 引用子文件。若 workspace-owned handbook SoT 缺漏但 repo-local ignored overlay 尚存在，可用 `scripts/lib/main-checkout.sh` 的 `resolve_main_checkout` 找 repo 主 checkout，再讀 `{main_checkout}/.claude/rules/handbook/index.md` 作為 `legacy-compat` fallback；不可把 fallback 視為新生成位置
2. 讀專案 `CLAUDE.md`
...
8. repo 的 workspace-owned `polaris-config/{project}/generated-scripts/ci-local.sh` 存在就必跑；在 worktree 內一律用 `bash "${POLARIS_ROOT}/scripts/ci-local-run.sh" --repo "$(git rev-parse --show-toplevel)"`，不要直接查 worktree 的 repo-local `.claude/scripts/ci-local.sh`；若 repo-local legacy script 存在但 canonical script 不存在，這是 migration blocker，不是可省略理由
...
- Phase 4（Developer）：Scope Gate → Step 2 `ci-local.sh` → Step 3 `run-verify-command.sh` → Step 3.2 Flow Gap Audit → Step 3.5 VR → Base Freshness → Commit → PR（`polaris-pr-create.sh`、不可 draft）→ JIRA → Completion Gate（remote PR readiness/body）→ Worktree Cleanup
- Phase 4（Local Extension）：Scope Gate → Step 2 `ci-local.sh` → Step 3 `run-verify-command.sh` → Step 3.2 Flow Gap Audit → Step 3.5 VR → Base Freshness → PR（if required by local policy）→ Handoff Package → Local Extension → Extension Verification → Worktree Cleanup
...
同一步驟必須完成 handbook gate：讀 `{company}/polaris-config/handbook/index.md` + index 引用子文件，再讀 repo handbook index + 引用子文件。Revision 分類會用這些 handbook 規則判斷 CI / review signal，不能只依 PR check 訊息表面文字決定。
...
讀取 `references/engineer-delivery-flow.md`，以 **Role: Developer** 執行機械自驗段：Step 1.5 Scope Gate → Step 2 前置 Rebase → Step 2 `ci-local.sh` → Step 3 `run-verify-command.sh` → Step 3.2 Flow Gap Audit → Step 3.5 VR → Step 5 Base Freshness → Step 6 Commit → Step 7 Push（含 Evidence AND Gate）→ Step 8 JIRA → Step 8.5 Completion Gate → Step 8.6 Worktree Cleanup。
這確保修正後的 code 仍通過所有機械品質門檻。

> **⚠️ Step 2（`ci-local.sh`）在 revision mode 尤其重要**：revision 常見原因就是 CI fail（codecov patch coverage 不足等）。修完後必須重跑 `ci-local.sh` 確認本地模擬 CI 通過，否則 push 後 CI 可能再次失敗。repo 對應的 `polaris-config/{project}/generated-scripts/ci-local.sh`（DP-079 路徑；該檔由 generator 產出且屬 workspace-owned）存在時，revision mode 就沒有豁免。`gate-ci-local.sh` 在 git pre-commit / pre-push 擋 evidence cache miss；`check-delivery-completion.sh` 則在回報完成前再擋一次，避免口頭結案。
>
...
- 若 drift 涉及 repo-specific 知識（框架 API、專案架構）→ 直接寫入 repo handbook（`{company}/polaris-config/{project}/handbook/`）
