...
**鏈上出現、樹裡還沒有的母單，重算逐張說出來，不補。** 以前它會把那些目錄造出來——而一個
只當路徑用、沒有內容的母單層，翻樹的人在它身上讀不到任何東西，機器又會把它算成一張單。
要不要開那張單是人的決定。
...
**位置是狀態的投影，不是第二個權威。** 一張單在哪一格由它自己的狀態算出來——所以要問「這張
單走到哪了」，讀那份紀錄，不要讀它躺在哪個資料夾。**手動把一張單搬到別處，下一次重算會把它
搬回狀態說的那一格**，而它搬不動的那幾張會留在原地並被指名。
...
推導出來的結果會寫回**單現在所在的那個目錄**（`{單}/.spine/placement.json`）——不是它「該
...
它也會拒絕**在同一個地方開第二輪**：同一棵單的目錄樹裡已經有一張走到終局站別、**而且還
沒有釋出紀錄**的單，而它記下的身分與現在求出的有**任何一個交集**，就不開。交集就夠了——
共用其中任何一個地方，新的一輪就疊在別人的歷史上，最後只能一起出去。這件事 2026-08-03
真的發生過一次，三份交付紀錄釘在同一個 head，而中間沒有任何一步說過話。拒絕訊息會指名
是哪一張單、交集在哪，以及往下走的兩條路（把那張單走完，或換一個工作區）。

**釋出過的單不佔位子。** 它的歷史已經出去了，新的一輪疊不到它上面——所以這一問是「那張單
釋出了沒」，不只是「它停在哪一站」。少了這一問的那一版，拒絕訊息建議的第一條路走不通：
它說「把那張單的釋出尾段走完」，而那張單早就走完了，走完也不會讓這個判定改變答案。
2026-09-16 量到的形狀：一棵樹上十張有釋出紀錄的單，走到終局站別的只有一張，而那一張從
2026-08-31 起永遠佔著那棵樹的預設分支，後面每一張單各自開一條繞路的 branch 過去，留下
六條。
