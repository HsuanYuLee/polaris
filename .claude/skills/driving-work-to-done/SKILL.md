...
它同時回答另一個「在哪」：**這個工作區還是不是這張單當初落腳的那一個。** 開輪次那一刻
記下的身分由領域決定（核心只當它是一個字串），之後每次問路都比一次。回 `DRIFTED` 表示
現在動手的地方跟這張單預期的不同——可能是有人切走了，也可能是另一個 session 正在共用
同一份工作區。回 `unmeasurable` 表示這一次什麼都沒比到，那不是「還在原地」。
