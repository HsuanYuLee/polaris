...
| Runtime model adapter policy | workspace 主 checkout | **絕對路徑semantic model class，Codex 用 `.codex/agents/polaris-*.toml` custom agent，Claude Code 用 subagent model frontmatter / per-invocation model |
...
> - model tier policy（若需指定 sub-agent model class）class，避免直接寫 provider model ID
