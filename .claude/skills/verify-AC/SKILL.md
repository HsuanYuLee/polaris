...
bash .claude/skills/verify-ac/scripts/frozen-assertion-fence.sh verify {source}/index.md
...
```bash
bash .claude/skills/verify-ac/scripts/enumerate-spine-inventory.sh --source {source}
bash .claude/skills/verify-ac/scripts/check-spine-cost-floor.sh --inventory {source}/.spine/inventory.json
```
...
```bash
bash .claude/skills/framework-release/scripts/spine-release.sh --source {source}            # 看它打算做什麼
...
```bash
bash .claude/skills/verify-ac/scripts/spine-loop-state.sh advance \
  --state {source}/.spine/loop-state.json --to engineering
```
