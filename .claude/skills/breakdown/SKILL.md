...
- Step D: 建立驗收單（依 `references/epic-verification-structure.md`）
  ...
  - **檔名命名（producer cutover 規範 — DP-039 一起 atomic 切）**：
    ...
    - **過渡期命名（DP-039 重構 verify-AC consumer 落地前）**：保留 `{V-KEY}.md`（如 KB2CW-3713.md，以 JIRA key 命名）— 因為 verify-AC 仍在讀 `{V-KEY}.md` 路徑；breakdown 同步切到 V{n}.md 會造成 producer / consumer 不同步
...
> **DP-028 D4 / D5 / D6 / D7**：task branch 依 `depends_on` DAG topological 順序建立；非線性 DAG 先拒絕；`Base branch` 寫入 snapshot 當下的正確值，無 `PR base` 欄位。另寫入 `Branch chain`（例：`develop -> feat/GT-478-... -> task/KB2CW-3711-... -> task/KB2CW-3900-...`）作為 engineering cascade rebase 契約。engineering 開工時若 snapshot 過時（依賴已 merge），Resolve 層在 `engineer-delivery-flow.md § 4.5 / § R0 / § Step 7` 動態改值——snapshot **不是永遠不變**。
>
...
develop
  └── feat/GT-478-...
        ├── task/KB2CW-3711-... (T3a, Base: feat/GT-478-..., Chain: develop -> feat/GT-478-... -> task/KB2CW-3711-...)
        │     └── task/KB2CW-3900-... (T3b, Base: task/KB2CW-3711-..., Chain: develop -> feat/GT-478-... -> task/KB2CW-3711-... -> task/KB2CW-3900-...)
        └── task/KB2CW-3xxx-... (無 depends_on, Base: feat/GT-478-...)
```
...
2. **Operational Context 表格** — 必填欄位：
   ...
   - Depends on（frontmatter `depends_on` 的 human-readable 摘要，例 `KB2CW-3711 (T3a — 需 dayjs plugin 就緒)`；無依賴時省略）
...
| 需讀 `.output/` / `dist/` / build artifact | `build` | `ls .output/public/_nuxt/*.js`（如 GT-478 T3 `grep moment .output`） |
| 需 `curl` live endpoint / 瀏覽器互動 / runtime API | `runtime` | `curl -sk https://dev.kkday.com/zh-tw`（如 GT-478 T2 `curl dev.kkday.com`） |
...
**填寫細節**：
- `{repo_name}` 填實際 repo name（如 `kkday-b2c-web`），對應 workspace-config `projects[].name`
