...
|-------------|--------|------|
| `jira` | `breakdown PROJ-123` / `breakdown TASK-123` | 既有 Bug Path / Planning Path / Scope-Escalation Intake Path |
...
> **DP-028 D4 / D5 / D6 / D7**：task branch 依 `depends_on` DAG topological 順序建立；非線性 DAG 先拒絕；`Base branch` 寫入 snapshot 當下的正確值，無 `PR base` 欄位。另寫入 `Branch chain`（例：`develop -> feat/PROJ-123-... -> task/TASK-123-... -> task/TASK-123-...`）作為 engineering cascade rebase 契約。engineering 開工時若 snapshot 過時（依賴已 merge），Resolve 層在 `engineer-delivery-flow.md § 4.5 / § R0 / § Step 7` 動態改值——snapshot **不是永遠不變**。
>
...
develop
  └── feat/PROJ-123-...
        ├── task/TASK-123-... (T3a, Base: feat/PROJ-123-..., Chain: develop -> feat/PROJ-123-... -> task/TASK-123-...)
        │     └── task/TASK-123-... (T3b, Base: task/TASK-123-..., Chain: develop -> feat/PROJ-123-... -> task/TASK-123-... -> task/TASK-123-...)
