...
- VR baseline 永久存 `{source_container}/tests/vr/baseline/`
...
| `runner` | string | 是（如果有 fixtures block） | Toolchain capability command（預設 `fixtures.mockoon.start`）。Skill 在 runtime 組合 `.claude/skills/onboard/scripts/polaris-toolchain.sh run fixtures.mockoon.start -- {company_specs_dir}/{EPIC}/tests/mockoon` 啟動 |
...
| `shared_config_dir` | string | 否 | 跨 Epic 共用 config 目錄（proxy-config.yaml 等）
