...
`.claude/skills/onboard/scripts/onboard-doctor.sh` 只輸出 connector 需要 runtime check 的 fact；active agent
...
DP-202 後，root runtime toolchain 的入口是 public task surface：

- `mise run bootstrap`
- `mise run doctor -- --profile core|runtime|delivery|full`
- `mise run doctor-mise`
- `mise run onboard-doctor`
- `mise run release-preflight`
- `mise run pr-create`
- `mise run spec-close-parent`
- `mise run script-audit`
- `mise run docs-health`
- `mise run verify`
- `mise run cross-runtime-sync`

`onboard` 不代跑 root toolchain install。它只讀 root doctor 結果，dashboard 顯示
`ready` / `partial` / `blocked`，並把 repair command 以 print-only 方式交給使用者選擇。
若 root doctor 失敗，保留 partial state，並提供明確 repair command：
...
直到修復。Doctor profile、status、repair command 與使用者下一步選擇都要 audit。
...
| `root_toolchain` | yes | `ready`、`partial` 或 `blocked`，附 doctor profile 與 print-only repair command |
...
Root toolchain repair 是 print-only：`onboard repair` 不執行 global install、不代跑
`mise run bootstrap`，只 echo command、記錄 audit，並讓使用者決定下一步。只有使用者明確
提供 `--execute` opt-in，且 command 被分類為 non-interactive / CI-safe，才可執行 repair。
