...
這是一段繁體中文 artifact，包含 `.claude/skills/onboard/scripts/validate-language-policy.sh`、https://example.com/path 與 `JSON` key，應該通過。
MD
...
```bash
LANGUAGE_POLICY_SELFTEST=1 bash .claude/skills/onboard/scripts/validate-language-policy.sh
```

- `source_type`: `dp`
- `task/DP-050-T3-workspace-language-policy-gate-script`
- https://example.com/docs/path
- `.claude/skills/onboard/scripts/validate-language-policy.sh --blocking --mode artifact`
MD
