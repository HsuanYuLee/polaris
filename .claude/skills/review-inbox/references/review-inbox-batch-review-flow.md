...
- **parent 不一定在這一輪，而那是另一種指示。** `stacked_on.in_this_round` 分得開兩種：
  `true` 是「那一顆的結論待會兒出來，可能影響你」；`false` 是「你的 base 裡有一段沒有人在
  這一輪看的改動，而它可能還帶著沒解除的 CHANGES_REQUESTED」。後者派不出去（它不在候選
  集），所以那一顆的 reviewer 要自己去看 parent 現在的狀態。
- **parent 表取的是該 repo 全部 open PR，不是本輪候選集。** 候選集決定誰被派，不決定誰算
  parent：2026-09-17 一輪裡三顆真的疊著的 PR 被判成「沒有疊在任何人身上」，因為它們的
  parent 各被三道濾網之一濾掉了——兩顆是「我方投過票」、一顆是 draft。其中 #3232 是那條
  9 層 stack 唯一的修正收斂點（44 個檔），差點被半審。
  表由第三腿 `scan-unreviewed-prs.sh --open-prs-out` 在**濾網之前**寫出來，`annotate` 用
  `--open-prs` 讀它。
- **沒有那份表的時候說出來，不要說成沒有疊。** base 是該 repo 預設分支的判 `not_stacked`
  （預設分支不會是誰的 head，這一格不需要表）；base 不是預設分支而手上沒有表的判
  `unmeasurable`，走完整 review，也不得因為檔案交集而被判成附屬顆——交集對串行堆疊恆為真。
