...
| `scripts/extract-pr-urls.py` | 從 Slack MCP output 確定性提取 PR URL + thread mapping | Slack | stdin (Slack MCP JSON) + `--org <org>` | stdout: PR URLs (一行一個); side: `--mapping` JSON file |
