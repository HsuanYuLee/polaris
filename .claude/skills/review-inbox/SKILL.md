...
| `scripts/extract-pr-urls.py` | 從 Slack MCP output 確定性提取 PR URL + thread mapping | Slack / Thread | stdin (Slack MCP JSON) + `--org <org>` + 可選 `--thread-ts <ts>` | stdout: PR URLs (一行一個); side: `--mapping` JSON file |
...
**Strategist 計算好參數後，dispatch 一個 sonnet sub-agent**，prompt 包含：
...
```
1. 呼叫 slack_read_channel({ channel_id: "<CHANNEL_ID>", oldest: "<timestamp>", limit: 100 })
2. 將 MCP 回傳的 JSON 寫入 /tmp/slack-raw.json（Write tool）
3. 執行 pipeline：
...
4. 讀取 /tmp/review-candidates.json 和 /tmp/pr-thread-mapping.json 的內容
5. 回傳 Completion Envelope：
   - Status: DONE
...
1. 呼叫 `slack_read_thread({ channel_id: "<CHANNEL_ID>", message_ts: "<THREAD_TS>" })`
2. 將 MCP 回傳的 JSON 寫入 `/tmp/slack-thread-raw.json`（Write tool）
3. 執行 pipeline（`--thread-ts` 讓所有 URL 映射到同一個 thread）：
   ```bash
   cat /tmp/slack-thread-raw.json \
     ...
     | $SKILL_DIR/scripts/check-my-review-status.sh $MY_USER > /tmp/review-candidates.json
   ```
4. 讀取 `/tmp/review-candidates.json` 進入 Step 3
...
對選中的 PR，**每個 PR 啟動一個獨立 sonnet sub-agent 平行執行 review**。Sub-agent 不能呼叫 Skill tool，所以直接讀 `review-pr/SKILL.md` 的流程 inline 執行。
...
- 用 bundled scripts 做掃描和狀態判斷，不要手動組裝 API 查詢
  ...
  - Slack 模式：整包委派 sub-agent（MCP → `extract-pr-urls.py` → `fetch-prs-by-url.sh` → `check-my-review-status.sh`），主 session 只收 filtered JSON
