...
2. 每個檔案 pipe 到 sanitizer：
   ...
   ```bash
   echo "$content" | python3 .claude/skills/learning/scripts/skill-sanitizer.py scan "$skill_name"
   ```
...
```bash
bash .claude/skills/learning/scripts/validate-learning-seed-contract.sh \
  --producer learning \
