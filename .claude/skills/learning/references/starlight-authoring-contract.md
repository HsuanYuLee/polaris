...
<!-- PROSE-EXTERNAL-PATHS: tasks/ — 這一節講的是 docs-manager 那個 repo 的 specs 樹，不是這裡的 -->

1. **Docs page**：會被 Starlight docs collection 載入的 markdown，例如 DP `{DP}/index.md`、
   `{DP}/plan.md`、`{DP}/refinement.md`、`tasks/Tn/index.md`、`tasks/Vn/index.md`。必須符合本檔
...
```bash
bash .claude/skills/learning/scripts/validate-specs-collection-shape.sh --all
```
...
新 DP 使用 folder-native `{DP}/index.md`；legacy DP 可能仍使用 `{DP}/plan.md`。
...
- Deterministic drift 先用 `.claude/skills/learning/scripts/sync-spec-sidebar-metadata.sh --apply <path>` 修正，再跑 `.claude/skills/learning/scripts/validate-dp-metadata.sh <path>` 驗證 DP metadata。
...
```bash
.claude/skills/learning/scripts/validate-starlight-authoring.sh path/to/file.md
.claude/skills/learning/scripts/validate-starlight-authoring.sh path/to/spec-container
```
