...
新增 0 份、改動 4 份、刪除 0 份。


## [5.1.96]

新增 0 份、改動 5 份、刪除 0 份。
...
新增 2 份、改動 2 份、刪除 0 份。


## [5.1.93]

新增 0 份、改動 8 份、刪除 0 份。


## [5.1.92]

新增 0 份、改動 3 份、刪除 0 份。
...
新增 3 份、改動 12 份、刪除 0 份。


## [5.1.47]

新增 1 份、改動 13 份、刪除 0 份。


## [5.1.46]

新增 0 份、改動 10 份、刪除 0 份。
...
新增 1 份、改動 4 份、刪除 2 份。


## [5.1.16]

新增 0 份、改動 81 份、刪除 0 份。
...
- d3b0f0a: DP-570：個人的工具搬出這個 repo，`standup` 收窄成只收集與列出
  上一版交付的那支每日 routine skill 宣告 `scope: company-only`，於是它進了版控——而這個
  repo 的 origin 是一間公司的 GitHub。它是使用者個人的工具，只是它處理的是那間公司的事情。
  **工具是工具，事情是事情**，名字裡的公司代號講的是它處理什麼，不是它屬於誰。
  - **那支 skill 搬到 `~/.claude/skills/`**，這個 repo 的版控裡一份都不留（含它在
    `.claude/skills/<公司>/` 底下的本體與那條同名連結）。它從那間公司自己的
    `workspace-config.yaml` 拿偏好，而那個目錄本來就不進版控；讀不到就說出來並照預設走。
  - **`standup` 只收集與列出。** 拿掉目的地解析器與它的 selftest、拿掉「產出與送出是兩段」、
    拿掉把落差寫回單／PR 那一步。本地那份 markdown 是它唯一寫的東西，因為明天的
    plan vs actual 要讀它。`standup-format-publish-flow.md` 改名成 `standup-format-flow.md`。
  - **送出歸個人 skill。** 送出之前把逐字全文交給人、等一句同意；**授權工具不是授權內容**
    ——授權了瀏覽器，那份要貼出去的東西他還沒看過。送出去之後要改，改動的全文再交一次。
    以前 `standup` 自己去問一個宣告、自己按下送出，於是「要送到哪裡」變成一支通用 skill 的
    問題，而那個答案只有某一家公司才有。送出是一個人在一家公司的行為，收集與列出不是。

## [4.65.0] - 2026-08-24

### Changed

- 9b4496a: 站會從「產出物」降成每日清單上的一項
  `standup` 修掉報告本身的四個洞：
  - **昨日對今日比的是內容，不是 ticket 號碼。** epic 標題本來就帶著號碼，所以「這張 epic
    出現在前一份的今日格裡」永遠成立、什麼都證明不了；空的今日格也不算一個計畫。
  - **產出與送出分成兩段。** 本地檔寫完、過完 gate 之後停下來，把要送出去的**逐字全文**交給
    人，等一句同意才送——工具的授權不是內容的授權。`publish: manual` 就是由人自己貼上。
  - **epic 的 assignee 不是自己時，三格寫的是自己名下那幾張子單**，不寫成整張 epic 由自己
    推進。assignee 在收集階段就要帶出來，停用的帳號也要說出來。
    同一張單另外長出一支**公司專用的每日清單 skill**（`scope: company-only`，不進 template
    repo）：trigger 一次產出今天要做的事——今天的會議與各自的前期準備、撿出今天做得動的單、
    等我 review 的 PR、今天要問誰的事、今天分到的估點份額，站會資訊是其中一項。它帶兩支腳本，
    偏好與會議前期準備從公司自己的設定讀（缺宣告回 4，不靜靜沿用上一份的措辭），估點份額由
    「剩下的張數 ÷ 到下一次 planning 還有幾個工作天」算（攤不開時回 `spread=no`，不假裝攤得開）。
...
- 4b1cb20: 回覆讀不讀得懂，現在有東西在量。
  4.62.0 加了 97 行規則，十二條驗收全部只 grep 那 97 行在不在檔案裡，沒有一條看過一則真的
  回覆——所以它全綠出貨，而隔天三則回覆把八個徵兆照犯一遍。
  `.claude/hooks/reply-guard.py` 掛在 Stop 上，話講完那一刻量一次自己剛講的話。抓到框架
  內部詞、自造的比喻、破折號夾插入語、名詞化動詞、被動句時，離場碼 2 把逐處清單交回去要求
  重寫。引述不算犯：被引號、反引號、圍籬區塊框起來的詞是在指名它，不是在用它。同一支吃檔案
  路徑時是命令列工具，話送出之前先量草稿。
  `--judge` 是機械量不到的那一半，找另一個模型判四題：讀完知不知道下一步、有沒有要回頭問的
  東西、每個判斷有沒有帶得走的根據、沒讀過前文的人願不願意回。分兩關——第一關的讀者知道前面
  聊過什麼，第二關的讀者一句都沒讀過。它慢且要花錢，所以不掛在會擋人的那一處。
  常駐規則的〈中文寫成什麼樣子〉不再自己列一遍徵兆，改成指向那支東西；〈回覆的形狀〉與
  〈術語〉收掉讀過就忘的部分。`style-and-language.md` 從 149 行減到 116 行。
  機制與資料分開：詞表住在 `.claude/hooks/reply-guard-words.json`，那一份屬於裝它的那個工作
  區，不跟著框架走。詞表不在的時候句型那幾條照樣跑，而且會說出詞表沒載到。
  `sync-to-polaris.sh` 以前只搬 `.claude/hooks/*.sh`。一支用 python 寫的 hook 因此靜靜留在
  本地，而 `settings.json` 照樣出去並指向它——目的地會拿到一份指向不存在檔案的設定。現在
  `*.py` 一起搬，資料檔不搬。
...
- df44434: 報告有了段落骨架，回覆有了形狀。
  **產出類**：交出去給人讀、讀完要做決定的東西（研究、調查、檢查結果、市場分析、異常排除）
  共用一份五段骨架，第 3 段隨類型換。三支會產出這種報告的 skill 各自帶一份，因為 skill 要能
  整包被帶到別的環境，指向目錄外的東西在那裡讀不到。副本一致由 `gate-copy-sets` 守。
  **怎麼回應人**：常駐規則多了三節。〈回覆的形狀〉定下結論／根據／待決三段加七條硬規矩；
  〈中文寫成什麼樣子〉列出八個「語言對了但句子是英文骨架」的徵兆；〈術語：用中文還是用原文〉
  把判準定成「台灣工程師平常怎麼講」，不是「有沒有正確的譯法」，並附三類例子。
  原本〈風格〉裡的「先講結論」「附證據」折進〈回覆的形狀〉，不留兩份。

## [4.61.5] - 2026-08-21

### Changed

- 1f6d186: 重算的報告把「這次問不到」與「問到了但答不出來」分開講了。
  `slot_from_resolver()` 的 `if returncode == RESOLVER_COULD_NOT_ASK or not answer:` 讓第二條
  return 永遠到不了：解析器答不出來時用 `die()` 收場，只寫 stderr，所以任何一種失敗的 stdout
  都是空的。那一段自己的註解寫著「exit 2 與 exit 1 是兩件事，報告上也必須是兩件事……第一版
  把兩者都寫成同一句」——而它被自己下一行的短路條件取消掉了。
  真樹上的症狀：一張上游明確回「找不到這個單號」的單，報告寫成 `這次沒問到——上游說沒有
  改成照離場碼分三條路：問不到／問到了但答不出來／回 0 卻什麼都沒印（那是解析器壞了，不是
  沒問到——沒問到的下一次可能問得到，這一種每次都會這樣）。

## [4.61.4] - 2026-08-21

### Changed

- 02f3ab9: 單樹重算現在穿得過「只是路徑」的那一層。
  `_walk_ticket()` 以前只往帶著 `.spine/` 的子目錄遞迴——**「什麼是單」與「要不要穿過去」
  被同一個答案答了兩個問題**。搬動會順手造出只當路徑用的中間層（沒有 `.spine/`、沒有正文，
  只有一個子單目錄），遞迴在那裡停下來，底下的單就永遠不進清單。
  那是一個會自己維持住的洞：孫單看不見 → 它的歸屬鏈沒有人讀 → 中間那個號永遠不會被補成
  一張單 → 孫單繼續看不見。真樹上量到 19 張深度 3 的孫單卡在後面，位置從此不會被重算。
  改成穿過每一個目錄，只有帶 `.spine/` 的才算一張單。真樹：認得的張數 701 → 720，等於樹裡
  帶著推導結果的目錄數。
...
- 8faf4a7: `next --across-issues` 的幾個數字現在加起來等於整棵樹。
  以前它的 `find_states()` 只收帶 `.spine/loop-state.json` 的目錄，所以狀態不在本地的單
  ——命名空間宣告了解析器、權威在外部系統的那些——從來沒進過清單：不在任何一格，也不在
  任何一個數字裡。2026-08-21 量到 700 張單只涵蓋 555 張，差的 145 張裡 121 張還在中間態。
  改成讀位置重算已經寫回每張單身上的 `.spine/placement.json`，並把分母交給重算自己認得的
  那組單。多了三個具名的數字（狀態在別處的、狀態在別處而且已經有結論的、兩層都問不到的），
  以及一行寫出等式的 `tree:`。逐張的清單不複製第二遍，指向重算產出的 `OPEN.md`。

## [4.61.1] - 2026-08-21

### Changed

- a88b935: 單樹重算的搬動改照**目的地**的深度排，母單先落地——以前照來源深度排，母子同深時子單先搬，
  順手把母單的目的地建成一個空路徑，母單自己那一步就撞上「目的地已存在」被安靜跳過，留下
  兩個同號目錄。
  報告也補上了：`打算搬 N 張＝真的搬了＋跟著母單一起走＋撞到已存在的沒搬成＋來源不見了`，
  四個數字自洽。沒搬成的逐張說出撞到什麼，並用 git log 各查一次兩邊路徑上次動過的日期
  （查不到就寫「不知道」，不寫今天），哪一份留下來由人決定。

## [4.61.0] - 2026-08-21

### Changed

- 014fe95: 單樹的路徑主軸從「每一張單各自的狀態」換成「歸屬」：`{命名空間}/{格}/[{日期}/]{鏈頂}/…/{它自己}`，
  中間每一層都是一個單號。格子投影的是鏈頂那張單的狀態，子單不因為自己的狀態搬家；鏈上出現
  的每一個單號都會被補出一格，`index.md` 帶著標題、上游狀態、負責人、上次更新與上游描述快照。
  `_{母單}-{標題}` 那種群組層只認得、不再產生。單底下帶著 `.spine/` 的目錄才算另一張單——舊層
  留在單裡的 `tasks/`、`T1/`、`evidence/` 那 259 個目錄不是單，不會被搬走。

## [4.60.0] - 2026-08-21

### Changed

- 6057a88: 新增 `check-your-own-work`：交出自己的改動之前，先對一次自己寫的東西。
  六問不是想出來的，是從 748 個 PR、829 則真人 review 逆推出來的六類反覆缺陷——宣稱與 diff
  對不上、repo 既有規範沒套用、只改一半、源碼推論未經實測、上一輪講過仍未修、恆真斷言。
  其中四類不需要任何領域知識就避得掉。
  `scope: standalone`：唯讀、不判紅、不擋人、不對外寫入，也不抄任何 repo 的規範進自己的目錄
  ——規範每次執行時從當次那個 repo 現場讀。環境殘缺（沒有 gh、沒有 PR、沒有上一輪意見、
  一份規範檔都沒有、diff 是空的）時它照樣走完六問，並逐問指名說出哪一問答不出來、為什麼。
...
- f33b46a: 一張單有還開著的 PR 就是送審中，外部系統的狀態欄不算——狀態欄是人手動推的，PR 不是。
  這一問完全發生在那個命名空間宣告出來的解析器裡，核心一行都沒動。

## [4.58.0] - 2026-08-19

### Changed

- 55dc662: 單樹的每一格底下多一層母單：翻樹的人不開檔案就讀得出一張單屬於誰。母單由那個命名空間的
  解析器回答，核心不認得任何一個外部系統的欄位名。

## [4.57.0] - 2026-08-19

### Changed

- 948e1ff: standup 的卡關格三件事：「等 review」預設不再算卡關、兩次 standup 之間發現的卡關有了契約內的落點、三格的長度規則在決定內容的那一站就載得到。
  - **准入表第三列從「是」改成「否」**。請人看 PR 正是自己動得了的事，跟同一節開頭那句
    「一件我自己動手就能推進的事不是卡關」直接矛盾。分辨法一併換掉：從「那件事回來之後，
    我手上會多出什麼工作？」換成「我現在還有沒有下一步動作可做」——舊的那句會把 review 判成
    卡關，所以它留在原地當標本。新增〈「等 review」預設不是卡關〉一節，列出三種形狀
    （純粹等看、時間已經說定、工作還沒開始）與它真正升成卡關的兩個條件。
  - **新增〈兩次 standup 之間發現的卡關，落在哪〉**：發現的當下寫回本地那份 standup 檔，
    第二個來源明天原樣讀到。這不是第四個來源——「從哪裡找」還是那三個，只是「事後往那個
    檔案裡寫」現在是契約裡的一步。
  - **三格的長度規則不新增**。那五條 2026-08-12 就在 `standup-template.md` 的〈怎麼寫：產出物
    精簡，證據不精簡〉裡；缺的是 `SKILL.md` 只在排版那一列才載它，而格子的內容在更早幾步就
    決定了。改成 planning 那一列也載，並在兩份 reference 各留一句路標。
...
- d949f46: DP-543：延續過來的卡關要帶今天的根據
  `standup-planning-flow.md`〈卡關 sources〉第二條原本只寫「前幾天 standup 卡關格中持續存在
  的 blocker」，而「持續存在」沒有判準——延續本身變成了證據。加一節把今天問到的四種答案分開：
  有新痕跡的說出痕跡、沒動靜的說出從哪天開始、問不到的標成待驗、使用者說收掉的拿掉。

## [4.55.1] - 2026-08-14

### Changed

- 624ed36: DP-542：站會報告不再報產生它的那套工具自己的進度
  2026-08-14 那份報告的昨日格出現一整排 DP 號碼與 v4.43.0→v4.49.0，今日格出現「DP-540 施工中」
  與「三張單位置與狀態對不上」。**那不是 LLM 自由發揮，是契約明文要求的**，所以人工刪掉之後
  下一份會再長回來。三處一起拿掉：
  - `SKILL.md` Flow 第 5 步裡「以及那棵單樹底下還沒收斂的單 ⋯ 跑位置檢查 ⋯ 轉述那個數字」整段
  - `standup-planning-flow.md` 的 §Polaris Backlog（列 top 3 進今日格、提醒有未提交的框架改動）
  - `SKILL.md` metadata 那條只為了上面兩段而存在的 `requires`
    **還有第二個獨立的理由，而它更硬**：`standup` 是 `scope: standalone`，會被單獨上傳到
    claude.ai 與 Cowork，而那三處指名的目錄與腳本在那裡一個都不存在。同一趟把
    `session-timeline.md` 裡指進另一支 skill 目錄的那條路徑也改成只講名字。
    Contract 裡留下一句說出為什麼不報，讓它不會再被加回來：**讀者是誰決定了報告寫什麼。**
...
- 5055ff4: 「這支 skill 走哪個通道」以前有兩種形狀與五份讀法，現在各剩一份。
  15 支會出去的 skill 把 `scope:` 寫在 frontmatter 頂層；9 支 `company-only` 的寫在 `metadata:`
  底下。**分界正好落在「准不准出去」上**——拿任何一個正經的 frontmatter 解析器去讀，讀得到的
  是 15 支會出去的，讀成空的是 9 支不准出去的，而沒宣告的預設是會出去。
  它一直沒出事，是因為五個消費者（同步腳本兩處、外洩掃描、readme-lint、gate-source-destination）
  沒有一個讀 YAML，全部拿正則掃原始文字，而正則對 YAML 巢狀是瞎的。**這是一個靠「讀得不夠仔細」
  才成立的保護**：任何一次「把它改成正確地讀 frontmatter」的清理——那是所有清理裡最像對的
  那一種——都會讓 9 支公司 skill 靜默地開始同步到一個公開的 template repo。
  反方向也漏：那五條正則裡有兩條是 `grep 'scope:.*maintainer-only'`，連行首錨點都沒有，掃的
  還是整份檔案。一支 skill 只要在正文任何一行提到那個字串，就會被靜默排除在 template 之外。
  修法是兩半一起：9 支的宣告搬到頂層，五處讀法收成 `lib/skill_scope.py` 一支——它只認
  frontmatter 頂層的宣告，巢在別的鍵底下不算、正文裡出現同一個字串也不算。把任何一支的宣告
  搬回 `metadata:` 底下，同步預演現在會直接把它列進要複製出去的清單。
  順手補掉同一個形狀的另一個洞：模板的清除那一步以前只問 `maintainer-only`，跟複製那一步的
  判準不一樣，所以一支後來改成不出去的 skill 會被複製跳過、被清除留著，然後永遠停在模板裡。
  兩步現在問同一個地方。
...
- 6fbeb1f: 面向人的說明從 3,283 行變成一份 173 行的 `README.md`。
  以前那一套（README 的英文版與中文版、`docs/` 底下八份、`_template/` 的路由決策樹）描述的
  是一套已經不存在的目錄：README 第一句的 logo 指向另一個 repo，〈What You Can Do〉六條裡有
  兩條沒有對應的 skill，〈Governance Philosophy〉整段講三個早就被拿掉的東西，而把工作從一句
  話帶到交付的那五支 skill 一次都沒被寫進去過。1,940 行的 workflow-guide 是最大的一塊，也是
  最鏽的——它指向八支不存在的 skill。
  **它刪掉而不是重寫**：一份重述每一支 skill 在做什麼的手冊必然是第二份權威，而第二份會漂。
  行為的權威留在各自的 `SKILL.md`，路由留在 frontmatter 的 `description`。新的 README 只回答
  三個問題——這個 repo 是什麼、一件工作怎麼進來、一支 skill 怎麼被帶走——並且只給名字與一句話。
  所有文件併成單一語言的一份，不再有翻譯副本要同步。
  那支 docs linter 跟著縮小：以前查觸發詞表、mermaid 節點、導入文件的用詞，那些文件整批走了，
  檢查跟著走。留下的是它唯一守得住而且看 diff 看不出來的東西——README 講的 skill 名單與真的
  存在的 SKILL.md 兩個方向都要對得上，而 README 會跟著 template repo 出去。它每次都會說出
  自己掃了幾支、對照哪一份，不再有「目標消失所以是綠的」這種綠。
  同步到 template repo 那一段補上兩個它一直沒做的清除：頂層退休的說明文件，以及 `_template/`
  底下退休的範例。以前複製那一步只往前寫，所以一個檔案在這裡被刪掉之後，模板那一份會永遠
  留著——而模板正是別人 clone 下來讀到的東西。頂層檔案的清單現在只有一份，複製與清除共用。

## [4.50.0] - 2026-08-14

### Changed

- 80c3d00: 兩支每天在用的 skill 身上只留它自己的東西。
  **standup** 以前帶著這個框架自己的迭代程序（壓版之後那條鏈、待辦掃描、驗證過的樣式怎麼
  升格）與一支比對文件與 skill 目錄的 linter。那兩樣跟「產出每日站會報告」沒有關係，只是
  歷史；它們搬到 `framework-release`，那裡本來就是壓完版之後的事。另外刪掉一個孤兒
  lib——它在這支 skill 裡唯一的呼叫者上一版被拿掉了——以及一行只為了蓋住那個孤兒而存在的
  「這條路徑住在 repo 外」宣告。〈Flow〉十步、〈Data Rules〉、〈Write Rules〉逐位元沒動。
  那支 linter 搬家時順手修好一件事：它算根目錄的方式是「往上兩層」，而那算出來的是它自己
  所在的 skill 目錄，於是照文件打的那一行只會掃到 0 支 skill 並印「OK, all documented」——
  一個什麼都沒量到的綠燈，跟真的乾淨長得一模一樣。現在它往上找到帶著 skill 目錄的那一層。
  **review-pr** 的散文現在指名它那份 routing 評估集（13 句真的會被打出來的話，7 句該觸發、
  6 句不該），說出它是什麼、什麼時候要補一條進去——一份沒有人知道存在的評估集等於沒有。
  取件腳本補上 `--help`：以前少給參數會吐出一行讀起來像 bug 的 shell 訊息，現在印的是用法。
  `APPROVE` / `COMMENT` / `REQUEST_CHANGES` 的判準、對外寫入閘、語言閘、四支 skill 之間的
  觸發詞邊界一行都沒動。
  順帶修掉判定端一個會讓整趟交付沒有結論的洞：重跑量測命令那一步用 `text=True` 收輸出，
  所以量測命令一旦吐出不是 UTF-8 的位元組（掃到二進位檔，或 shell 把一個多位元組字元
  切斷），它丟的是 traceback——那既不是通過、也不是「量不到」，是一個沒有判定的離場。
  改成收 bytes 再自己解碼。
...
- 6786276: DP-535：寫摩擦的時候指名是哪一支，以及「寫死之前」
  兩件事，同一個方向：**判斷什麼該寫死、什麼該留給推測，而判準來自真的跑過的那幾次。**
  ## 一、〈寫死之前〉
  以前的說法是「推測的東西越少，做得越對」。那一句只對一半，而錯的那一半很貴——把本來推得
  對的東西也寫成固定流程，付的是 context 與維護，收的是零。外部量到的方向正好相反：**更強
  的模型需要更少的規定性工程**，規定性是隨模型能力遞減的東西。
  改寫成：**該由人決定的事不要留給推測；推得對的事不要寫死。** 分界線不另外發明，就是
  `refinement` 那四格在問的東西。**推錯的成本是重做一次，寫死的成本是每一次都付。**
  〈檢查類腳本的門檻〉是這一句用在檢查上的特例，所以擺在它後面。
  ## 二、〈寫摩擦的時候指名是哪一支〉
  「這一套東西有沒有讓工作更常做對」只能從真的跑過的那幾次讀出來，而**寫的時候不指名，事後
  就讀不出來**。量到的：1,721 份活文件裡有 345 份寫了摩擦，其中 323 份文件裡也出現某支 skill
  的名字——但那是**同現，不是歸因**。被指名最多的三支正好是每張單都會經過的三站。素材有量
  （345 筆），形狀答不出那個問題。
  所以摩擦與幫助各記一行，就寫在既有的活文件裡：
  ```
  <!-- SKILL-UTILITY: {skill 名} | helped|hurt | {一句話} -->
  ```
  三個刻意的選擇：**兩個方向都記**（只記摩擦的話，讀出來永遠是「這套東西都在擋人」）；
  **讀它用 `grep -rn`，不寫腳本**（它不擋任何人，而一個會擋人的 utility 判準會在第三次被
  繞過）；**不新增儲存體**。
  已知的覆蓋缺口寫在同一段：**它記得到的只有走過單的工作**，沒有立案就用掉的那些不留活文件。
  「沒有紀錄」不等於「沒有問題」。
  ## 三、回填了十筆真實紀錄
  八筆回填進事情真的發生的那五張已釋出的單，兩筆來自這一趟自己。現在 `grep` 讀得出來：
  | | helped | hurt |
  |---|---|---|
  | 釋出尾段那一層 | 2 | 2 |
  | 判定那一層 | 1 | 6 |
  | 送審那一層 | 0 | 1 |
  **helped 全部同一形狀**——擋下會出去、而且看 diff 看不出來的東西。**hurt 也全部同一形狀**
  ——判定機具自己的成本落在施工的人身上。
  ## 四、順帶量掉的 routing 現況
  24 支 skill 全部有 routing description，本體都在 500 行以內；三支的描述說不出「什麼時候用」，
  逐支指名在單裡。外部掃 55,315 支公開 skill 的對照數字是 26.4% 完全沒有 routing description。
  **這一趟不修那三支**——量過一次並指名就是這一輪要的，修不修是下一個決定。

## [4.48.0] - 2026-08-13

### Changed

- 11b241c: DP-531：檢查類腳本的預設是不要有
  使用者 2026-08-13 的原話：「我們不能一直發明 gap，去阻礙流程正常的流轉」「務必記取過去
  大量無用 selftest、阻礙大量開發的教訓」「這是我對你不做減法、只不斷疊 legacy 的明確質疑」。
  量到的支撐：`.claude/skills` 底下 43,296 行 shell 裡有 21,538 行（49.7%、109 個檔）是檢查
  類；整套 selftest 跑一次 121.8 秒。同一天釋出的九張單，只有兩張改到了使用者會經歷的東西，
  其餘七張都是機具在修自己。
  ## 門檻
  寫進 `.claude/instructions/core/bootstrap.md`（常駐指令的來源）：
  **預設是不要有。** 每多一道閘、每多一支 selftest，都要先證明它守著一個**不可逆、或會出去
  到這個 repo 之外**的後果，而且那個後果**看 diff 的人看不出來**。三個條件同時成立才留，缺
  一就不要寫——不是寫小一點，是不要寫。
  會被誤認成理由、但都不算的：「這樣才知道有沒有壞掉」（壞掉會在下一次用的時候壞給人看，
  那是最便宜的回饋）、「這條斷言要有機械的量測才算數」（框架自己就有〈靜態檢視〉那一格，
  判斷報告不擋人）、「順手把它守起來」（順手加的東西不會順手被刪掉）。
  **散文斷言散文不是檢查，是重複。** **檢查自己的檢查一律不做**——一道閘的第二層保險由
  「它紅的時候有人在看」提供，不由另一支腳本提供。
  ## 刪掉了什麼
  - **十三支檢查自己的檢查**：十一支 `gate-*-selftest.sh`、外洩掃描器的空掃 selftest、
    以及 selftest 跑者自己的 selftest。（跑者那支測「它會不會弄壞叫它的 repo」的留著——
    那一條守的是工作區被清掉，屬於不可逆。）
  - **四道閘**：`gate-layer-vocabulary`（哪些詞不得出現在哪支 skill 的散文裡——散文斷言
    散文）、`gate-ignore-classes`、`gate-dangling-declarations`、`gate-docs-collection`。
    後三道判準寫得對，但它們守的後果留在這棵樹裡，而且會在下一次有人用到的時候大聲壞掉。
  - **三支散文斷言散文的 selftest**：兩份參考文件的內文斷言、一支後台指示的 45 條憑據
    （連同那支 380 行的憑據檢查腳本本身）。**被斷言的那些內容原樣留著**——刪的是斷言它們
    的腳本，不是它們本身。
  - **七個沒有任何呼叫者的副本**：其中一支對外寫入前的語言閘全樹兩份、零呼叫者，而副本
    一致檢查一直在幫它們維持同步。
  ## 留下來的九道閘，各自說得出自己為什麼還在
  每一支的檔頭現在都有一行「為什麼這一道還在」，指名它守的那個不可逆或會出去的後果。
  說不出來的那一道不該還在——下一次要加閘的人先寫那一行，寫不出來就不要寫那支腳本。
  ## 量到的結果
  |                         | 之前            | 之後             |
  | ----------------------- | --------------- | ---------------- |
  | 檢查類檔數              | 109             | 85               |
  | 檢查類行數              | 21,538          | 16,500（−23.4%） |
  | skill 底下 shell 總行數 | 43,296          | 37,652           |
  | 整套 selftest           | 74 支、121.8 秒 | 58 支、104.0 秒  |
  同一趟另外關掉四張只在機具內部循環的單（審查意見的守衛、量測探針的共用 base 解析器、
  重跑層的去重、以及「掃出全部同類斷言」），理由各自寫在單上。

## [4.47.0] - 2026-08-13

### Changed

- d910c63: DP-529：一條命令、N 條斷言，跑一次
  同一條量測命令常常同時是好幾條斷言的量測，而每條斷言的正向證據不一樣。以前一份
  `--evidence-out` 只能對一條斷言，所以要產 N 份證據就得 invoke N 次——**而每一次都真的把那
  條命令再跑一遍**。
  量出來的代價：一張產品單的十六條斷言共用一條 495 秒的命令，跑十六趟共兩小時十分。把十六份證據
  的 stdout 逐行比對，34 行裡 31 行逐位元組相同，只有三行計時數字不一樣。十五趟買到三個數字。
  這個原則框架自己已經接受了，只是只做在後半段：`verify-ac` 的散文寫著「同一條命令通常被好
  幾條斷言共用，重跑照命令去重——八條斷言三條命令就跑三次，不是八次」。交付那一層去重了，
  產生證據那一層沒有。
  ## 改法
  ...
  --assertion A-P1 --expect-evidence '<A-P1 自己的痕跡>' --evidence-out …/A-P1.json \
  --assertion A-P2 --expect-evidence '<A-P2 自己的痕跡>' --evidence-out …/A-P2.json
  ```
  命令跑一次，每一組在同一份輸出上**各自判、各自寫**。這一點是整條路的重點：把同一個判定
  複製 N 份也會一樣快、看起來一模一樣，差別只在「這條斷言真的被檢查過」是不是真的。所以每
  份證據只記自己那一組的樣式與判定，而 selftest 對這件事判——不只對「有沒有變快」判。
  **不完整的分組會停**，各有自己的 marker：一組沒有輸出路徑
  （`POLARIS_ORACLE_GROUP_INCOMPLETE`）、兩組指到同一個檔案
  （`POLARIS_ORACLE_GROUP_DUPLICATE_OUT`）、分組模式下還有一個沒有歸屬的 `--evidence-out`。
  三種都會產出一份看起來正常、但屬於別條斷言的證據。
  **不給 `--assertion` 時一個字都沒變**：綠的、缺證據的、命令自己紅的三種收場，輸出、證據內容
  與離場碼都跟以前一樣（selftest 三條各驗一種）。
  ## 驗證
  11 個 case 的新 selftest，其中「命令只跑一次」是用計數檔量的，不是讀輸出推的。全樹 74 支
  selftest 全綠——既有那幾支跑的正是舊呼叫法。
- 0cd8d90: DP-528：清不掉的那一層快取——一句必要條件被寫成了充分條件
  BE2 挖字的知識裡有一句話抄在三個檔案裡：
  > 字表快取不會自己過期（`locale.lang-map` 的 ttl 是 `-1`）。新建的字要使用者登出重登、或
  > 清掉瀏覽器 localStorage 的那一筆才看得到。
  > **它漏了一層。** 2026-08-12 在 BE2-SIT 建完 6 顆挖字之後照這句做——清 `locale.lang-map` 重整
  > 三次、完整登出再登入——畫面仍然是六個 `Lang Key Not Set`。清掉之後重新抓回來的字表本身就
  > 沒有那六顆（重抓後仍是 1,040,392 bytes）。擋著的是 `/be2/api/v1/lang` 自己的伺服器端快取，
  > 它清不掉，也不吃登出重登。隔天沒做任何動作，畫面自己好了。
  > 代價是真的：當下據此往「key 是不是建錯位址」查了一輪，而 key 從一開始就建對了。**一句把
  > 必要條件寫成充分條件的話，會讓照著做的人去修一個沒有壞的東西。**
  ## 改了什麼
  完整的量測日誌收成一份權威，放在 `i18n.md` 的〈多久看得到〉底下——那一節本來就是「我建好
  的字為什麼還看不到」的家，現在三個答案（跨環境 6 小時、SIT 手動催同步、同環境的伺服器端
  快取）在同一個地方。另外三處只指過去：
  - `console-setup.md` 的坑 3——新建的字還沒出現時**先等**，不要回頭懷疑字沒建成功。
  - `be2-product-repo.md`〈兩道閘〉——`Lang Key Not Set:` 多出第三個可能，它不一定是字沒建成功。
  - 本機開發那份 handbook——那個情境保留，但拿掉充分條件的說法。
    **週期不補。** 量到的只有兩個界：小於 18 小時、不受登出重登影響。補一個看起來完整的週期
    就是把觀察寫成機制，而 `i18n.md` 對 BE2 那條同步刻意避開的正是這件事。順帶把兩者分開講明：
    那條 6 小時是 Lokalise → lang api，不是這一層。
  ## 驗證
  `i18n-knowledge-selftest.sh` 13 → 19 條。對改動前的 HEAD 餵進去，新的六條紅五條（B-N2 是副
  作用殼，舊樹裡沒有那一節所以它在那邊是空的綠），而原本 13 條在舊樹上仍然全綠——沒有一條
  為了讓新內容通過而被改寬。
...
- 0edc207: DP-526：發現規則只認檔名，於是七份紅著的 selftest 從來沒有被執行過
  `run-selftests.sh` 找 selftest 的方式是 `find -name '*-selftest.sh'`。自我檢查還有第二種
  形狀——藏在一支正常腳本的 `--selftest` 旗標後面——而那一種它看不見。
  量出來的：`validate-language-policy.sh --selftest` 是 11/12、離場碼 1，而它在樹裡有七份逐
  位元相同的副本，七份全紅。**沒有任何東西呼叫過它們。** 一支從來沒有被執行過的檢查，跟沒有
  那支檢查的差別只有它看起來很完整。
  ## 兩層
  **一、發現規則多認一種形狀。** 判準是那支腳本真的有 `--selftest)` 那個 case 分支，不是它
  提到過那個字——註解、usage、字串裡到處都是那個字，把它們拿去跑，跑到的是那支腳本的正常
  模式。輸出把兩種來源分開報數（`獨立檔 66 支、藏在 --selftest 旗標後面的 7 支`）：一個混在
  一起的總數，在旗標型那一類整批消失時仍然看起來正常。紅的時候指名帶著旗標，照著印出來的那
  一行重跑得到的是同一件事。
  **二、那條紅著的 case 改成斷言現在真正的行為。** 沒有 `language` 宣告時它退到英文並放行，
  而那是**刻意的**——程式碼自己的註解寫著理由：一支 skill 被單獨帶到沒有這個 workspace 的地方
  時，規則說它照樣工作，而腳本以前是 exit 1。漂掉的是沒有跟著改的那條斷言。改法不是只把 1 改
  成 0：放行要，**說出自己退到了英文**也要，所以那一條現在同時驗離場碼與那句話（新的
  `assert_rc_says`）。七份副本 13/13 全綠、逐位元仍然一致。
  ## 驗證
  - 全樹 73 支 selftest 全綠（改動前是 66 支綠 ＋ 7 支看不見的紅）。
  - `run-selftests-selftest.sh` 13 → 18 個 case。把發現規則退化回只認檔名，五條轉紅。
  - `gate-copy-sets` 綠：38 組副本逐位元一致。

## [4.45.0] - 2026-08-13

### Changed

- 6f3b3bd: DP-525：那道閘去問已經有答案的那一份，不再自己抄一份清單
  `gate-source-destination` 判「宣告 `destination: workspace` 的單有沒有檔案落在會出去的位置」，
  而它只認一個刻意很小的安全子集：gitignore、公司自己的目錄、宣告 company-only／maintainer-only
  的 skill。`.changeset/` 不在裡面——但它**確定不會出去**：`sync-to-polaris.sh` 從來不複製它，
  而且會把 template repo 端殘留的整個移除。
  代價已經發生過兩次。DP-523 與 DP-517 釋出時被這道閘判紅，唯一「認不出來」的檔案是那張單
  自己的 changeset，兩次的處理都是回頭把宣告改成 `template`。DP-517 那次是錯的：它四個檔案
  沒有任何一個會離開這個 repo。沒有東西外洩（`template` 是比較嚴的那一邊），但那張單的紀錄
  說了一件不是真的事。
  修法不是在閘裡加一條路徑。什麼會被複製出去只有一個東西說了算，就是 `sync-to-polaris.sh`
  自己——所以它現在用一行機器讀得到的宣告說出來，而閘去讀那些宣告：
  ```
  # <!-- POLARIS-NOT-SYNCED: .changeset/ — 本 repo 自己的發版設定，不屬於框架模板；… -->
  ```
  閘裡沒有第二份 `.changeset` 路徑清單（selftest 對原始碼判這件事，不只對行為）。讀不到那份
  宣告時判定回到嚴格的那一邊，並且把「這一次沒讀到」印出來——讀不到不是放行，但它也不該是
  安靜的；「讀到 0 條」與「讀不到那個檔案」在輸出上分得開。
  12 個 case 的 selftest 守住這條線：宣告消失、那段移除行為消失、閘改成寫死路徑，三種都會紅
  （實際餵過一份退化的閘，三條轉紅）。DP-517 的宣告改回 `workspace`，並在那張單上記下為什麼
  改過兩次；拿它原本那四個檔案重跑同一道閘，現在是 4 個留得住、0 個認不出來。

## [4.44.0] - 2026-08-13

### Changed

- f38a22b: DP-517：三個環境同一條 6 小時，而慢的那扇門也會開
  i18n 的知識裡有兩段講時程，兩段都是錯的：
  - 〈多久看得到〉說 prod 與 stage 走一條即時的推送、SIT 是三個裡最慢的。**沒有那條推送。**
    三個環境是同一條——STAGE 與 PROD 的 lang 跟著 SIT 一起更新，約 6 小時刷新一次
    （來源：2026-08-13 使用者說明，標明了是人說的，不是量出來的）。
  - 〈這份文件沒有回答的一題〉根據 2026-08-10 單一次量測，叫規劃者把 prod 當「要另外問人」。
    2026-08-12 再量一次 production 到齊了，中間沒有人執行過倒挖字。兩扇門最後都開了。
    **說法與量測對不上的那一格留著，沒有刪掉任何一邊。** 6 小時解釋不了 production 落後三天
    （8/7 上傳、8/10 查還是空的、8/12 才到齊）。一份抹掉矛盾的文件，讀起來跟一份真的沒有矛盾
    的文件一樣——所以兩件事都寫進去，各自標明來源，規劃時按慢的那一邊算。剩下的未知（那條同步
    的觸發者、prod 要不要跟版本走）仍然明講沒有人說得出來。
    `i18n-knowledge-selftest.sh` 從 8 條變成 13 條。新的五條餵舊版（改動前的 HEAD）全部判紅、
    原本那八條在舊版上仍然全綠——沒有一條為了讓新內容通過而被改寬。
    被推翻的兩句在新文字裡是轉述而不是逐字引用：一句被推翻的話留成原樣，下一個人複製它的時候
    不會看到旁邊那行說它是錯的。
