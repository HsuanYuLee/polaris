...
- c8437af: DP-522：文件站台收進去的每一份，還帶著它的 schema 要的東西嗎
  DP-518 驗那三條 viewer alias 的時候量到的：接線全通、astro 真的起來，然後死在一份缺
  `title` 的 markdown 上——而所有的閘都是綠的。既有那幾道問的都是「宣告指得到檔案」，沒有
  一道問「這個入口跑起來會怎樣」。
  `gate-docs-collection.sh` 補上那一道，接進 `run-gates.sh`（現在十道，合計 3.1 秒）。它不跑
  astro、不要求 `node_modules`、純讀檔——一個要先裝依賴才跑得動的檢查，在沒裝的機器上永遠
  量不到，而那跟沒有那道檢查一樣。它檢查的是擋住建置的那個契約本身：內容集合收進去的每一份
  markdown 都要帶著 `title`。
  **收哪些檔案不由這支腳本決定**：pattern 與排除規則從內容集合自己的設定
  （`docs-manager/src/content.config.ts`）讀出來，這裡不抄第二份清單。設定多一條排除規則，
  判定就跟著變——selftest 有一個 case 專門盯這件事。
  它比其他九道多一個離場碼：`3` ＝ 這棵樹沒有這個入口。`docs-manager/` 不走 template 通道，
  所以在 template repo 上它沒有東西可量——那不是紅也不是綠，混進 `|| fail=1` 的話那邊每次
  commit 都會紅，而一道每次都紅的閘會在第三次被關掉。`0`／`1`／`2`／`3` 四種輸入在 selftest
  裡各有一個 case，另有一個 case 專門確認三種不綠彼此分辨得出來。

## [4.40.0] - 2026-08-13

### Changed

- 38209f2: DP-521：描述著一個已經不存在的站台的 VR 設定
  VR 設定裡兩種「宣告與現實不符」在同一天同時發生，而沒有任何東西會紅。
  **一、宣告的瀏覽器不是實際會跑的那顆。** 宣告只有 chromium，而某個 project 用的 device
  跑的是 webkit。機器上沒裝，於是那一趟每一張都 `Executable does not exist`——那句話講的是
  執行檔不在，所以人會先去找環境的問題，而真正的問題是宣告跟 project 對不起來。
  **二、設定裡列的頁面死了。** 2026-08-13 量：四個裡三個跟到最終位置之後是 404，只有首頁
  活著。而其中**兩個第一個回應碼是 301／302**——只看第一跳會以為它們活著。設定檔自己那行
  「search-results 已移除 — production 404」是同一件事上一次發生的紀錄：有人手動清過一次，
  然後就沒有再清。**一次手動清理不會變成下一次的檢查。**
  ### Added
  - **`check-vr-config.sh`，三個子命令。** 分開是因為代價不一樣：`browsers` 與 `knowledge`
    不連網，`pages` 要連網；混成一個會讓沒有網路的地方連前兩個都跑不了。
  - `browsers`：把每個 project 解析成一顆引擎（具名 device、直接宣告 `browserName`、
    兩者都沒有），要求它落在宣告的集合裡。**第三種不判綠**——跑哪一顆由執行時的預設決定，
    而那正是這道檢查在問的東西；認不出來的 device 名字也一樣說出來，不當成 chromium。
    紅的時候明講**修法不是去裝一顆瀏覽器**：裝下去等於偷偷擴充工具鏈，而宣告仍然是錯的。
  - `pages`：每個頁面送一次請求並**跟到最終位置**才判，非 2xx 的逐個說出它最後停在哪。
  - `knowledge`：兩件只有撞過才知道的事有沒有還寫在那裡（錨＋內容憑據）。
  - **`check-vr-config-selftest.sh`（27 條）。** 三種離場碼各自都被驗到，**量不到那一種
    最容易被弄丟**——一個掃到 0 個目標的檢查，輸出跟「掃過了、都對」長得一模一樣。連網那
    一半靠注入的探測器，不打任何真的站台：判定邏輯要能在沒有網路的地方被驗，否則它的紅控
    只能靠某個外部站台今天剛好是什麼樣子。另外掃這支檢查自己的原始碼，確認它不帶任何安裝
    動作。
  ### Changed
  - **`visual-regression-config.md` 多兩段，各自帶錨。**
  - **那棵 tooling 樹不在版控裡**：它被公司目錄那條忽略規則蓋住，而那個目錄本身也不是一個
    獨立的 repo。所以那裡的東西沒有歷史、丟了就沒了，在那裡修東西不會出現在任何 diff 上。
    因此會漂的那兩件事由這個 skill 這一側的檢查守——檢查在版控裡，被檢查的東西不在，
    這是刻意的分工，不是遺漏。
  - **readiness selector 不跨組件樹共用**：同一個 URL 的桌機版與行動版可能由兩棵不同的樹
    渲染，而它們沒有共用的 selector（量到的那一組，行動版 SSR 整頁零個 `data-testid`）。
    一份只在其中一棵驗過的等待條件在另一棵上會逾時，而逾時讀起來像「頁面壞了」——這是它
    難查的原因，不是它罕見的原因。
...
- 04f880d: DP-519：報告送去一個它自己看不到的地方
  `standup` 的整個 publish 半邊寫死指向一個外部頁面——SKILL.md、格式契約、一份專門的操作
  手冊，加上公司設定裡的一個頁面 ID。而那個目的地 2026-06-25 就搬走了，形狀也換了（原本
  四區塊加團隊分組，那裡是逐張 epic 三格）。一個多月裡沒有任何東西會紅，本機 standup 檔案
  就從那天起斷到 2026-08-13。形狀跟 DP-518 是同一個：宣告的入口搬走了，沒有東西會紅。
  ### Added
  - **`resolve-standup-destination.sh`——目的地從宣告來，不從散文來。** 這支 skill 現在不
    認得任何一個目的地：送到哪、那裡的表單長什麼形狀、送出是人做的還是程式做的，全部住在
    那家公司自己的 `workspace-config.yaml`。四種答案的離場碼互不相同，**不得收斂成同一句
    「找不到」**：0 齊全、3 宣告在但缺欄位、4 沒有宣告、2 量不到。3 與 4 分開是因為半條宣告
    比沒有宣告糟——它看起來像有人設定過。沒有宣告時報告照常產出、照常寫本地檔案，說出是哪
    一家缺什麼，**不猜也不沿用上一個目的地**。
  - **`resolve-standup-destination-selftest.sh`（11 條）。** 正負兩表都跑，另外單獨驗一件
    事：三種輸入的**第一行**互不相同。只逐條比對子字串的話，三種都印同一句而各自剛好含著
    不同的片段仍然會全綠。
  ### Changed
  - **產出形狀改成以 epic 為主體、每張 epic 三格（昨日／今日／卡關）**，對齊目的地實際的
    表單，而不是先產一份四區塊的東西再現場翻譯。舊的四個名字沒有消失，它們變成格，對映
    表只有一份；`口頭同步` 留下來但**只留在本地**——表單沒有那一格。沒有 epic 的東西集中
    在一筆具名的非 epic 條目裡，不散落。本地那份檔案不再是「備份」，它就是要送出去的內容。
  - **plan vs actual 的比對來源改成這支 skill 自己每天落下的那份本地檔案。** 以前它讀那個
    外部頁面上「今天以前最近一筆」，目的地一搬就再也沒有新的一筆，於是這一步每天都走進
    skip 條件——而 skip 條件在散文裡長得像偶發狀況。現在生產者與消費者是同一個檔案，而且
    每次都要說出拿哪一天的哪一份比的；沒有可比的前一份時說出為什麼，**不沉默跳過**。
  - **卡關的准入判準從一句話變成一張照著判得出來的表。** 「等 X 確認／覆核」「等 X 回」
    「等 X review」「等 X 拍板」「要跟別的團隊聯絡」逐種列出並判進卡關，理由是它們都要
    開會或通知當事人。判準會判錯的那一列仍然指名：一張已經 merge、後面由別人接的單**不是**
    卡關，因為那件事回來之後我手上不會多出工作。判準放寬的是什麼進得去，**不擴張從哪裡找**。
  ### Removed
  - **一份有消費者、沒有生產者的輸入。** 排序狀態檔的讀取整個拿掉了：寫它的那支 skill 早就
    不在，本機那份停在 2026-06-16，而契約每天照著說一次「今日沒有那份狀態」——那句話沒有
    收件人。一個永遠只有一種答案的輸入不是輸入，它是儀式。這跟〈問不到就說出來〉不衝突，
    兩者現在互為對照並寫在一起：有生產者而這次問不到，要說出來；根本沒有生產者，要拿掉。
  - **那份專門的外部頁面操作手冊。** 目的地一搬它就是屍體，而且它只有這一個消費者——散文裡
    說「部分 skill 也用它」的那句話，量下去 0 命中。
  ### Fixed
  - **契約檢查跟著改，而且不准靜靜地換。** `check-standup-contract.sh` 多四個契約點，並多
    一組方向相反的檢查：**不得再出現的東西**（那份沒有生產者的輸入、那份被刪掉的手冊）掃
    整個 skill 目錄，因為一句指向死掉的東西的指示出現在腳本註解裡跟出現在契約裡一樣會誤導
    人。宣告面與注入面自己要寫得出那些字串，所以它們被豁免——**豁免逐個印出來**。
    被取代掉的兩條不變量（四區塊、依 team 分組）連同它們原本守的斷言留在腳本裡並每次印出
    來：一個不變量默默消失，與一個不變量被有意識地替換，在檔案裡長得一樣。

## [4.38.0] - 2026-08-13

### Changed

- d825f72: DP-518：搬走了而沒有說一聲的那些入口
  `gate-dangling-declarations.sh` 是為了 2026-08-02 那次 `teardown: scripts/ 歸零` 寫的，它的
  檔頭寫著判準：**入口就是沒有別的東西引用它的宣告，所以沒有別的閘會在它壞掉時變紅**。判準
  寫對了，適用範圍抄漏了——它只掃 `mise.toml` 與 `.claude/settings.json` 兩處，而同一次刪除
  在同一個 repo 裡留下的另外十條死宣告，它一條都看不到，還印著「✅ 5 個執行宣告都指得到現在
  存在的檔案」。
  ### Changed
  - **`gate-dangling-declarations.sh` 多掃 `package.json` 的 `scripts`。** 擴進來的當下判紅
    **10/15**——根目錄那十條 npm script 沒有一條例外，全部指向被那次刪除帶走的 `scripts/`，
    而四個目標裡有三個在整棵樹上都不存在。同時補上揭露：有幾條 script、其中幾條沒有指名檔案
    路徑（代理給 `pnpm` 的那種）因而不判定，逐條印名字——不判定不等於沒有那些東西。
  - **`gate-prose-matches-behaviour.sh` 多判一種散文：腳本的檔頭。** 一支腳本的檔頭寫「細節見
    `references/X.md`」也是指路，也會在搬家之後失效，而它不會炸——註解不被執行——所以可以這樣
    待很久。**只看檔頭那一段連續註解／docstring**，這條界線是量出來的：全樹這樣的相對指路約
    80 處，絕大多數落在 selftest 中段（造假樹用的字串，本來就該不存在）；限定檔頭之後 244 支
    腳本裡只有 1 條，而那 1 條正是斷掉的那條。
  - **`polaris-env.sh` 的工作區根改成用找的**（往上走，第一個底下有 `.claude/skills/` 的那一
    層），不再是寫死的 `$SCRIPT_DIR/..`——那個相對深度在腳本歸位到 skill 目錄之後就停錯了一
    層，於是每一次 config 查找都去撈一個永遠不存在的路徑。修法不是把 `..` 改成 `../../../..`：
    那只是把同一個假設寫得更深，下一次搬家一樣安靜地錯。
  ### Added
  - **`visual-regression` 有 selftest 了**（在這之前一個都沒有）。它**執行**這支 skill 散文
    指名的每一個入口，而不是閱讀它們——這一類腳本自己在執行期算出「工作區根在哪」，路徑是變
    數，靜態的閘讀不到（樹上那道掃引用的閘自己就把 1952 條這樣的引用列在「判不了」那一格）。
    把修好之前的那一行放回去，它立刻判紅並印出那個錯誤的路徑。
  ### Removed
  - **toolchain runner 與它的 manifest 退場。** 那支 runner 的 parser 在腳本歸位那次被刪掉、
    沒有跟著搬，所以它從那天起就是屍體，不是一個路徑修得好的東西。連帶修掉的**活**消費者，
    每一個都壞了同樣久而沒有人知道：
  - workspace bootstrap 的三行 `run <capability>`——也就是說「初始化執行環境」這個公開入口
    整段期間是壞的。現在直接下那三條 `pnpm --dir …` 安裝命令。
  - doctor 的兩格 capability 檢查每一次都 fail，而訊息是「fixtures.mockoon.doctor failed」
    ——讀起來像 mockoon 壞了，不像 runner 不見了。
  - 狀態頁的 runner 那一格檢查那支腳本存不存在，答案一直是不存在，而它報的修法是「去
    `git restore`」——一個救不回來的修法。
  - manifest 退場的理由是它退場之後**一個內容消費者都沒有**（只剩兩處存在性檢查與一則
    註解），而它記的 tool 版本 `mise.toml` 已經有一份。兩份會漂。
  - **根目錄十條 npm script**。四個目標裡三個整棵樹上都不存在，所以處置不是「指向新位置」。
    viewer 改成三條直接代理到 docs 站台的 alias；toolchain 的能力走 `mise run bootstrap` 與
    `mise run doctor`；另外兩條沒有接手的人，就說它沒有。
  ### Fixed
  - 一支腳本的 docstring 指向一份不存在的 reference（同目錄下有的是另一個名字）。
  - **一條會對每一張後續的單噴假紅的負向 selftest。** 某支 skill 的形狀 selftest 有一條
    「這張單在本 skill 以外刪了東西 → 紅」，而「這張單」是寫下它的那一張。它比的是
    `merge-base(HEAD, main)..HEAD`——一支常駐 selftest 手上永遠只有這個，而它分不出那是原本
    那張單還是後來的任何一張。所以那張單併回主幹之後，這一條對**每一張**在別支 skill 底下
    刪東西的單都是誤判，訊息還指名一支跟那次改動毫無關係的 skill。這次退場一支死掉的
    runner 時撞上了。
    第一次的修法是「先問手上的 diff 動到那支 skill 了嗎」，當場被自己推翻：**改那支 selftest
    本身就算動到**，同一個誤判立刻回來。所以拿掉的是那半條，而不是把它調鬆——一條單票範圍
    的負向斷言沒有辦法用常駐 selftest 表達，它的紅控是那張單的 diff，由看 diff 的人負責。
    留下來的是它真正還成立的那一半（那幾份複本還在不在），而且那一格明講自己只做了一半：
    一個安靜地縮水的負向檢查，跟一個完整的負向檢查在輸出上長得一樣。
  ### 已知的界線
  - **那三條 viewer alias 的接線驗過了，但 docs 站台在寫這行的機器上跑不起來**：靜態網站產生
    器真的起來、sync content，然後在一份**未追蹤**的內容檔上失敗（缺 `title` frontmatter）。
    那個路徑由 `.gitignore` 排除，不在這個 repo 裡，所以這張單沒有動它——另開了一張單追。
    這裡如實寫下來，因為「alias 接對了」與「viewer 跑得起來」是兩件事，而只寫前一句會讓下一
    個人以為後一句也成立。
  - **副作用一則**：掃引用那道閘的「只有一條候選而它落空」從 1 降到 **0**——那個唯一活著的
    實例就是被退場的那支 runner。上一版揭露的那條已知界線現在只剩 fixture 撐著，兩處註解都
    改了。判準沒變，撐著它的東西變了。
  - 這一輪只動版控裡的東西。種子單記下的四條 domain 設定發現落在 `.gitignore` 排除的那一棵，
    不在這張單的落腳處，另開一張單。
...
- 2227579: DP-513：這道閘只看自己家門口
  `gate-skill-script-references.sh` 印的是「✅ 241 個檔的同目錄引用都對得上」，而它眼裡一條
  跨目錄的引用都沒有——正則不吃 `..`，而且要求副檔名是 `.sh`／`.py`／`.mjs`，所以指向別支
  skill 的、指向目錄的、指向散文的，一整批都不在管轄內。
  實際後果 2026-08-12 量到了：兩支 `fetch-pr-info.sh` 的 `github-rest.sh` 候選清單三條全是
  DP-462 之前的 repo 根佈局、三條都落空，於是那整條 REST 路徑**永久是死的**。而因為 10 個
  呼叫點都用 `declare -F` 包著、有 `gh` CLI 的 else 分支，輸出上跟它正常運作長得一模一樣。
  那不是一個 crash，是一個安靜的第三態。
  跟 DP-514（v4.35.0，副本一致檢查進常駐閘）是同一個病的兩面：那一張是「檢查看得見但不在
  閘裡」，這一張是「檢查在閘裡但看不見」。
  ### Changed
  - **管轄的分界不再是「跨不跨目錄」**，改成兩層，兩層都跟目錄無關：
  1. **這條引用指名了東西嗎**——往上爬完之後還剩不剩一個具名的元件。`$SCRIPT_DIR/../..`
     這種沒有，對它做存在性檢查永遠是綠的，所以不判定但**印出條數**。
  2. **它被存在性檢查包住嗎**——包住的是**候選**，不是要求。`for candidate in …` 那種清單、
     `if [[ -f X ]]; then . X; fi` 那種先問再用，**整組全落空才紅、而且判一次不是逐條**。
  - **目標可以是目錄、可以是散文。** 一支腳本指名另一支 skill 的目錄，跟指名一個檔案一樣會在
    搬家之後變成洞；沒有副檔名不是放它過的理由。
  - **哪個變數算「從自己的位置算起」由賦值的形狀決定，不由名字白名單決定。** 那個名單是這道閘
    兩次踩過的同一個坑：第一版只認大寫、漏掉 `$script_dir`，洞活到 4.0.0 的釋出尾段才炸；
    補成大小寫都收之後，`polaris-doctor.sh` 的 `SCRIPT_DIR="$WORKSPACE_ROOT/scripts"` 還是解
    不開——不是因為它不自明，而是因為 `WORKSPACE_ROOT` 不在名單上，儘管它自己就是
    `$(cd "$(dirname "$0")/.." && pwd)`。而舊實作對認不出來的賦值一律退回「那就是同目錄」，
    在這棵樹上每次剛好猜對，於是它看起來像解析。
  - **判定條數 190 → 299**（放寬前的舊閘只看得到同目錄那一批）。九道閘合計 2.55 秒。
  ### Added
  - **每次印一行 DISCLOSURE，綠的那一次也印**（DP-514 立的先例）：判不了的六類各自的條數——
    純往上爬、路徑不自明、解到 repo 之外、註解裡、這一行在建立它、只有一條候選而它落空。
    不判定不等於沒有那些東西。
  - **selftest 從 15 格擴到 42 格**，每一個放寬出來的類別都有正反兩例。兩類真樹上沒有實例
    （多目標候選群組的全落空與至少一條命中），檔頭就寫明那兩格只有 fixture 撐著。
  ### Fixed
  - **兩支 `fetch-pr-info.sh` 的 lib 解析**改成 `${SCRIPT_DIR}/lib/github-rest.sh`，找不到就
    `exit 2` 並說出來，不靜默退回 `gh`。實跑對照（`bash -x`，open PR）：修前 `polaris_gh_api`
    **0 次**／`gh api repos/…` fallback 6 次，修後 **4 次**／0 次，兩邊都 exit 0。
  - **`standup/scripts/lib/tool-resolution.sh` source 一個不在旁邊的 `tool-attribution.sh`**
    ——放寬之後才露出來的第三個洞。補齊那個鄰居並登錄進副本組（38 組）。
  ### 已知的界線
  - **一組只有一個目標而它落空，這道閘不判定。** 它分不出「一條永遠走不到的 fallback」與
    「一個本來就該落空的探測」——`polaris-toolchain.sh:18` 問的是「這個 skill 目錄自己是不是
    workspace 根」，而 `polaris-toolchain.yaml` 真的存在、在 repo 根。兩者形狀一模一樣。
    那一類進 DISCLOSURE，走的是負向表列明文允許的第二條路，不是就地放寬。
  - 因此**樹上現在活著的候選形狀（4 組單目標）全部落在不判定那一格**，多目標那一類只有
    fixture 撐著。這不違反任何斷言，但它是下一次問「這道閘擋得住什麼」時該先讀到的話。
...
- 1731211: DP-516：一張單最新的話在留言裡
  2026-08-12：寫當天的 standup，被使用者連續校正四次。四次都是同一個形狀——**我把「系統現在
  長什麼樣」當成了「這個人昨天做了什麼、今天要做什麼」。** 描述、狀態名、拆單表全都是系統的
  表面，全都會 stale；帶著時間、只能追加的那一份（留言、merge 事件）才排得出先後。
  四個洞逐一修在 `standup` 自己的散文裡：
  - **留言從來沒被收過。** 整個 `references/` 樹裡 `comment` 這個字只出現過一次，指的還是
    PR 那一節。實例：一張單的描述寫著「三種 flow 尚未量到」，而同一晚 01:00 與 02:21 兩則
    留言已經把三格都補上了，報告照描述寫成「今日要補量」。現在留言要收，用
    `created >= YDY_DATE` 綁在查詢那一層——不是先全部拿進來再挑，一張活躍的單光描述加六則
    留言就大到必須落檔。衝突時留言勝過描述，理由不是「留言比較重要」，是**只有留言證明得了
    自己什麼時候寫的**。
  - **昨天的事漏掉了分支那一層。** merge 事件、收到的 review 意見、CI 狀態變化三類進 YDY。
    兩條昨天各自被 merge 的分支，報告一開始把它們寫成還在進行中。
  - **狀態名不是待辦。** `Ready for Stage` / `Waiting for QA` 生出「追 QA、排 stage」，而那
    張單 merge 就結束了；一張過期的拆單表生出「AC-5 未交付」，實際上那條掛在別的 Epic、由
    別人負責、東西早就進了；`--author <自己>` 濾別人的東西濾不到，就下了「沒人做」的結論。
    三種都寫進〈狀態不是意圖〉，指名為不得作為待辦來源。
  - **BOS 以前只有格式，沒有一個字說什麼算 blocker。** 判準改成「我在等誰」，含會判錯的那
    一格：「等 QA 排程」字面上完全符合「需要別人動」，但那張單對我而言已經結束——分辨法是
    問「那件事回來之後我手上會多出什麼工作」。
  - **產出物精簡，證據不精簡。** 五條規則只管拿來掃的東西（站會四區塊、看板欄位、口頭同步）；
    量測條件、成因推導、驗證紀錄不套——那一類的價值就在完整。
    另外多一件這一輪長出來的：standup 是唯一會把「單上寫的」與「量到的」擺在一起看的時刻，
    落差不列下來就消失。所以草稿多一段〈發現 N 處與現況不符〉，逐條給單號／段落／量到什麼／
    建議改成什麼，並把「量到」與「待驗」分開標。**但不自動寫**——這一輪我自己就推導錯兩次，
    自動寫會把錯的變成別人讀到的事實。
    散文契約由 `scripts/check-standup-contract.sh` 守：11 個錨、5 條不變量、45 條憑據。錨管
    「有沒有說新的」，不變量管「舊的還在不在」，缺一邊就有一整類的漂沒有人看得到。只有錨沒有
    內容一樣判紅——不然刪掉散文、留下註解就過了。
  ### 這一輪自己踩到的
  - **`$name（` 在這個 locale 下是 unbound variable**，全形括號的第一個 byte 被讀進變數名。
    selftest 在第 106 行整支死掉，而它印出的 exit code 是 **0**——一支自己死掉的餵紅工具，
    跟一支全過的在輸出上長得一模一樣。
  - **第一趟 13 條紅控裡有 5 條回 exit 0，全部是注入太弱**：憑據寫成 `A|B` 的 alternation 而
    我只拿掉一個分支，或同一個詞出現三次而 `replace(..., 1)` 只吃第一個。拿光之後五條全紅。
    這跟 DP-512 記下的是同一句話：**注入本身也要先確認它真的改到了要測的那件事。**

## [4.33.0] - 2026-08-12

### Changed

- f388109: DP-512：處置要回到提出者那裡，不只回到別的地方
  2026-08-11：一個 PR 收到三位 reviewer 的意見，全部查證、修掉、斷言重跑全綠，也回了另一個
  系統的訊息串——**但那個 PR 上一則回覆都沒有**，是人自己發現的。偵測有主人（誰欠我回覆、
  誰欠我審查，都有東西在算），**動作沒有主人**。而「還沒被 approve」這個狀態，跟「我還沒
  動手」長得一模一樣。
  跟 2026-08-03「開 PR 在三站裡沒有主人而被刪掉」是同一個形狀，所以結論也一樣：歸
  `swe-knowledge`。
  ### Changed
  - **DoD 從四條變五條**：第 5 條是「reviewer 提的每一條，處置回到那條意見上」。判準不是
    「有沒有處理」，是**提出者拿不拿得到那個處置**——他看的是他留言的地方。
  - **`check-swe-done.sh` 從查三件事變成四件**。第 5 條算的是「串頭有沒有被作者接過話」：
    一串意見的所有回覆都指向那一串的第一則，所以問的是「有沒有一則回覆的作者是 PR 作者」。
    **另一位 reviewer 在同一串裡接話不算**——把「這串有人回過」當成成立的那一版，會讓兩個
    reviewer 互相討論就把作者放行。問不到（沒裝 `gh`、遠端回非 0、解不出 owner/name）各自
    說出自己是哪一種並且回非 0，不當成通過。
  - **`SKILL.md` 多一段〈回覆 reviewer〉**：三步，順序不得顛倒——落地成檔案 → 過對外寫入
    的檢查 → 才送。送出去那一行跟〈開 PR〉同一個待遇，一行 `gh api`，**不帶包住它的腳本**。
  ### Added
  - `swe-knowledge/scripts/resolve-external-write-gate.sh`——那道對外寫入的檢查**不住在這
    裡**，由認領它的那支 skill 自己宣告，形狀照既有的宣告點：
    <!-- {任意前綴}-EXTERNAL-WRITE-GATE: {檢查文字的命令} -->
    這支 skill 到現在為止指不到自己目錄以外的任何東西，而那是刻意的——它會被單獨匯進別人的
    環境，寫死一條指向別支 skill 的路徑，在那裡是一個安靜讀不到的東西。**沒有人宣告時它
    說出來並回非 0**：那個離場碼的意思是「這個環境沒有那一層，落地成檔案你自己要做」，不是
    「不用檢查了」。宣告了兩份也停——兩個答案不比一個答案好。
  - 帶著那道檢查的那支 skill 補上宣告；它本來就是那道閘的呼叫者。
  - `swe-knowledge/scripts/selftests/review-reply-selftest.sh` ＋ 一份假的 `gh`，13 條斷言
    逐條餵紅過。
  ### 這一輪自己踩到的
  - **`IFS=$'\t'` 底下連續的 tab 會被併成一個分隔符**（tab 屬於 IFS 的空白類），所以
    `read a b c` 讀 `1\t\talice` 會把 `alice` 讀進 `b`。那一版把每一則留言都當成回覆，於是
    「有人沒回」永遠是綠的——而且它一次都不會紅。修法是讓查詢在那一欄吐 `-` 而不是空字串。
  - **餵紅的腳本用 `git checkout --` 還原，把還沒 commit 的兩個檔案回捲到 HEAD**，於是後半
    段每一條都在對著沒有這次改動的舊版量——十三條裡有一半的「紅」是假的。先 commit 再餵紅，
    才知道紅的是注入還是自己手滑。
  - **一個沒有真的改到順序的注入會通過**：B-P2 第一次的注入只改了段落標題的字，位置沒動，
    於是量測照樣綠。注入本身也要先確認它真的改到了要測的那件事。

## [4.32.0] - 2026-08-12

### Changed

- e76e4f4: DP-510：判定產得出一份交得出去的東西，發佈由宣告驅動
  判定過了，證據卻只留在本機——`report-assertions.sh` 只印到終端，沒有檔案可以交給下一層。
  2026-08-11 有一趟真的要把佐證貼回單上的工作，因此是整段手工重做的：手挑檔名、手 curl
  上傳、手拼 wiki markup。那一趟證明了機制是對的，問題只在沒有任何一步會自己走到那裡。
  ### Added
  - `verify-ac/scripts/render-evidence-report.sh`——把已經判定過的東西排版成 `report.md`
    （人看的）與 `manifest.json`（機器讀的）。**唯讀**：不寫輪次狀態、不寫交付紀錄、不碰
    證據檔案，也不成為第二條可以宣稱 PASS 的路徑。離場碼說的是「產不產得出來」而不是
    「過了沒」——`0` 產得出來、`2` 前置條件不到、`4` 產得出來但沒有人說要送去哪。
    **有東西沒過照樣產得出來**，因為最想看報告的那一刻正是那一刻。
  - `verify-ac/scripts/lib/evidence_report.py`——排版那一段。逐條判定仍然住在
    `lib/assertion_verdicts.py`，與交付那條路共用同一份。清單上多一格
    `measurement_ledger: absent|present`，因為「這一層沒得做」與「做了而且是空的」是兩件事。
  - `verify-ac/scripts/resolve-evidence-publish.sh`——新的宣告點 `EVIDENCE-PUBLISH`，形狀照
    既有的 `ISSUE-STATE`／`PR-CONTEXT`：
    <!-- {任意前綴}-EVIDENCE-PUBLISH-{命名空間}: {命令} -->
    **核心不認得任何一個目的地**，也不認得任何一家公司的追蹤系統。沒有人宣告某個命名空間
    時它指名那個命名空間、印出修法、報告留在本機，不沿用別人的宣告也不猜一個。
  - 公司層那一支 skill 補上宣告與它指向的留言器：上傳圖檔與報告，然後用 v2 API 的 wiki
    markup 貼一則留言（v3 的 markdown 會把 `!檔名.png|thumbnail!` 渲染成字面文字，圖片不會
    出現）。它不判定任何事。
  - 兩支 selftest，14 條斷言逐條餵紅過。**它們刻意分成兩支**：可攜的那 7 條留在 `verify-ac`，
    另外 7 條非得指名某一家公司的介面才量得到，所以住在那家公司自己的那一層。
  ### Changed
  - 附件上傳器（樹上兩份逐位元組相同的複本一起改）修好兩個洞：憑證現在收 `JIRA_USERNAME`
    或 `JIRA_EMAIL`，讀不到的時候說出它找了哪幾個名字、實際設了哪幾個；站台位址改從
    `workspace-config.yaml` 的 `jira.instance` 來，**而且沒有預設值**——用單號前綴對
    `jira.projects[].key` 挑那份設定，問不到就停下來說去哪裡設。多一個
    `--print-target <單號>` 模式，讓留言那一支跟它問站台與帳號，不自己再算一次。
  - 那份講追蹤系統介面細節的報告模板，從視覺回歸那一層搬進認得它的公司層，指過來的三處
    散文一起改。
  ### 這一輪自己踩到的
  - **「找到了但找錯」第三次**：解析站台時掃到 `_template/workspace-config.yaml`，回一個
    佔位的站台位址——它有答案、離場碼 0、看起來成功。修法是用單號前綴挑設定，並在兩層都
    排除 `_template/`。
  - **`if 命令; then …; fi; rc=$?` 讀到的是 `if` 的結果**，而那永遠是 0：訊息會說「離場碼 0」
    然後宣稱失敗，讀的人兩句話都不能信。改成 `set +e; 命令; rc=$?; set -e`。
  - **`gate-template-leaks` 擋下來是對的訊號不是雜訊**，這一輪擋了兩次：第一次是那 7 條量測
    非得指名公司介面，卻寫在會同步出去的可攜層；第二次是這一則 changeset 自己——它的散文
    指名了公司與單號，而 `CHANGELOG.md` 會被同步到公開的 template repo。兩次的修法都是把
    內容放回它該住的那一層，不是把閘放寬。
...
- d3c9d2f: ### Added
  一張單的終局不再只有一個來源說得出口（DP-508）。
  - **`spine-loop-state.sh released`**：核心多認得一件事——**釋出紀錄可以由別的東西寫**。
    它去問那張單的領域知識宣告的終局訊號（`<!-- SWE-DELIVERED: ... -->`），把單記下的落腳處
    與身分原樣接上去跑，訊號說走完了才寫 `release.json` 並重算位置。核心仍然不認得 PR、
    不認得 merge、不認得任何一種釋出。
  - **`swe-knowledge` 說出停止點**：落在別人 repo 的工作，**那個 PR 被 approve 或被 merge
    就是走完**。之後那個 repo 自己怎麼釋出不由這裡管、不偵測、不記錄——每個 repo 有自己的
    節奏，而我們不是它的 owner。上線也不進這套流程：沒有欄位，就沒有東西會把「還沒回填」
    讀成別的意思。
  - **`close` 收拾自己留下的痕跡**：新的 `<!-- SWE-CLOSE-CLEANUP: ... -->` 宣告讓關單同時
    關掉這張單自己的 PR（留言附 `git fetch origin refs/pull/<n>/head` 當復原路徑）並刪掉
    已經併進預設分支的本地 branch。**還活著的東西一律不動**，只逐個列出來並且非 0。
    2026-08-08 DP-440 關單之後，它的 branch 與 PR 活到 2026-08-10 才被人工看見。
  ### Fixed
  - **釋出自己記帳**：`spine-release.sh` 寫完釋出紀錄之後會重算位置。在這之前那一段從來
    沒有做過投影——398 張躺在 `released/` 的單位置是對的，但它們是靠後來某張不相干的單跑了
    `record` 才被順手擺正的，所以**永遠是最後釋出的那一張錯著**，直到下一個人開下一張單。
  ### 量它們的儀器
  四支新 selftest 共 24 項，`gh` 用 PATH 上一支餵定值的樁（拿真 PR 量到的是「GitHub 今天回
  什麼」）。三刀紅控各自一份乾淨的副本：把「沒宣告就拒絕」拿掉、把收尾從 `close` 拆掉、
  把「只刪已併入的」那道判斷拿掉——每一刀都先驗它真的改到了東西，再確認 selftest 真的紅。
...
- 708cb5e: ### Added
  `run-selftests.sh` 的環境隔離現在有一道會紅的檢查在看著（DP-507）。
  - **它擋的是什麼**：git 跑 hook 的時候一定會把 `GIT_DIR` 放進環境，而**顯式的 `GIT_DIR`
    蓋過 `-C`**。一支 selftest 對自己的 fixture 樹下 `git -C "$d" config ...`，寫進去的是
    真的那個 repo 的 `.git/config`。2026-08-10 的實際損害：32 個 commit 的作者被寫成
    `t <t@t>`，工作區索引從 499 個檔清成 2 個。
  - **在這之前沒有人在看**：擋著它的只有 `run-selftests.sh` 裡一行 `env -u`。把那一行拿掉，
    48 支 selftest 全部照樣綠——一道不會紅的保護跟沒有保護的差別只有原始碼裡的字數。
  - **量的是行為，不是原始碼**：新增 `run-selftests-env-isolation-selftest.sh`，造一棵假的
    skill 樹、裡面一支刻意複製出事形狀的 canary，在帶著 `GIT_DIR` 的環境下跑一輪，然後看
    借出方的設定、HEAD 與索引有沒有被動到。只讀「那一行還在不在」不算：換個寫法它就靜靜
    失效了。
  - **先證明注入是真的**：一個根本沒注入成功的環境，跟一個注入了但被擋住的環境，在「repo
    沒被動到」這件事上長得一模一樣。所以這支先不設防跑一次 canary，確認它真的弄髒了借出方，
    再跑受保護的那一次。正向控制不成立時是量不到（exit 2）並說出原因，不是通過。
...
- 98d34c3: ### Fixed
  `verify-ac` 的第三層（重跑一次）現在把證據記下的工具交還給 oracle，而且判紅時說得出理由（DP-506）。
  - **重跑拿得回當初探過的工具**：oracle 會把 PATH 釘死成宣告的那幾個系統目錄，只有
    `--require-tool` 探到的才 symlink 進去；證據裡本來就記著那份清單（`tools`，帶名字與能力
    探針），但 `_rerun()` 只組回 `--expect-evidence` 與 `--forbid-evidence`，從來沒讀過它。
    於是一條當初靠 `gh` 才跑得起來的命令重跑時 exit 127——這一層從「再驗一次」變成
    「懲罰用過外部工具的單」。2026-08-10 DP-505 八條斷言裡的兩條就這樣被判紅，而那兩份證據
    是好的。
  - **判紅要說得出為什麼**：oracle 判紅時 marker 與說明印在 **stderr**，而 `_rerun()` 讀的是
    stdout，於是唯一看得到的訊息是「（沒有輸出）」。一個判紅而說不出理由的閘，跟一個沒有理由
    的通過一樣不能用。現在往回取三行——最後兩行是 oracle 說的，再前面一行是命令自己說的
    （`gh: command not found` 那句才是人要看的）。
  - **沒有 `tools` 欄位的舊證據照沒探過跑**：那是一個答案，不是缺料。DP-506 之前的證據全部
    長那樣，而它們的命令本來就只用釘死的 PATH 上那幾支。
  - **去重不再把另一條斷言的答案借給這一條**：重跑照 `(命令, 樹)` 去重，而決定那一趟結果的
    還有證據樣式與工具。兩條斷言跑同一條命令、各自要求不同的正向證據時，第二條拿到的是第一條
    的答案，它自己的樣式從來沒有被檢查過——加了紅控之後量到：舊的鍵之下整份報告 exit 0 全綠。
    這是「沒量到被讀成過了」的形狀，而它在 v4.27.0 是活的。鍵改成整趟重跑的全部條件。
  - **失敗路徑上的說明真的印得出來**：`report-assertions-selftest.sh` 與 `no-orphans-selftest.sh`
    裡 12 處 `$RC：` 被 bash 3.2 把全形冒號吃進變數名，於是紅的時候炸的是
    `RC：: unbound variable`，不是那句寫好的說明。這兩支的紅控是在這一輪第一次被走到才發現的。
...
- c65f817: ### Changed
  `verify-ac` 現在答得出「這張單過了幾條」，而且交付那條路不再只讀幾個 JSON 檔（DP-504）。
  - **一個唯讀命令逐條回答**：新增 `report-assertions.sh`，輸入是 fence 裡的斷言，輸出是每一條
    過、沒過、還是量不到。在這之前那段判定只長在交付那條路上，而交付**任一條不成立就整支拒絕、
    什麼都不印**——想知道現在到哪裡了，只能把 oracle 一條一條重跑自己拼。它不寫任何檔案：
    能宣稱「這張單可以出貨」的仍然只有交付紀錄，有兩個地方能宣稱，它們遲早會給出不同的答案。
  - **報告與交付共用同一段判定**：抽進 `scripts/lib/assertion_verdicts.py`，
    `record-delivery-intent.sh` 從 515 行降到 347 行。改判準只會改到一個地方。
  - **證據的可信度分三層，交付三層全做**：檔案自洽（原有）、**登錄相符**（證據記的命令要等於
    這條斷言在量測登錄裡簽過的那一條）、**重跑一次**（拿那條命令現在再跑一次）。在這之前，
    一份手寫的 `{"producer": "run-hardened-oracle.sh", "verdict": "PASS", "head_sha": 對的}`
    就交付得出去，而外部量到的 agent 行為第一項就是改與評分有關的檔案。「這張單沒有量測登錄」
    不是豁免——少了第二層，一份手寫證據可以自己指名一條一定會過的命令。
  - **擋不住什麼寫進 SKILL.md**：逐條指名，包含「一個能在同一棵樹上執行任意命令的施工端」。
    三層不是證明，它把偽造的成本從「寫一個 JSON」提高到「改一條登錄過的命令，而那個改動會
    出現在 diff 裡」。說它擋得住所有偽造，會讓下一個人不再看 diff。
  - **註解裡的旗標不算存在**：`gate-prose-matches-behaviour.sh` 剝掉整行註解再取詞表。腳本的
    `# Usage:` 檔頭是散文的一種，拿散文去驗散文永遠是綠的。收緊讓四筆本來對的散文變紅，全部
    指向一支 exec 到 `lib/` 的殼——修法是讓詞表跟得到那種寫法的殼，不是放寬判準。
  - **沒有理由的東西拔掉**：刪掉 766 行零呼叫的 `lib/release_closeout_helpers.py`、
    `enumerate-spine-inventory.sh --out`（唯一的讀者讀的就是預設位置）、
    `run-hardened-oracle.sh --min-evidence-bytes`（預設關閉、零呼叫，而 `--expect-evidence`
    嚴格更強）。留下來但沒有呼叫者的三個旗標各自寫下理由，並由新的 `no-orphans-selftest.sh`
    盯著；`standalone-selftest.sh` 把整支 skill 複製到只有它自己的地方跑一次，證明
    `scope: standalone` 不只是一句宣告。
...
- 5263d91: 釋出尾段從任何中斷點重跑都對，而且說得出自己走到哪：`--status` 逐項問每一個真的擁有那件事的系統（沒有進度檔——那會是同一件事的第二個答案）；交付紀錄釘的不是 HEAD 時分得出「上一趟自己壓的版號 commit」與「有人塞了沒被判定看過的改動」，前者自己重釘往下走、後者照舊拒絕並說出哪一項不成立；tag 與 GitHub release 各問各的系統，問不到 release 在不在就停，不當成已經有了

## [4.24.0] - 2026-08-10

### Changed

- 56f1b4e: 釋出尾段被打斷之後接得回去：交給下游腳本的單路徑改由單一產生者算成絕對路徑（不再讓下游對著呼叫者的 cwd 解析、把「你站錯地方」說成「這張單沒有 index.md」），促進那一步找不到 open PR 時先問 git 那條分支在不在 `origin/main` 裡——在就說出上一趟已經做完並跳過，不在才拒絕

## [4.23.0] - 2026-08-10

### Changed

- 1d45476: 釋出尾段不再把一個狀態回報成另一個狀態：縮寫的 `--head` 會被解開並說出來（解不開就明說解不開）、被說成不同的兩個 sha 印出來一定看得出不同、commit 之間的差異改問一組候選的樹而不是綁在一棵釋出前一步就會被移除的 worktree 上、`sync-to-polaris.sh` 把「沒有新變更要同步」與「沒有東西要推」分成兩件事
...
- f11fdcb: DP-387：指向一個不存在的地方，要有東西看得見
  這張單原本是 company handbook 遷移（`.claude/rules/{公司}/handbook/` →
  `{公司}/polaris-config/handbook/`）。三個對象逐項量過，**一個都不存在了**——那個遷移已經
  由 DP-484 用另一條路完成：handbook 搬進 skill 自己的目錄。活下來的是意圖，不是做法。
  **兩道閘中間有一格誰都沒有。** `gate-skill-knowledge-locality` 只判**版控之外**的引用，
  `.claude/` 開頭的直接出局；`gate-prose-matches-behaviour` 判斷指標，但它的三條正則都要求
  結尾是 `.md`／`.json`／`.yaml`／`.yml`。於是一個指向被拆掉**目錄**的指標兩邊都是綠的——
  而後者的訊息還寫著「斷指標由 `gate-prose-matches-behaviour` 管」，指向一道當時並不管它的閘。
  判準沿用這道閘自己既有的那一句：**解不解得出唯一位置。** `.claude/` 與 `_template/` 是這個
  repo 追蹤的兩棵樹，一份散文寫出它們就是在指這裡的某個位置；其餘一律不判，但逐條印出來。
  分界是量出來的，不是挑的：全樹 139 筆目錄型指標，全判的話 115 筆變紅——那會讓這道閘在
  三次之內被關掉，連它本來判得到的那些也一起沒了。照這個前綴判，12 筆解得到、3 筆解不到，
  而那 3 筆是同一個真的洞：
  - `repo-notes/SKILL.md` 指著四類公司級知識，而它們早就搬進同一支 skill 的 `references/`；
  - 兩份 `review-lesson-extraction.md` 叫人把 PR review 學到的東西寫進兩個都已經不存在的
    位置（一個沒有版控、一個是被拆掉的相容層）。
    順帶收掉一個沒有覆蓋的寫法：不帶結尾斜線、也沒有副檔名的路徑。全樹目前一筆都沒有，所以
    它的紅控只能長在 fixture 上——一條只在「剛好有人這樣寫」時才存在的檢查，等於沒有檢查。
...
- ae05996: DP-497：東西住在哪一層，變成一件會被說出來的事
  三半共用同一個判準——一段話、一支腳本、一個檔案，屬於哪一層要有東西判得出來：
  - **F 框架脈絡不住在可攜的 skill 裡。** `scope:` 的判準改寫成一個問題：照這支 skill 自己
    的描述做成通用、不依賴環境，是不是更好用？答案幾乎總是「是」，所以 `framework` 那一格
    只剩 `framework-release`。五類回答「這個 workspace 的某個東西在哪」的解析器在沒有工作區
    的環境裡不再無聲失敗——它們說出自己答不出來，然後照樣工作。腳本 185 → 176、副本 68 → 58。
  - **K 一段話寫進錯的層時有東西會說話。** 三條 `PROSE-LAYER` 宣告 ＋ `gate-layer-vocabulary.sh`
    ＋ 九個 case 的 selftest，三層各有自己的紅控。`SWE-ONLY-VOCABULARY` 從零個讀者變成有讀者：
    往那一行加一個詞，立刻多抓到一處。
  - **B `destination` 的宣告重新被驗。** `gate-source-destination.sh` 補回 2026-08-03 拆卸時
    刪掉沒搬走的那道檢查，掛在釋出尾段同步之前。認不出來的位置算不成立——過度嚴格，不過度
    放行；不成立時逐條說出是哪幾個檔案，而且不代人改宣告。
    順手修掉兩個量出來的東西：`scan-template-leaks.sh` 的 `--only-path` 只比全等檔名，所以
    `--only-path <目錄>` 一個檔案都比不到而回 0 hits；語言政策鏈在工作區外硬失敗，跟
    `style-and-language.md` 寫的英文退路相反。

## [4.16.0] - 2026-08-09

### Changed

- 7dc4ebf: 釋出尾段壓完版之後，不再逼人把全部斷言在一個尾段自己剛造出來的 commit 上重量一次。呼叫者指名那段差異碰得到哪些路徑，可攜層去 git 驗這句話——只碰指名路徑就讓證據延續並把豁免寫進紀錄，碰到指名以外的照舊拒絕並說出是哪個檔案。

## [4.15.0] - 2026-08-09

### Changed

- 9710bfa: 單的身分是它的名字，位置一律當場問
  一張單的位置是它狀態的投影，會隨狀態重算而變。但流程把投影出來的結果存了下來：交付紀錄
  與清單記的是路徑，量測命令的字串裡嵌著路徑。實測存過的 19 條單路徑全部指向已經不存在的
  目錄，268 條量測命令有 54 條寫死機器絕對路徑。
  三件事：`spine-loop-state.sh find <單名>` 吃名字吐它現在住在哪（含沒走過輪次的單，同號
  多張時全部回傳）；交付紀錄與清單改記單的名字；量測登錄在寫入與判定兩端都拒絕把機器路徑
  抄進命令的做法，真的必須指出去的用具名豁免記在那一筆登錄上。
...
- ea28710: 每一張本地開的單自己帶號。號從哪裡來不需要新的宣告：宣告了狀態解析器的命名空間，它的單來自
  外部系統、本地不鑄造（造一個出來，下一個人會拿它去那個系統搜）；沒宣告的，本地就是權威，
  取現有最大號加一。連帶把樹裡五個沒有號的目錄補完，其中兩張改 `.claude/skills/` 的單搬回
  framework——分界是 harness 還是產品，不是內容跟哪一家公司有關。

## [4.12.0] - 2026-08-08

### Changed

- 59bf482: 每一張還沒出去的單都找得到。開發途中長出來的東西一個命令就有載體（`open-seed-issue.sh`），
  它不簽斷言、不開 worktree、不碰手上那張單；開出來就進「接下來做哪一張」的答案，並標明還
  沒簽斷言。位置從六格變七格，多出 `closed`（不做了——放棄、被取代、需求消失），`closed` 與
  `released` 一樣底下多一層日期，查不到日期的進 `closed/undated/`。舊層 541 張全部帶著自己
  寫下的狀態，沒有一個目錄留在判定之外。
...
- edc52e9: 釋出尾段寫完釋出紀錄之後不再死在自己的訊息上：`$version）` 的全形括號會被 bash 收進變數名，
  `set -u` 之下整個尾段停在 tag 與 release 都已送出之後，看起來像「釋出成功但沒收尾」。

## [4.11.0] - 2026-08-08

### Changed

- e1d8d0d: 知識收斂在 skill 自己的目錄：拆掉四份指向沒版控目錄的 resolver，五個開發環境逐個實測後
  入帳，多 repo 的單開工條件改成判宣告的落腳處而不是判當下站在哪，並加一道 pre-push 閘擋住
  「只在寫下它的人的機器上跑得動」的 skill。
...
- b037c34: DP-479：平行工具在脊椎上的可用度——拔掉 11 支，把剩下的斷指標清到 0
  DP-462 依裁示刪掉了那層共用底座。刪是對的，也照做了。**沒做的是第二半**：把引用那層底座的
  20 支改造成自足。三天過去，2026-08-06 實測 34 支 skill 有 23 支指到不存在的東西，其中 19 支
  是 SKILL.md 本體；五份已刪的底座檔仍被 54 個檔案引用。
  最貴的一種失效是安靜的：`review-inbox` 與 `standup` 的散文都寫著「送出前必須通過
  `workspace-language-policy.md` language gate」，而那個檔案不存在——那是一道永遠不會執行的
  把關，讀起來卻很完整。
  **拔除 11 支，35,084 行**（佔 skill 總量 35%），每一支都在單的〈去留宣告〉裡寫下是誰、在
  什麼理由下拔的：`validate`（自己 0 支腳本，references 指向約 40 支已刪腳本）、`converge`、
  `onboard`、`docs-sync`、`my-triage`、`sasd-review`、`unit-test`、`codex-mcp-setup`、
  `sprint-planning`、`use-company`、`intake-triage`。連同 40 個舊層孤兒腳本與 2 支零呼叫的
  孤兒閘。
  剩下的改造到指哪有哪：斷指標 181 → 0，舊交付語彙 16 處 → 2 處（兩處都是宣告過的豁免，理由
  寫在單裡）。同名散文副本收斂成逐位元一致的宣告集合，宣告源只回答「為什麼」，「有哪些副本」
  由檢查腳本從磁碟數出來——寫進宣告源的話，那份清單自己就會變成第二個會漂的來源。
  VR 產出物改成跟著單走（`issues/{命名空間}/{單號}/tests/`），被帶到沒有 `issues/` 的地方時
  建一個以單號命名的目錄，取不到單號就停下來問。
  **順手修掉三個擋住釋出的框架洞**，都不在這張單的凍結區裡，但不修就推不出去：
  - 交付紀錄現在記下它屬於哪個 repo（`delivering_repo`）。`issues/` 是跨 repo 共用的一個
    目錄，所以產品單的交付紀錄躺在框架自己的紀錄旁邊，而它的 head 正確地不在框架 repo——
    閘把那個不存在讀成「陳舊」，從第一次產品交付被記下那一刻起擋住每一次 push。跳過的紀錄
    會被逐筆列印：一道安靜丟掉自己判不了的東西的閘，跟一道判過了說沒問題的閘，長得一樣。
  - pre-push 的六道閘本來解錯 repo 根。git 跑 hook 時環境裡有 `GIT_DIR`，而
    `rev-parse --show-toplevel` 在那個情況下回的是**當下的工作目錄**——於是六道閘拿著一個
    只裝著它們自己的「repo 根」去掃，兩道因為找不到檔案而炸，另外四道安靜地回「0 個檔，
    都對得上」。現在由 hook 解好根往下傳，解錯根要停、不回綠。
  - `mise run bootstrap` / `doctor` / `doctor-mise` 指向的執行檔隨 `onboard` 一起被拔掉。
    那三支是 workspace 的 runtime 工具，被 18 處錯誤訊息與 README 指名，屬於 F-N1 說的「還
    有用的工具」，所以搬進 `framework-release/scripts/` 留著；只有真的隨 skill 退場的
    `onboard-doctor` 與 `cross-runtime-sync` 兩個 task 移除。

## [4.7.0] - 2026-08-03

### Changed

- aa0abc6: DP-478：閘一交一份草案，不交一串問題；環境是宣告出來的列表
  DP-477 把 `grilling` 的「一次一題，等回覆再問下一題」抄進了閘一。**那一條在這裡是錯的**，
  而且它跟這個 repo 自己的規矩衝突——`.claude/rules/style-and-language.md` 寫著「有標準就直接
  套，提一個方案」。DP-477 自己也寫了「What 由 LLM 先推斷一版再拿去確認」，然後只把那條套在
  What 上，其餘三項留成問答。那是不一致，不是設計。
  `grilling` 的情境不一樣：它壓測的是**一個人腦中的計劃**，知識全在人身上，所以只能逐題挖。
  閘一不是——LLM 讀得到整張 Epic、讀得到 repo、讀得到 git 歷史。推得出來的東西做成提問，是把
  自己查得到的事丟回去問，而對一張十幾個 AC 的 Epic 那是幾十輪往返。
  〈怎麼問〉整節改成〈**交一份草案，不交一串問題**〉：整張讀完 → 每一格都先填 → 查得到的
  自己查 → 標好每一格是誰給的 → 人直接在草案上改 → 定版。`grilling` 唯一留下的是**事實與
  決策的分界**，而它不需要靠問答活著——`source` 欄位就是那個分界。看得出哪幾格是我編的，
  比逐題問更快也更誠實。
  順帶讓「要起哪些環境」可驗證。走完 5W1H，人本來就會說出要起哪些東西，但寫成自由文字沒有
  任何東西讀得懂。所以 `how` 多帶一個 `environments` 列表，`check-plan-answers.sh` 掃所有
  `.claude/skills/*/SKILL.md` 找宣告行：
  ```
  <!-- {任意前綴}-ENVIRONMENT-{環境名}: {起它的命令} -->
  ```
  列出來卻找不到任何一份領域知識會起它的，就是**該沉澱一份領域知識的那一個**——而這件事在
  斷言被凍結之前就說得出來，比等 `init` 撞 `POLARIS_SPINE_PACK_UNRESOLVED` 早一站，那時候
  計劃還改得動。字面值 `none` 表示這件工作不需要起任何環境：**那是一個寫下來的答案，不是欄位
  不見**，跟 `not_applicable` 是同一條分界。核心與腳本都不認得任何一個環境的名字，前綴由那份
  知識自己定，跟 `SWE-PRECONDITION` / `SWE-WORKSPACE-IDENTITY` 同一種形狀。
  散文也補上一句「閘一對標整張 Epic」。機制早就在——`frozen-assertion-fence.sh:108` 把檔案裡
  **所有** fence key 掃出來，`assertions_hash` 是一個 map，一張單掛多組具名斷言一直成立。
  缺的只有那句話，於是那件事一路靠記憶，而靠記憶的規則的失效率這個 repo 已經量過了。

## [4.6.0] - 2026-08-03

### Changed

- 269d53d: DP-477：只有人知道的事，問一次、寫下來、之後不再問
  閘一簽的是 AC——「怎麼算成功」。它不問三件同樣只有人知道、而且會決定 AC 本身寫得對不對的
  事：什麼時候要、提出的人真正想解決什麼、拿什麼測。沒被問的那一刻 LLM 不會停下來，它會自己
  填一個；而判定那一站只驗實作有沒有達成那份 AC——AC 若照著一個編出來的意圖寫，整條流程會
  全綠地交付錯的東西。
  `refinement` 多一節〈問出只有人知道的事〉，問法取自
  [mattpocock/skills 的 `grilling`](https://github.com/mattpocock/skills/blob/main/skills/productivity/grilling/SKILL.md)：一次一題等回覆再問下一題、每題附上建議的答案、
  **查得到的事實自己查只有決策才問**、「這一版要做什麼」先推一版再拿去確認。
  答案寫進 `index.md` 的 frontmatter `plan:` 區塊——跟 `destination` 同一個地方，不另開結構。
  每一項帶 `answer` + `source`（`human` / `environment` / `inferred_confirmed`），或明講
  `not_applicable` 並說為什麼。**空著與不適用是兩件事**，這是新的
  `check-plan-answers.sh` 唯一在防的形狀；它掛在 seal 之前，缺項就指名並拒絕。剖析器刻意很窄
  （不拉 YAML 套件，skill 要能被複製到沒有它的地方），所以看不懂的行是**拒絕**而不是略過。
  「測試環境怎麼起」「owner 是誰」「去哪裡問」「部署到哪裡測」這一類每張單答案都相同的，
  不進單，進那個 repo 的領域知識。`POLARIS_SPINE_PACK_UNRESOLVED` 的拒絕訊息現在說得出怎麼
  凝聚一份，不只是說它不在——原本那個拒絕留下的路只剩亂猜一個名字或記成沒有領域，而後者買到
  的是一個永遠不會被檢查的完成條件。**沒有為此開新 skill**：殼已經擁有「哪個領域、載它、
  載不到就拒絕」，閘一已經是人正在被訪談的那一站。
  推不出來的困難用 `record-outreach.sh`：擬稿 → 人確認（要那個人自己說的話）→ 送出 → 回覆
  寫回活文件。**送出動作本身腳本攔不到**，它攔得住的是紀錄——沒有人的原話就記不下送出，
  而一個留不下紀錄的動作事後看起來就是沒發生。跟 `reset --authorization` 同一種形狀：機制讓
  謊變得看得見，不是讓謊不可能。
  殼不知道要問哪幾題，也不知道去哪裡問——這由一條斷言守著。

## [4.5.0] - 2026-08-03

### Changed

- 36bd609: DP-476：一個工作區同時只交付一張單，而工作區身分是一個集合
  2026-08-03 三張單全部在同一份 checkout 上做完，三份交付紀錄釘在同一個 head，最後一起
  出去，釋出時得對兩條已經合出去的分支 force-push。那個局面不是誤操作，是 `init` 只看得到
  它自己那一張單的狀態檔，所以開得出來。
  `init` 現在會掃同一棵單樹：已經走到終局站別的單，若它記下的工作區身分與現在求出的**有
  任何一個交集**，就不開新輪次，並指名是哪一張單、交集落在哪、以及往下走的兩條路（把那張
  單的釋出尾段走完，或換一個工作區）。交集為空不擋——這道閘不因為「這個地方交付過東西」
  就永久封鎖它。單樹的根用 `issues_root_of()` 解（repo 根當權威），不從路徑數層數。
  同時把工作區身分從單一字串放大成**集合**：領域宣告的命令印幾行就記幾個，
  `loop-state.json` 存 `workspace_identity.values`（陣列）。一張單牽涉多個 repo 是這條開發鏈
  一開始就有的前提——交付紀錄早就同時釘兩個 repo 的 head，`assertions_hash` 早就是 map——
  窄口只在 DP-475 那一行 `head -1`。舊的單一 `value` 讀成單成員集合，照樣比對、照樣參與判定。
  `where` 的漂移比對改成**集合相等**（少一個、多一個都算漂，並說出少了與多了哪些）；這跟
  閘的交集比法刻意不同，因為兩者問的不是同一件事。
  `swe-knowledge` 的 `workspace-identity.sh`：`--repo` 可以給很多次，一個 repo 印一行
  `<repo 目錄名>:<分支名>`（帶目錄名是因為兩個 repo 上同名的分支到處都是，否則會塌成一個）；
  任何一個求不出來就整體非 0 而且一行都不印——印一半會讓核心記下一個比實際涵蓋範圍小、
  之後卻永遠自洽的集合。它現在有自己的 selftest。
...
- 915c953: 脊椎三張單一疊落地：
  - **DP-468** 沒有東西是隱形或過期的：忽略規則改成有分類、可機械檢查的宣告；新增
    `gate-dangling-declarations`，讓執行宣告指不到檔案時會紅。
  - **DP-472** 一張單一個家：513 個容器搬進 `issues/{命名空間}/{單}/`，位置由輪次狀態投影，
    歸檔器的根用問的不用數的。
  - **DP-466** 要人簽字的地方，人得簽得下去：連續未收斂與停點都改成一句自然語言就解得開，
    人的原話逐字進 git，歷史一輪不少。
...
- a752be2: DP-464：簽完斷言說一句「開工」，流程自己走到交付紀錄。
  三個入口原本每一個都要人再叫一次。實務上人在閘一簽完斷言之後說的是「開工」，
  期待後續直接到完成，而不是每完成一段就被問「下一步做什麼」。終局定在**交付紀錄
  寫成**，不是「釋出完成」——量過，八支脊椎腳本七支沒有框架相依，只要 bash / git /
  python3；壓版、同步 template、打 tag 那一段是 Polaris 自己的尾段，別人的專案接別的
  東西。
  站別與四種宣告過的停點寫進 `loop-state.json`。`where` 是接手用的視圖；`stop --kind`
  只認 `assertion_wrong` / `surfaced_concern` / `unconverged_cap` / `unauthorized_action`，
  其餘字串拒絕；停著時沒有 `--by <human>` 不能 `advance`。**能停在任何地方的流程需要
  人一直看著，等於沒有自動**——停點事先宣告，人才敢走開。`next` 統一用 `stop:<kind>`
  回答，包含上限那一種；原本回 `escalate` 等於要讀的人自己知道那個字偷偷是四種之一。
  上限那個停點是從 status 推導的，不另存一份：同一個事實兩份紀錄就會漂。
  `record-delivery-intent` 現在逐條檢查 fence 宣告的每個斷言：要有證據、要是
  `run-hardened-oracle.sh` 產的、`verdict` 是 `PASS`、`head_sha` 等於要交付的 head。
  拿它回測上一版就知道這不是假想的洞——七條斷言裡一條完全沒有證據，其餘六條全部量在
  更早的 head 上。在這之前沒有任何東西要求交付前得有證據：這支只重驗 fence，
  `gate-spine-delivery` 自承「檢查的是過期，不是存在」，釋出尾段對 evidence 零引用，
  「judge 說 PASS」一路靠散文帶著走。
  上匝道原本對每個非斜線訊息都問一次立案判斷，包含工作中途的「繼續」，但它承載的斷言
  說的是「**當一件工作進來時**」——實作超出了自己的斷言。代價不只是多幾行字：每輪逼出
  一句形式化的「不用立案」就是用散文應付散文，而且在一條「開工之後不需要人再指定下一
  站」的流程裡每輪重插一個判斷點。改成從磁碟決定：有 source 站在 `work` / `judge` 就
  報站別。判準是**站別**不是 loop status——第一版用 status，於是本單自己站在 judge、
  證據都齊、正等釋出時又收到完整上匝道。
  順手修掉上一版送進 `main` 的兩處 `$VAR<非 ASCII>` 邊界紅。那條紅在 `main` 上單獨跑
  同一支 lint 一樣紅：上一輪只跑針對性 selftest，而**釋出尾段也不跑 PR gate**，兩邊都
  沒跑就沒人跑。
...
- ac73896: DP-462：把第 4 輪的結果寫回活文件與 backlog。
  backlog:243（leak 掃描 source 不對稱）只結一半：主因隨 v3.85.0 解掉，同一筆下面記著的另一個
  獨立成因——worktree 內缺公司設定導致 pattern 沒載入的假性 0 hits——不在射程內，留在原處。
  活文件同時更正自我交付的輪次計數，並記下舊層擋下脊椎的第六道 gate 是腳本清冊 parity。
...
- f4dc936: LOCKED amendment explicit-file authority 與 writer preflight
