---
title: "Polaris 開發者工作流程"
description: "Polaris 從接單到上線的端到端開發者工作流程。"
---
...
`engineering` 是主要的流程協調器。`refinement` 處理 Bug source 診斷，之後交由 `breakdown` 與 `engineering` 執行開發。Feature、Bug 和 Refactor 路徑共用品質檢查 → PR → 發布的尾端流程。
...
## 大型 Stack 與 Sibling Epic

`refinement` 與 `breakdown` 會在 suggested task structure 或 task write 前執行 stacked delivery lens。若某條 lane 已經像 `TXa -> TXb -> TXc`，且第一張 task 是自然的 aggregation / feat branch，planning 必須先處理 sibling Epic 策略，不能直接建立 task.md 或 JIRA child。

當這條 lane 可以獨立 review、merge、release 或 revert 時，正規形狀是：

- 原 Epic 保留為 umbrella，持有 roadmap、跨 lane context 與 aggregate verification。
- 為該 delivery lane 建立 sibling Epic。
- `TXa` 是該 sibling Epic 的 feature branch owner。
- `TXb~TXn` 必須透過 GitHub PR merge event 合入 `TXa`。
- sibling Epic 的 feature PR 在 review 與 CI ready 後再合回產品 base branch。

Polaris 不會只因 lens 命中就自動建立 sibling Epic。所有外部寫入仍需要使用者明確確認並通過正常 write gate。一到兩張 task 的小型 dependency 不會觸發這個 fail-stop。
...
### 步驟 2. 🤖👤 Bug 診斷（refinement）

```text
修 bug PROJ-432
```

AI 執行 `refinement` Bug source mode — **僅處理診斷**，非端到端：
...
> 注意：「fix」+ JIRA key → `refinement` Bug source mode；「fix」+ PR URL → `engineering`（revision mode）
...
3. 帶著新 ticket key 路由到 `refinement` Bug source mode
