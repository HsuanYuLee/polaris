...
# `issues/.../DP-467-.../.spine/evidence/h-n1-check.py`，沒有任何關卡叫它。一支不被呼叫的
...
# 一個東西。純用檔名分組會噴 16 組，其中大半是假的，而一道會噴假紅的關卡會在三次之後被關掉。
# 收窄之後 12 組全是真副本、0 個假陽性（2026-08-12 量的）。
#
# **鍵是檔名，所以同一份東西被抄成另一個名字這道關卡看不見。** 這不是能靠換鍵修掉的——內容
...
# 才是最容易被讀成「掃完了」的那一次。
print(
    f"{prefix} DISCLOSURE 鍵是檔名，所以「同一份東西被抄成另一個名字」這一類這道關卡看不見"
    f"（管轄內 {len(groups)} 個不同名字）。擋這一類的是看 diff 的人，不是這道關卡。"
)
