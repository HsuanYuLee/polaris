...
# skill 目錄之後，那一行算出來的是 `.claude/skills/{那支 skill}`。實測：同一棵樹、同一
...
# 都印 ✅。一個掃不到東西而回綠的掃描，跟一個掃過了沒問題的掃描長得一樣，是這道關卡失效
# 最安靜的方式。
#
...
def skip_path(root: Path, path: Path, source_name: str, gitignored=frozenset()):
    ...
    # declaring company or personal, so neither surface can reach the template no matter
