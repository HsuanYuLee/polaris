...
python3 - "$specs_root" "$tmp_files" <<'PY'
import sys
...
def is_markdown(path: Path) -> bool:
    return path.suffix in {".md", ".mdx", ".markdown", ".mdown", ".mkdn", ".mkd", ".mdwn"}
...
def is_excluded_sidecar(rel: str) -> bool:
    parts = rel.split("/")
    return any(part in {"jira-comments", "escalations", "refinement-inbox", "tests"} for part in parts)

def is_d2_transport(rel: str) -> bool:
    marker = "/artifacts/external-writes/" in f"/{rel}" or "/artifacts/research/" in f"/{rel}"
...
for raw in list_file.read_text(encoding="utf-8").splitlines():
    ...
    required = {"artifact_type", "source", "created"} if is_d2_transport(rel) else {"title", "description"}
...
if failures:
    for rel, missing in failures.items():
        if {"artifact_type", "source", "created"} & set(missing):
            for key in missing:
                print(f"ERROR: D2 transport artifact missing `{key}`: {rel}", file=sys.stderr)
        else:
            for key in missing:
                print(f"ERROR: docs collection page missing `{key}`: {rel}", file=sys.stderr)
    print(f"FAIL: specs collection shape violations: {len(failures)} file(s)", file=sys.stderr)
    raise SystemExit(1)
PY
