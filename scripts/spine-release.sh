...
[[ -f "$RECORD" ]] || die "POLARIS_SPINE_RELEASE_NO_RECORD" \
  ...
  "Run judge, then: bash scripts/record-delivery-intent.sh --source $SOURCE_DIR ..."
...
# from whenever judge ran.
if ! bash "$SCRIPTS/frozen-assertion-fence.sh" verify "$REPO_PATH/$SOURCE_DIR/index.md" >/dev/null 2>&1; then
  die "POLARIS_SPINE_RELEASE_FENCE_UNVERIFIED" \
    "$SOURCE_DIR/index.md no longer matches what was signed; refusing to ship." \
    "  bash scripts/frozen-assertion-fence.sh verify $SOURCE_DIR/index.md"
fi
if ! bash "$SCRIPTS/gates/gate-spine-delivery.sh" --repo "$REPO_PATH" >/dev/null 2>&1; then
  die "POLARIS_SPINE_RELEASE_RECORD_STALE" \
    "the delivery record describes a different commit than HEAD; re-run judge's handoff step."
fi
...
if [[ "$DESTINATION" == "template" ]]; then
  ...
  if [[ "$before" != "$after" ]]; then
    ...
    bash "$SCRIPTS/record-delivery-intent.sh" --source "$SOURCE_DIR" \
