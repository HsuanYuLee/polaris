...
def producer_for(data, marker_kind, writer, owning_skill, path):
    ...
    for producer in data["producers"]:
        ...
        if owning_skill != producer.get("owning_skill"):
            continue
        if any(fnmatch.fnmatch(rel, glob) or fnmatch.fnmatch("./" + rel, glob) for glob in producer.get("path_globs", [])):
            return producer
    return None

def validate_marker(data, path, producer_map_data):
    errors = []
    required = ["schema_version", "marker_kind", "writer", "owning_skill", "source_id", "work_item_id", "status", "freshness"]
    ...
    if not isinstance(data.get("freshness"), dict) or not data.get("freshness"):
        ...
        if not any(key in freshness for key in ("head_sha", "source_artifact", "task_artifact_sha256", "pr_head_sha")):
            errors.append("freshness must include head_sha, source_artifact, task_artifact_sha256, or pr_head_sha")
    ...
    if marker_kind == "dp198_handoff":
        ...
        if not isinstance(data.get("evidence_paths"), list) or not data.get("evidence_paths"):
