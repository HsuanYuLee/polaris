...
#          work arrives, and reports the station instead when one is already
#          past the first gate.
# Inputs:  Hermetic sources/ fixtures under mktemp.
# Outputs: PASS when a cold project gets the full on-ramp, an open source at
#          work or judge gets the station line instead, a source still at assert
...
# assert is the on-ramp itself, so a source sitting there must not suppress it.
proj="$WORK/at-assert"
make_source "$proj" DP-000-selftest assert open
grep -q "先說出立案判斷" <<<"$(run_hook "$proj" "另一件事")" \
  || fail "a source still at assert wrongly suppressed the on-ramp"
...
# on-ramp back to a source that is standing at judge with its evidence complete,
...
proj="$WORK/converged-at-judge"
make_source "$proj" DP-000-selftest judge converged
grep -q "DP-000-selftest 在 judge" <<<"$(run_hook "$proj" "繼續")" \
  || fail "a converged loop at judge was wrongly read as delivered"
echo "  ok  assert and delivered do not count as in flight; a converged loop still does"

# A state written before stations existed does not know where it is, and
# guessing "work" from it would silence the on-ramp on a guess.
proj="$WORK/legacy"
