...
required_cols = ["mechanism", "path", "kind", "runtime", "fallback_script", "governance_role"]
missing = [col for col in required_cols if col not in header]
if missing:
    print(f"runtime annotation table missing columns: {', '.join(missing)}", file=sys.stderr)
    sys.exit(1)
...
for row_no, row in enumerate(rows[1:], start=2):
    ...
    if runtime == "claude-code-only" and role != "ux_enhancement_only":
        if not fallback or fallback == "N/A":
            errors.append(f"row {row_no}: claude-code-only governance mechanism requires fallback_script")
