...
if [[ "$DRY_RUN" == false ]]; then
  ...
  # Polaris backlog
  if [[ -f "$SOURCE_DIR/.claude/polaris-backlog.md" ]]; then
    cp "$SOURCE_DIR/.claude/polaris-backlog.md" "$POLARIS_DIR/.claude/polaris-backlog.md"
    echo "  + .claude/polaris-backlog.md"
  fi
fi
...
if [[ "$DRY_RUN" == false ]]; then
  ...
  for rf in "$POLARIS_DIR/README.md" "$POLARIS_DIR/CHANGELOG.md"; do
    [[ -f "$rf" ]] && TARGETS="$TARGETS"$'\n'"$rf"
  done
