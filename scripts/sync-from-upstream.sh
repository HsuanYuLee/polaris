...
#
# Usage:
#   ./scripts/sync-from-upstream.sh --source ~/work [--dry-run] [--verify]
#
# What it syncs:
...
if [[ "$DRY_RUN" == false ]]; then
  ...
  # Only sync the script itself; genericize-*.sed are instance-specific (generated by /init)
  cp "$SOURCE_DIR/scripts/sync-from-upstream.sh" "$POLARIS_DIR/scripts/sync-from-upstream.sh"
