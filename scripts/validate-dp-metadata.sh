...
VALID_STATUSES = {"SEEDED", "DISCUSSION", "LOCKED", "IMPLEMENTING", "IMPLEMENTED", "ABANDONED", "SUPERSEDED"}
...
def validate_supersession(rows, file, status, supersession):
    if status == "SUPERSEDED" and not supersession:
        add(rows, file, "missing-supersession", "status SUPERSEDED requires supersession summary metadata")
        return
    if not supersession:
        return
    ...
    if not isinstance(successor_ids, list):
        add(rows, file, "invalid-supersession-successor-ids", "successor_ids must be a YAML list or bracket list")
    elif not all(isinstance(item, str) and item.strip() for item in successor_ids):
        add(rows, file, "invalid-supersession-successor-ids", "successor_ids must contain non-empty strings")
    if not isinstance(last_event_at, str) or not last_event_at.strip():
        add(rows, file, "invalid-supersession-last-event", "last_event_at must be a non-empty string")
    if not isinstance(residual_open, bool):
        add(rows, file, "invalid-supersession-residual-open", "residual_open must be true or false")

    if state == "partial" and isinstance(successor_ids, list) and len(successor_ids) == 0:
        add(rows, file, "partial-supersession-missing-successor", "partial supersession requires at least one successor id")
    if status == "SUPERSEDED":
        if state != "full":
            add(rows, file, "superseded-state-mismatch", "status SUPERSEDED requires supersession.state=full")
        if residual_open is not False:
            add(rows, file, "superseded-residual-open", "status SUPERSEDED requires residual_open=false")
        if isinstance(successor_ids, list) and len(successor_ids) == 0:
            add(rows, file, "superseded-missing-successor", "status SUPERSEDED requires at least one successor id")
