#!/usr/bin/env bash
# Purpose: DP-341 T5 one-time migration for active framework refinement.json
#          files. Removes legacy per-task packaging fields
#          (allowed_files / estimate_points) from refinement.json so task.md is
#          the only packaging authority.
# Inputs:  --workspace-root <root> [--dry-run]
# Outputs: MIGRATED / NO-OP / DEFERRED lines. Stale refinement-vs-task.md
#          mismatches are reported explicitly and migrated in favor of task.md.
...
# Validate with the implementation checkout's validator, not necessarily the
# target workspace root. During DP-341 T5 the target workspace may still be on
# main, whose validator has not yet learned the post-DP-341 intent-only shape.
VALIDATOR="$DEFAULT_ROOT/scripts/validate-refinement-json.sh"

python3 - "$WORKSPACE_ROOT" "$VALIDATOR" "$DRY_RUN" <<'PY'
import json
import os
import re
import subprocess
