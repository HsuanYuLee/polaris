...
run_delayed_terminal_archive_case() {
  local tmp repos repo template dp_dir archived_dp_dir branch task_md moved_task_md wt task_head workspace_commit template_commit evidence
  tmp="$(mktemp -d -t framework-closeout-delayed-archive.XXXXXX)"
  trap 'rm -rf "$tmp"' RETURN
  repos="$(make_repos "$tmp")"
  repo="$(printf '%s\n' "$repos" | sed -n '1p')"
  template="$(printf '%s\n' "$repos" | sed -n '2p')"
...
- [ ] T1: First task — `tasks/T1.md`
- [ ] Manual audit item that is not tied to a task
...
path = Path(sys.argv[1])
text = path.read_text(encoding="utf-8")
text = text.replace("- [ ] Manual audit item that is not tied to a task", "- [x] Manual audit item that is not tied to a task")
path.write_text(text, encoding="utf-8")
PY
