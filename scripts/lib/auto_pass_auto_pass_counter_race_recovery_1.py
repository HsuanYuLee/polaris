...
for key in ("engineering_to_breakdown", "breakdown_to_refinement_inbox"):
    ...
    actual = (
        engineering_to_breakdown_actual
        if key == "engineering_to_breakdown"
...
if validate_script.is_file():
    result = subprocess.run(
        ["bash", str(validate_script), str(tmp_path)], capture_output=True, text=True
    )
    if result.returncode != 0:
        tmp_path.unlink(missing_ok=True)
        print(
            f"POLARIS_COUNTER_RECOVERY_PRECONDITION_FAILED: new ledger failed validation:\n{result.stderr}",
            file=sys.stderr,
        )
        sys.exit(1)
...
tmp_stamp.write_text(
    json.dumps(stamp_payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8"
)
