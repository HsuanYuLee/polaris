...
stage, source_id, work_item_id, head_sha, ledger_arg, resolver_path, pr_state_file = (
    sys.argv[2:9]
)
...
def emit_tool_missing(reason, evidence_path=None):
    ...
    print(
        f"POLARIS_TOOL_MISSING:gh review-state unavailable ({reason}){detail}",
        file=sys.stderr,
    )
    raise SystemExit(3)
...
def review_state_route(state_file_arg):
    ...
    if has_ownership_payload:
        ...
        if gate.returncode != 0:
            ...
            reason = (
                detail[0]
                if detail
                else f"auto-pass PR ownership gate exit {gate.returncode}"
            )
    ...
    if not readiness and not revision_class:
        if has_ownership_payload:
            return None
        emit_tool_missing(
            "pr-state file has no readiness_state/revision_class", state_path
        )
...
if stage == "source":
    ...
    try:
        refinement = json.loads(
            (container / "refinement.json").read_text(encoding="utf-8")
        )
    except Exception as exc:
        emit(
            "BLOCKED",
...
if stage == "breakdown":
    for subdir, terminal, action, default_reason in (
        ...
        if path.is_file():
            ...
            reason = (
                marker_reason
                if isinstance(marker_reason, str) and marker_reason.strip()
                else default_reason
            )
    ...
    if inbox_matches:
        ...
        emit(
            ...
            "refinement_amendment",
            inbox_matches[0],
            "refinement inbox present (amendment loop)",
        )
...
if stage == "engineering":
    if not head_sha:
        emit(
            ...
            "blocked",
            None,
            "engineering probe requires --head-sha",
        )
...
if stage == "verify-AC":
    if not head_sha:
        emit(
            ...
            "blocked",
            None,
            "verify-AC probe requires --head-sha",
        )
    spec_issue = (
        evidence / "ac-verification" / f"spec-issue-{work_item_id}-{head_sha}.json"
    )
    if spec_issue.is_file():
        ...
        emit(
            status_of(spec_issue) or "ROUTE_BACK_AMEND",
            None,
            "refinement_amendment",
            spec_issue,
            "verify-AC spec issue (amendment loop)",
        )
