...
def validate_source(container, source_id, refinement_hash_value, errors):
    ...
    if (
        ...
        or not refinement_md.is_file()
        or not refinement_json.is_file()
    ):
        return
    status = frontmatter(index_path).get("status")
    if status != "LOCKED":
        errors.append(
            f"source index.md status must be LOCKED, got {status or 'missing'}"
        )
    try:
    ...
    if refinement_hash_value != actual_hash:
        errors.append(
            "source.refinement_hash is stale or does not match refinement artifacts"
        )
...
if is_jira_source:
    if status_record is None:
        ...
        if (
            ...
            and source.get("id")
            and record_source_id != source.get("id")
        ):
            errors.append(
        ...
        if status_record.get("granted_at"):
            parse_iso(
                status_record.get("granted_at"),
                "jira_status_consent_record.granted_at",
                errors,
            )
else:
...
if pause is not None:
    if not isinstance(pause, dict):
        ...
        if (
            ...
            and terminal_status != "paused_for_user_external_write"
        ):
            errors.append(
                "paused_for_user_external_write pause requires matching terminal_status"
            )
        if kind == "session_handoff":
            if terminal_status not in (None, ""):
                errors.append(
                    "session_handoff pause is non-terminal and requires terminal_status=null"
                )
    ...
    if not isinstance(friction_log, list):
        ...
        for idx, entry in enumerate(friction_log):
            ...
            if not stage:
                ...
                errors.append(
                    f"{prefix}.stage must be one of {sorted(FRICTION_STAGE_ENUM)}"
                )
            ...
            if not kind:
                ...
                errors.append(
                    f"{prefix}.friction_kind must be one of {sorted(FRICTION_KIND_ENUM)}"
                )
