"""Structured validator authority extracted from scripts/validate-specs-bound-write-contract.sh."""
...
for raw in raw_files:
    ...
    producer = next(
        (
            p
            for p in producers
            if any(fnmatch.fnmatch(path_rel, glob) for glob in p.get("path_globs", []))
        ),
        None,
    )
    if producer is None:
