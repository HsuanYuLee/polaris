...
def categorize_command(cmd: str):
    ...
    if any(
        k in lower
        for k in ["eslint", "prettier --check", "check-lint-baseline", "stylelint"]
    ) or re.search(r"\blint\b", lower):
        return "lint"
    ...
    if lower.startswith(
        ("echo ", "if ", "for ", "while ", "case ", "export ")
    ) or re.match(r"^[a-z_][a-z0-9_]*=", lower):
        return "setup"
    ...
    for path in files:
        ...
        for job_name, job in pipeline.items():
            ...
            for cmd in flatten_commands(job.get("commands")):
                category = categorize_ci_command(
                    cmd, str(path.relative_to(repo)), job_name
                )
                checks.append(
                    {
...
def discover_gitlab_ci():
    ...
    for job_name, job in payload.items():
        ...
        for cmd in flatten_commands(script):
            category = categorize_ci_command(
                cmd, str(path.relative_to(repo)), str(job_name)
            )
            checks.append(
                {
                    "source_file": str(path.relative_to(repo)),
...
provider = (
    discover_woodpecker()
    or discover_github_actions()
    or discover_gitlab_ci()
    or {
        "provider": "unknown",
        "files": [],
        "checks": [],
