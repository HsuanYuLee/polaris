"""Structured validator authority extracted from scripts/validate-breakdown-ready.sh."""
...
check_verify_command_executability = (
    script_dir / "lib" / "check-verify-command-executability.sh"
)
...
def validate_delivery_unit_shape(
    target: Path, files: list[Path]
) -> tuple[str, str] | None:
    ...
    """
    task_shapes = [
        parse_task_shape(file) for file in files if task_id_for_file(file) is not None
    ]
    if not task_shapes:
        return None
...
def validate_package_graph_scope(
    file: Path, text: str, allowed: list[str]
) -> list[str]:
    errors: list[str] = []
    ...
    touches_package_json = any(
        entry.endswith("package.json") for entry in normalized_allowed
    )
    mentions_package_graph = any(hint in text.lower() for hint in PACKAGE_GRAPH_HINTS)

    if (
        touches_pnpm_catalog or (touches_package_json and mentions_package_graph)
    ) and not allowed_contains(allowed, "pnpm-lock.yaml"):
        errors.append(
...
def validate_test_command_debug_hygiene(file: Path, text: str) -> list[str]:
    ...
    is_nuxt_vitest_app_command = bool(
        ...
        or (
            re.search(r"\bvitest\b", command_space)
            and ("apps/main" in command_space or "nuxt" in text_lower)
        )
    )
...
def validate_command_executability(file: Path, text: str) -> list[str]:
    ...
    for heading in ("## Verify Command", "## Test Command"):
        ...
        proc = subprocess.run(
            [
                "bash",
                str(check_verify_command_executability),
                "--label",
                f"{file}:{heading}",
            ],
            input=command,
...
def validate_branch_identity_gate(file: Path, text: str) -> list[tuple[str, str]]:
    ...
    detail = (
        ...
        or "resolve-task-branch.sh validate_branch rejected the Task branch"
    )
    return [
        (
...
def validate_verify_command_specificity(file: Path, text: str) -> list[str]:
    ...
    scans_dependency_or_bundle = bool(
        re.search(
            r"\b(package\.json|pnpm-lock\.yaml|pnpm-workspace\.yaml|\.output/|node_modules/moment|/moment@|moment/min|moment/locale)\b",
            verify_command,
        )
    )
...
def validate_scope_trace(
    ...
    if missing_columns:
        errors.append(
            f"{file}: Scope Trace Matrix missing columns: {', '.join(missing_columns)}"
        )
        return errors
    ...
    for row_number, row in enumerate(rows[1:], start=2):
        ...
        if not goal or goal in {"-", "--"}:
            errors.append(
                f"{file}: Scope Trace Matrix row {row_number} must include Goal / AC"
            )
        owning_files = split_path_cells(owning_raw)
        if not owning_files:
            errors.append(
                f"{file}: Scope Trace Matrix row {row_number} must include owning files"
            )
        if not surface or surface in {"-", "--", "N/A", "n/a"}:
            errors.append(
                f"{file}: Scope Trace Matrix row {row_number} must include surface/boundary"
            )
        if not tests or tests in {"-", "--", "N/A", "n/a"}:
            errors.append(
                f"{file}: Scope Trace Matrix row {row_number} must include tests"
            )
        for owning_file in owning_files:
            if not path_token(owning_file):
                errors.append(
                    f"{file}: Scope Trace Matrix row {row_number} owning file is not a path/glob token: {owning_file}"
                )
                continue
            ...
            if not allow_uncovered and not path_covered(owning_file, allowed):
                errors.append(
                    f"{file}: Scope Trace Matrix row {row_number} owning file is not covered by Allowed Files: {owning_file}"
                )
        if needs_render_surface(goal, surface) and not has_render_api_surface(
            owning_files, surface
        ):
            errors.append(
...
def validate_one(file: Path) -> list[str]:
    ...
    if schema.returncode != 0:
        errors.append(
            f"{file}: validate-task-md.sh failed; fix schema before readiness handoff"
        )
    ...
    for entry in allowed:
        if not path_token(entry):
            errors.append(
                f"{file}: Allowed Files entry is not a machine-matchable path/glob token: {entry}"
            )

    is_dp_task = (
        "/design-plans/DP-" in normalized or "| source type | dp |" in text.lower()
    )
    if is_dp_task and not is_carve_out_shape:
        non_spec_allowed = [
            entry
            for entry in allowed
            if not normalize_path_token(entry).startswith(
    ...
    errors.extend(
        validate_scope_trace(file, text, allowed, allow_uncovered=is_carve_out_shape)
    )
    ...
    if not matrix.strip():
        ...
        pass_idx = next(
            (
                ...
                if "pass condition" in cell or "通過條件" in cell
            ),
            2,
        )
        owner_idx = next(
        ...
        for gate in ("scope", "test", "verify", "ci-local"):
            ...
            if not has_meaningful_cell(row, pass_idx):
                errors.append(
                    f"{file}: Gate Closure Matrix '{gate}' row must include a pass condition"
                )
            if not has_meaningful_cell(row, owner_idx):
                errors.append(
                    f"{file}: Gate Closure Matrix '{gate}' row must include owner/decision"
                )
        if "n/a" in lower and not re.search(r"n/a.{3,}", lower):
            errors.append(
                f"{file}: Gate Closure Matrix N/A entries must include a reason"
            )
...
if target.is_dir() and validate_task_md_deps.exists():
    ...
    if deps.returncode != 0:
        all_errors.append(
            f"{target}: validate-task-md-deps.sh failed; fix dependency closure before readiness handoff"
        )

if (
    all_errors
    or executability_violations
    or branch_name_violations
    or branch_identity_violations
):
    print("validate-breakdown-ready.sh FAIL", file=sys.stderr)
    ...
    if executability_violations or branch_name_violations or branch_identity_violations:
        ...
        for offending_file in dict.fromkeys(
            file for file, _ in executability_violations
        ):
            print(
                f"{VERIFY_COMMAND_NOT_EXECUTABLE_MARKER}:{offending_file}",
                file=sys.stderr,
            )
