"""Structured validator authority extracted from scripts/validate-engineering-self-review-result.sh."""
...
def validate_canonical_result_path(path, data, evidence_dir, errors, label):
    ...
    if resolved.parent != evidence_dir.resolve():
        errors.append(
            f"{label} must live in canonical engineering-self-review evidence dir"
        )
    expected = (
...
def validate_transition(current, prior, errors, label="prior"):
    ...
    if prior.get("reviewed_head_sha") == current.get("reviewed_head_sha") and prior.get(
        "reviewed_state_sha256"
    ) == current.get("reviewed_state_sha256"):
...
def validate_shape(data, errors, label="result"):
    ...
    if review_round == 1:
        ...
        if not isinstance(prior_file, str) or not re.fullmatch(
            ...
            errors.append(
                f"{label}.prior_result_file must be a canonical result basename"
            )
...
if args.print_current_state:
    ...
    try:
        ...
        print(
            f"POLARIS_ENGINEERING_SELF_REVIEW_STATE_UNAVAILABLE:{exc}", file=sys.stderr
        )
        raise SystemExit(2)
    raise SystemExit(0)
...
validate_canonical_result_path(args.result, data, result_evidence_dir, errors, "result")
...
if type(review_round) is int and review_round > 1:
    if not args.prior:
        ...
        if Path(args.prior).is_file() and data.get(
            "prior_result_sha256"
        ) != file_sha256(args.prior):
