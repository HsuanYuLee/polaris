...
gap_preflight_raw = os.environ.get("GAP_PREFLIGHT_OUTPUT_RAW", "")
gap_authority = None
if gap_preflight_raw:
    try:
        gap_preflight = json.loads(gap_preflight_raw)
        gap_authority = gap_preflight.get("delegation_authority")
    except Exception:
        gap_authority = None
...
def emit(payload):
    payload.setdefault("delegation_authority", gap_authority)
...
def list_v_entries(directory):
    ...
    for child in sorted(directory.iterdir()):
        ...
        if child.is_file() and child.suffix == ".md" and V_STEM_RE.match(child.stem):
            ...
            child.is_dir()
            and V_STEM_RE.match(child.name)
...
def required_manifest_v_ids(container):
    ...
    if (
        ...
        or strategy.get("mode") != "source_level_v_required"
    ):
        return []
...
def list_t_entries(directory):
    ...
    for child in sorted(directory.iterdir()):
        ...
        if child.is_file() and child.suffix == ".md" and T_STEM_RE.match(child.stem):
            ...
            child.is_dir()
            and T_STEM_RE.match(child.name)
            and (child / "index.md").is_file()
        ):
            entries.append(child)
    return entries
...
def source_context(repo_path, scripts_dir):
    ...
    if proc.returncode != 0:
        ...
        return None, gate_blocked(
            "source context: resolver failed: "
    ...
    if not container.is_dir():
        return None, gate_blocked(
            "source context: resolved container missing", container
        )
    dp_match = re.match(r"^(DP-\d{3})(?:-|$)", container.name)
    dp_prefix = (
        dp_match.group(1)
        if (dp_match and container.parent.name == "design-plans")
        else None
    )
    return {
        "container": container,
...
def terminal_complete_v_gate(repo_path, scripts_dir):
    ...
    missing_manifest_vs = [
        stem
        for stem in required_manifest_vs
    ...
    remaining_t = [
        ...
        and frontmatter_field(t_entry_status_file(entry), "task_shape")
        not in T_CARVE_OUT_SHAPES
    ]
    if remaining_t:
...
def map_next_action(
    probe_status, probe_terminal, probe_next, probe_evidence, probe_reason
):
    ...
    if probe_status == "PASS":
        ...
        if stage == "engineering":
            ...
            if next_work_item:
                next_skill = (
                    "verify-AC"
                    if re.search(r"-V\d+$", next_work_item)
                    else "engineering"
                )
                return {
                    "status": "PASS",
...
if mapped.get("terminal_status") == "complete":
    ...
    gate_result = terminal_complete_v_gate(
        Path(repo).resolve(), Path(ledger_validator).parent
    )
...
payload = {
    ...
    "source_id": source_id,
    "stage": stage,
    "delegation_authority": gap_authority,
}
