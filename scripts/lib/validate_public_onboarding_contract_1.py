"""Structured validator authority extracted from scripts/validate-public-onboarding-contract.sh."""
...
try:
    ...
    print(
        f"ERROR: PyYAML is required to validate public onboarding contract: {exc}",
        file=sys.stderr,
    )
    raise SystemExit(2)
...
if minimum.get("node"):
    for rel in technical_docs:
        require(
            rel, "Node >= 22.12.0 prerequisite", r"Node(?:\.js)?[^\n|,;。]*22\.12\.0"
        )
