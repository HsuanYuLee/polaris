...
if not has_anything:
    ...
    parts.append('python3 - "$EVIDENCE_PATH" "$BRANCH" "$HEAD_SHA" "$EVENT" "$BASE_BRANCH" "$SOURCE_BRANCH" "$REF" <<\'CILOCAL_NCC_PY\'')
    parts.append('import json, sys, datetime')
    parts.append('path, branch, sha, event, base_branch, source_branch, ref = sys.argv[1:8]')
    ...
    if codecov_gates:
        ...
        parts.append('    return matched, files_with_coverage_data, covered, total')
        parts.append('')
        parts.append('flag_results = []')
        parts.append('for gate in gates:')
        parts.append('    flag = gate.get("flag")')
        parts.append('    matched, files_with_coverage_data, covered, total = compute(gate.get("include_paths", []), gate.get("exclude_paths", []))')
