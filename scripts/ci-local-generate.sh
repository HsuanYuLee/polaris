...
has_anything = bool(checks) or bool(policy_checks) or bool(forced_skip_checks) or bool(dev_hooks)
...
if not has_anything:
    ...
    parts.append("print(f\"[ci-local] {overall}: {summary['failed_checks']} check failures (evidence: {ev_path})\")")
