...
parts.append("# Location: <repo>/.claude/scripts/ci-local.sh (gitignored via .git/info/exclude).")
...
parts.append("")
parts.append('# Locate repo root (this script lives in <repo>/.claude/scripts/).')
parts.append('# Prefer git rev-parse so the script is depth-independent; fall back to')
parts.append('# `../..` from the script directory if git is unavailable (e.g., bare invocation).')
