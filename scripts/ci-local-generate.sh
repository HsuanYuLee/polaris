...
parts.append("# ci-local.sh — Tool-agnostic local mirror of repo CI checks (DP-032 D12 + DP-079).")
...
parts.append("#   bash <company>/polaris-config/<project>/generated-scripts/ci-local.sh --repo <wt>    # validates worktree <wt>")
...
if not has_anything:
    ...
    parts.append('  if [ -n "${CI_LOCAL_DEBUG+x}" ]; then')
    parts.append('    CI="$COMMAND_CI" TZ="$COMMAND_TZ" DEBUG="$COMMAND_DEBUG" bash -lc "$(cat "$cmd_file")" >"$out_file" 2>&1 || rc=$?')
    parts.append('  else')
    parts.append('    env -u DEBUG CI="$COMMAND_CI" TZ="$COMMAND_TZ" bash -lc "$(cat "$cmd_file")" >"$out_file" 2>&1 || rc=$?')
    parts.append('  fi')
    ...
    if codecov_gates:
        ...
        parts.append('')
        parts.append('def normalize_coverage_path(root, sf):')
        parts.append('    p = Path(sf)')
        parts.append('    if p.is_absolute():')
        parts.append('        try:')
        parts.append('            return p.resolve().relative_to(root.resolve()).as_posix()')
        parts.append('        except Exception:')
        parts.append('            raw = p.as_posix()')
        parts.append('            marker = "/apps/"')
        parts.append('            idx = raw.find(marker)')
        parts.append('            if idx != -1:')
        parts.append('                return raw[idx + 1:]')
        parts.append('            return raw')
        parts.append('    return p.as_posix()')
        parts.append('')
        parts.append('def line_coverage_from_v8(file_cov):')
        parts.append('    line_cov = {}')
        parts.append('    statement_map = file_cov.get("statementMap") or {}')
        parts.append('    counts = file_cov.get("s") or {}')
        parts.append('    for sid, rng in statement_map.items():')
        parts.append('        count = counts.get(sid, 0)')
        parts.append('        start = ((rng.get("start") or {}).get("line"))')
        parts.append('        end = ((rng.get("end") or {}).get("line"))')
        parts.append('        if not start or not end:')
        parts.append('            continue')
        parts.append('        for line in range(int(start), int(end) + 1):')
        parts.append('            line_cov[line] = bool(line_cov.get(line)) or count > 0')
        parts.append('    return line_cov')
        parts.append('')
        parts.append('def partial_branch_lines_from_v8(file_cov):')
        parts.append('    partial = set()')
        parts.append('    branch_map = file_cov.get("branchMap") or {}')
        parts.append('    branch_counts = file_cov.get("b") or {}')
        parts.append('    statement_map = file_cov.get("statementMap") or {}')
        parts.append('    statement_counts = file_cov.get("s") or {}')
        parts.append('    line_stmt_count = {}')
        parts.append('    for sid, rng in statement_map.items():')
        parts.append('        line = ((rng.get("start") or {}).get("line"))')
        parts.append('        if line:')
        parts.append('            line_stmt_count[int(line)] = max(line_stmt_count.get(int(line), 0), statement_counts.get(sid, 0) or 0)')
        parts.append('    for bid, branch in branch_map.items():')
        parts.append('        counts = branch_counts.get(bid) or []')
        parts.append('        loc = branch.get("loc") or {}')
        parts.append('        line = ((loc.get("start") or {}).get("line")) or branch.get("line")')
        parts.append('        if not counts or not line:')
        parts.append('            continue')
        parts.append('        line = int(line)')
        parts.append('        if any((c or 0) == 0 for c in counts):')
        parts.append('            partial.add(line)')
        parts.append('            continue')
        parts.append('        stmt_count = line_stmt_count.get(line, 0)')
        parts.append('        if stmt_count > 0 and max(counts) < stmt_count:')
        parts.append('            partial.add(line)')
        parts.append('    return partial')
        parts.append('')
        parts.append('def parse_v8_coverage(root, flag):')
        parts.append('    candidates = []')
        parts.append('    for path in root.rglob("coverage-final.json"):')
        parts.append('        if path.parent.name == flag:')
        parts.append('            candidates.append(path)')
        parts.append('    if not candidates:')
        parts.append('        candidates = list(root.rglob("coverage-final.json"))')
        parts.append('    data = {}')
        parts.append('    for coverage_json in candidates:')
        parts.append('        try:')
        parts.append('            raw = json.loads(coverage_json.read_text(encoding="utf-8", errors="ignore"))')
        parts.append('        except Exception:')
        parts.append('            continue')
        parts.append('        for sf, file_cov in raw.items():')
        parts.append('            rel = normalize_coverage_path(root, sf)')
        parts.append('            data[rel] = {')
        parts.append('                "line_cov": line_coverage_from_v8(file_cov),')
        parts.append('                "partial_branch": partial_branch_lines_from_v8(file_cov),')
        parts.append('            }')
        parts.append('    return data')
        parts.append('')
        parts.append('lcov_map = parse_lcov(repo)')
        parts.append('v8_maps = {str(g.get("flag")): parse_v8_coverage(repo, str(g.get("flag"))) for g in gates}')
        parts.append('')
        ...
        parts.append('')
        parts.append('def is_test_only_file(path):')
        parts.append('    name = Path(path).name')
        parts.append('    parts = set(Path(path).parts)')
        parts.append('    return (')
        parts.append('        name.endswith((".test.ts", ".test.tsx", ".test.js", ".test.jsx",')
        parts.append('                       ".tests.ts", ".tests.tsx", ".tests.js", ".tests.jsx",')
        parts.append('                       ".spec.ts", ".spec.tsx", ".spec.js", ".spec.jsx",')
        parts.append('                       ".specs.ts", ".specs.tsx", ".specs.js", ".specs.jsx"))')
        parts.append('        or "__tests__" in parts')
        parts.append('        or "tests" in parts')
        parts.append('    )')
        parts.append('')
        parts.append('def resolve_coverage_entry(path, coverage_map, prefixes):')
        parts.append('    entry = coverage_map.get(path)')
        parts.append('    cov_key = path if entry is not None else None')
        parts.append('    if entry is None:')
        parts.append('        for pre in prefixes:')
        parts.append('            if path.startswith(pre + "/"):')
        parts.append('                stripped = path[len(pre)+1:]')
        parts.append('                entry = coverage_map.get(stripped)')
        parts.append('                if entry is not None:')
        parts.append('                    cov_key = stripped')
        parts.append('                    break')
        parts.append('    if entry is None:')
        parts.append('        for k, v in coverage_map.items():')
        parts.append('            if k.endswith(path) or path.endswith(k):')
        parts.append('                entry = v')
        parts.append('                cov_key = k')
        parts.append('                break')
        parts.append('    return entry, cov_key')
        parts.append('')
        parts.append('def compute(include, exclude, flag):')
        ...
        parts.append('    v8_map = v8_maps.get(str(flag)) or {}')
        parts.append('    coverage_source = "v8" if v8_map else "lcov"')
        parts.append('    for f in matched:')
        parts.append('        lines = changed.get(f, set())')
        parts.append('        if v8_map:')
        parts.append('            v8_entry, v8_key = resolve_coverage_entry(f, v8_map, prefixes)')
        parts.append('            if v8_entry is not None and v8_key != f:')
        parts.append('                path_mismatch_files.append({"changed_file": f, "coverage_file": v8_key})')
        parts.append('            if not v8_entry:')
        parts.append('                continue')
        parts.append('            files_with_coverage_data.append(f)')
        parts.append('            line_cov = v8_entry.get("line_cov") or {}')
        parts.append('            partial_branch = v8_entry.get("partial_branch") or set()')
        parts.append('            for ln in lines:')
        parts.append('                if ln not in line_cov:')
        parts.append('                    continue')
        parts.append('                total += 1')
        parts.append('                if line_cov.get(ln) and ln not in partial_branch:')
        parts.append('                    covered += 1')
        parts.append('            continue')
        ...
        parts.append('    return matched, files_with_coverage_data, path_mismatch_files, covered, total, coverage_source')
        parts.append('')
        parts.append('flag_results = []')
        parts.append('for gate in gates:')
        parts.append('    flag = gate.get("flag")')
        parts.append('    matched, files_with_coverage_data, path_mismatch_files, covered, total, coverage_source = compute(gate.get("include_paths", []), gate.get("exclude_paths", []), flag)')
        ...
        parts.append('            "total_lines": total, "coverage_percent": pct, "coverage_source": coverage_source,')
        ...
        parts.append('                 "coverage_percent": pct, "coverage_source": coverage_source, "matched_files": matched,')
        ...
        parts.append('            if matched and all(is_test_only_file(f) for f in matched):')
        parts.append('                entry["status"] = "SKIP"; entry["reason"] = "test_only_patch_no_instrumented_lines"')
        parts.append('            else:')
        ...
        parts.append('                            g.get("reason") in ("no_instrumented_patch_lines", "test_only_patch_no_instrumented_lines")')
        parts.append('                            for g in explicit)')
        parts.append('    all_test_only_no_lines = all(g.get("status")=="SKIP" and')
        parts.append('                                  g.get("reason")=="test_only_patch_no_instrumented_lines"')
        parts.append('                                  for g in explicit)')
        ...
        parts.append('    if all_skip_no_lines and has_matched and not has_coverage_data and not all_test_only_no_lines:')
