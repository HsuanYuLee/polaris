    ...
    if not assets.is_dir():
        ...
        for child in assets.iterdir():
            if child.is_dir() and child.name not in ALLOWED_ASSET_DIRS:
                errors.append(fail(root, f"unknown assets subdir: assets/{child.name}/"))
        ...
        try:
            ...
            if not isinstance(manifest, dict):
                ...
                if manifest.get("schema_version") != 1:
                    errors.append(fail(root, "publication-manifest.json schema_version must be 1"))
                if not isinstance(manifest.get("artifacts"), list):
                    errors.append(fail(root, "publication-manifest.json artifacts must be an array"))
        except Exception as exc:
