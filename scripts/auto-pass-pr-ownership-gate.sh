...
python3 - "$STATE_FILE" "$READ_STDIN" <<'PY'
import json
import sys
...
try:
    raw = sys.stdin.read() if read_stdin else Path(state_file).read_text(encoding="utf-8")
    data = json.loads(raw)
...
def present_pass(value):
    ...
    if isinstance(value, dict):
        status = value.get("status")
        if isinstance(status, str) and status.strip().upper() in {"PASS", "PRESENT", "OK", "FOUND"}:
            return True
        if value.get("present") is True or value.get("pass") is True or value.get("found") is True:
            return True
    return False
...
draft_value = first(("isDraft",), ("is_draft",), ("draft",), ("pr", "isDraft"), ("pull_request", "isDraft"))
...
if publisher not in ALLOWED_PUBLISHERS:
    blocked(OWNERSHIP, f"publisher must be polaris-pr-create.sh, got {publisher or '<missing>'}")
...
no_bypass_required = as_bool(first(("engineering_no_bypass_required",), ("no_bypass_required",)))
if no_bypass_required:
    required = [
        ("task.md lineage", first(("task_md_lineage",), ("lineage", "task_md"), ("lineage",))),
        ("resolver lock", first(("resolver_lock",), ("work_order_lock",))),
        ("readiness pack snapshot", first(("readiness_pack_snapshot",), ("baseline_snapshot",), ("planner_owned_baseline_snapshot",))),
        ("skill boundary marker", first(("skill_boundary_marker",), ("skill_workflow_boundary_marker",), ("boundary_marker",))),
    ]
    for label, value in required:
        if not present_pass(value):
            blocked(OWNERSHIP, f"{label} must be present/PASS for engineering no-bypass")
