...
latest_evidence_from_log() {
  python3 "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/lib/ci_ci_local_run_1.py" "$1"
}

evidence_status() {
  python3 "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/lib/ci_ci_local_run_2.py" "$1"
}

evidence_identity() {
  python3 "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/lib/ci_ci_local_run_3.py" "$1"
}

emit_blocked_env_payload() {
  local evidence_path="$1"
  shift
  python3 "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/lib/ci_ci_local_run_4.py" "$evidence_path" "$@"
}
