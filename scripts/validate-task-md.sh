...
# Product tasks use JIRA keys (PROJ-123). Source-backed work orders use
# pseudo IDs ({SOURCE}-T1 / {SOURCE}-V1), including DP-backed and Bug-backed
# sources, but otherwise follow the same task.md schema.
# ---------------------------------------------------------------------------
is_valid_source_work_item_id() {
  local value="$1"
  [[ "$value" =~ ^[A-Z][A-Z0-9]*-[0-9]+-[TV][0-9]+[a-z]*$ ]]
}

is_valid_task_identity() {
  local value="$1"
  [[ "$value" =~ ^[A-Z][A-Z0-9]*-[0-9]+$ ]] || is_valid_source_work_item_id "$value"
}
...
validate_file() {
  ...
  if [[ -n "$header_task_token" ]]; then
    if ! is_valid_task_identity "$header_task_token"; then
      errors+=("invalid Task identity in metadata line: got '$header_task_token' (expected JIRA key like PROJ-123 or source work item like DP-047-T1 / BUG-123-T1)")
    fi
    ...
    if [[ -z "$header_jira_token" ]]; then
      ...
      errors+=("invalid task identity in metadata line: got '$header_jira_token' (expected JIRA key like PROJ-123 or legacy source work item like DP-047-T1 / BUG-123-T1)")
    fi
  fi
  ...
  if grep -qF "## Operational Context" "$FILE"; then
    ...
    if [[ -z "$task_identity" ]]; then
      ...
      errors+=("Operational Context task identity has invalid value '$task_identity' (expected JIRA key like PROJ-123 or source work item like DP-047-T1 / BUG-123-T1)")
    fi
    ...
    if [[ "$canonical_identity" -eq 1 ]]; then
      ...
      case "$source_type" in
        dp|jira|bug) ;;
        *) errors+=("Operational Context canonical identity requires Source type = dp|jira|bug (got: '${source_type:-<empty>}')") ;;
      esac
      if [[ -z "$source_id" ]]; then
