...
# 反向對照組在 selftest 的 fixture 上，不在這棵樹上。
...
# 道關卡在三次之內被關掉，連它本來判得到的那些也一起沒了（assertion A-N3 就是為了擋這件事）。
...
#
# 全樹目前**一筆都沒有**（2026-08-10 量的：收緊之後零筆新紅），所以反向對照組在 selftest 的
...
# 真的程式碼剝掉、然後把對的散文判紅——那是這道關卡最不該犯的錯（assertion A-N3）。
