...
# Polaris deterministic gates for every Git client and agent runtime.
#
...
#   bash scripts/install-git-hooks.sh --status    # check status

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
ROOT_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
GIT_HOOKS_DIR="$ROOT_DIR/.git/hooks"

POLARIS_MARKER="# [polaris-git-hooks]"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      echo "Usage: bash scripts/install-git-hooks.sh [--remove|--status]"
      exit 0
      ;;
    *)
