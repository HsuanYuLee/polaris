...
python3 - "$ROOT_DIR" "$REGISTRY" "$SOURCE_CLOSEOUT" <<'PY'
import json
import re
import subprocess
import sys
...
allowed_source_kinds = {
    "artifact", "api_response", "command_exit", "file_state", "git_state",
    "lifecycle_state", "structured_payload",
}
required_exclusions = {
    "research", "exploration", "design", "code_generation", "prose_generation",
    "intent_clarification", "business_judgment",
}
...
if not isinstance(data, dict):
    print("POLARIS_SKILL_FLOW_TRANSITION_REGISTRY_INVALID:registry root 必須是 object", file=sys.stderr)
    raise SystemExit(2)
...
try:
    ...
    print(f"POLARIS_SKILL_FLOW_TRANSITION_REGISTRY_INVALID:無法讀取 script manifest：{exc}", file=sys.stderr)
    raise SystemExit(2)
manifest_rows = manifest.get("scripts") if isinstance(manifest, dict) else None
if not isinstance(manifest_rows, list):
    print("POLARIS_SKILL_FLOW_TRANSITION_REGISTRY_INVALID:script manifest scripts 必須是 array", file=sys.stderr)
    raise SystemExit(2)
...
if aggregate_result.returncode != 0:
    ...
    print(f"POLARIS_SKILL_FLOW_TRANSITION_REGISTRY_INVALID:無法解析 aggregate enrollment：{detail}", file=sys.stderr)
    raise SystemExit(2)
enrolled_selftests = {
...
for index, transition in enumerate(transitions):
    ...
    if not isinstance(transition_id, str) or not re.fullmatch(r"[a-z0-9_]+(?:\.[a-z0-9_]+)+", transition_id):
    ...
    for group in observation_groups:
        ...
        for observation_index, observation in enumerate(observations):
            ...
            if source_kind not in allowed_source_kinds:
                errors.append(f"{observation_prefix}.source_kind 無法機械觀測：{source_kind}")
            ...
            if observation.get("mechanically_decidable") is not True:
                errors.append(f"{observation_prefix}.mechanically_decidable 必須是 true")
    ...
    for field in ("path", "protocol", "selector", "output"):
        if not isinstance(callable_interface.get(field), str) or not callable_interface[field].strip():
    ...
    if source_closeout:
        ...
        forbidden_source_fields = sorted(
            field for field in ("source_type", "dp_only", "jira_only")
    ...
    for role, owned_path in (("producer", producer_value), ("validator", validator_value)):
        ...
        if len(matches) != 1:
            errors.append(f"{prefix}.{role} 在 script manifest 必須恰有一筆 exact-path enrollment")
            continue
    ...
    if not isinstance(consumer_value, str) or consumer_value not in enrolled_selftests:
        errors.append(f"{prefix}.consumer 必須存在於 aggregate runner canonical --list 集合")
    if blocking_value != consumer_value:
        errors.append(f"{prefix}.blocking_invoke_point 必須等於已 enrollment 的 consumer selftest")
...
if errors:
    for error in errors:
        print(f"POLARIS_SKILL_FLOW_TRANSITION_REGISTRY_INVALID:{error}", file=sys.stderr)
    raise SystemExit(2)
profile = " + DP-422 source closeout" if source_closeout else ""
print(f"PASS: skill-flow transition registry ({len(transitions)} transition(s){profile})")
PY
