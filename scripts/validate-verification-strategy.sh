...
if strategy is None:
    print(f"validate-verification-strategy.sh PASS - no verification_strategy present ({path})")
    raise SystemExit(0)
if not isinstance(strategy, dict):
    fail("POLARIS_VERIFICATION_STRATEGY_INVALID", "verification_strategy must be an object")
...
v_tasks = [task for task in tasks if isinstance(task, dict) and short_task_id(task).startswith("V")]
t_tasks = [task for task in tasks if isinstance(task, dict) and short_task_id(task).startswith("T")]
...
if mode == "per_task_self_verify":
    for idx, task in enumerate(t_tasks):
        ...
        verify_command = verification.get("verify_command") if isinstance(verification, dict) else None
