...
<!-- 來源：.claude/instructions/manifest.yaml 的目標 copilot -->
<!-- 重新產生：bash .claude/instructions/compile.sh --target copilot -->
...
**任何會改變程式碼或行為的請求，第一站是 `refinement`。** 使用者不需要知道這個名字，也不會
說出來——「幫我做 X」「這個壞了要修」「想重構 Y」都算。`refinement` 被叫起來的第一件事是
判斷這件事要不要立案並把依據說出口，不是動手寫斷言。

**只讀的問題不走這條。**「這支腳本在幹嘛」「查一下 X」沒有「怎麼算成功」需要人簽，
直接回答。

立案之後三站：`refinement` 簽下成功的定義 → `engineering` 施工 → `verify-ac` 判定。頭尾兩個閘在，
中間才可以很隨便。
...
要開新的 source、發明新機制、加新結構之前，先回答「現在有什麼在管這件事」並把它用盡。
...
在 `.claude/rules/style-and-language.md`，那是唯一一份常駐規則。

## Copilot

這份指令只為這個 workspace 產生。產品 repo 的 `.github/**` 屬於那個 repo，不由這裡修改。

**Copilot 沒有 hook。** 任何靠 hook 擋下來的東西在這邊都不存在，包含語言檢查。
